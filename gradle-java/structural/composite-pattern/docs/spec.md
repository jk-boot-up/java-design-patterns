# Composite Pattern — Project Specification
...
[`composite-pattern-explained.md`](composite-pattern-explained.md); the problem it
...
- Not a catalog engine. The tree is built in code; a real one comes from a database and needs paging.
- Not a claim that every hierarchy should be a composite. The written notes cover the cost: a shared interface that leaves half its methods meaningless on a leaf is a warning sign, not a success.
...
7 production classes under `com.jk.explore.composite`:
...
1. **No `instanceof` and no casting in client code.** This is checkable by grep and is the single clearest signal that the pattern is doing its job.
2. **The recursion lives in `Category`**, not in the caller. A client that walks children itself has taken the responsibility back.
3. **Depth is unbounded**, and the demo tree is nested deeply enough — three levels — that a design only working one level down would fail.
...
- `./gradlew build` passes. 13 test methods across `CatalogDemoTest`, `CategoryTest`, `NaiveCatalogPrinterTest`, `ProductTest`.
...
| `composite-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/composite-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Composite pattern in
...
| Runtime | ~7:41 over 15 scenes, 107 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Composite Pattern in Java - Nested Category Trees"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Decorator.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py composite`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py composite
python3 ../../docs/make_specs.py composite
```
