#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Composite pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Composite Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("a leaf and"):
...
def kind_console(scene, img, d):
    ...
    for ln in lines:
        ...
        if ln.startswith("$"):
            ...
            col = GREEN
        elif ln.startswith("Total") or ln.startswith("Product"):
            col = CLIENT
...
def kind_diagram(scene, img, d):
    ...
    # The component interface, shared by both other roles.
    d.rounded_rectangle([120, 240, W - 120, 380], radius=18,
                        outline=ACCENT, width=3)
    d.text((150, 258), "COMPONENT", font=f(SANS_B, 22), fill=ACCENT)
    box(d, 760, 288, 400, 90, "CatalogComponent", "«interface»", ACCENT)

    # Two implementers: the leaf and the composite.
    arrow(d, 900, 380, 480, 480, LINE)
    arrow(d, 1020, 380, 1440, 480, LINE)

    box(d, 280, 480, 400, 110, "Product", "the Leaf", GREEN)
    box(d, 1240, 480, 400, 110, "Category", "the Composite", CLIENT)

    # Category holds a list of CatalogComponent children. Drawn as a small
    # self-loop off its right edge, since a child may be another Category --
    # the tree nests without any new type or new code.
    d.line([(1640, 505), (1740, 505)], fill=LINE, width=2)
    d.line([(1740, 505), (1740, 555)], fill=LINE, width=2)
    arrow(d, 1740, 555, 1640, 555, LINE, 2)
    d.text((1240, 615), "children: List<CatalogComponent>",
           font=f(SANS, 24), fill=MUTED, anchor="la")
    d.text((1240, 648), "a child may itself be a Category",
           font=f(SANS, 24), fill=MUTED, anchor="la")

    box(d, 280, 650, 400, 110, "NaiveCatalogPrinter", "«the trap»", ROSE)
    d.text((280, 780), "asks instanceof Product / instanceof Category,\nrepeated per operation",
           font=f(SANS, 22), fill=MUTED)

    d.text((W // 2, 900),
           "A Category's children are typed as CatalogComponent, not as Product or Category.",
           font=f(SANS_B, 32), fill=TEXT, anchor="ma")
    d.text((W // 2, 956),
           "That is the only reason the tree can nest arbitrarily deep with zero new code.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "COMPOSITE", font=f(SANS_B, 128), fill=TEXT)
    ...
    # The naive instanceof branch vs. the one uniform polymorphic call.
    pill(d, 150, 556, 700, 170, RED, "instanceof Category",
         "repeated per operation", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "child.totalPrice()",
         "same call, leaf or branch", tag="AFTER")
    ...
    d.text((W // 2, 758), "One tree. One interface. Zero instanceof.",
