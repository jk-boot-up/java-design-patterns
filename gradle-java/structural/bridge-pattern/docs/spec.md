# Bridge Pattern — Project Specification
...
[`bridge-pattern-explained.md`](bridge-pattern-explained.md); the problem it
...
- Not a template for every pair of varying concepts. The written notes are clear that the pattern earns its keep when both dimensions really do keep growing, and is over-engineering when one of them is fixed.
...
13 production classes under `com.jk.explore.bridge`:
...
1. **A notification holds a `MessageChannel`, never a concrete channel.** If `OrderConfirmationNotification` names `EmailChannel`, the two hierarchies are still fused.
2. **Adding a channel changes no notification class**, and adding a notification changes no channel class. This is the property that distinguishes the pattern from a plain interface.
3. **Channels are testable without I/O**, via a recording channel the tests supply in place of a real one.
...
- `./gradlew build` passes. 11 test methods across `EmailChannelTest`, `NaiveNotificationsTest`, `NotificationDemoTest`, `NotificationTest`, `PushChannelTest`, `SmsChannelTest`.
...
| `bridge-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/bridge-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Bridge pattern in
...
| Runtime | ~7:45 over 15 scenes, 112 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Bridge Pattern in Java - Notifications and Channels"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Composite.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py bridge`. They are the one part of
...
integrated, -3.91 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py bridge
python3 ../../docs/make_specs.py bridge
```
