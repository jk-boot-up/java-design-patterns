#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Bridge pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Bridge Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("two small"):
...
def kind_code(scene, img, d):
    ...
    for ln in lines:
        ...
        if s.startswith("//"):
            ...
            col = AMBER
            fnt = f(MONO_B, size)
        elif s.startswith("private") or s.startswith("static"):
            col = MUTED
...
def box(d, x, y, w, h, label, sub, colour):
    ...
    d.text((x + w // 2, y + h // 2 - 26), label, font=f(SANS_B, 30),
           fill=TEXT, anchor="ma")
...
def kind_diagram(scene, img, d):
    ...
    # Two independent hierarchies, side by side, connected by one bridge line.
    d.text((280, 250), "ABSTRACTION", font=f(SANS_B, 24), fill=GREEN)
    box(d, 200, 290, 480, 100, "Notification", "«abstract» holds a channel", GREEN)

    d.text((1240, 250), "IMPLEMENTOR", font=f(SANS_B, 24), fill=ACCENT)
    box(d, 1200, 290, 480, 100, "MessageChannel", "«interface»", ACCENT)

    # The bridge itself: one horizontal line, labelled, connecting the two boxes.
    arrow(d, 680, 340, 1190, 340, CLIENT, 4)
    d.text((935, 300), "channel", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")

    # Refined abstractions underneath Notification.
    arrow(d, 300, 390, 260, 490, LINE)
    arrow(d, 440, 390, 440, 490, LINE)
    arrow(d, 580, 390, 620, 490, LINE)
    box(d, 100, 490, 280, 100, "OrderConfirmation", "refined", GREEN)
    box(d, 400, 490, 280, 100, "ShippingUpdate", "refined", GREEN)
    box(d, 700, 490, 280, 100, "PasswordReset", "refined", GREEN)

    # Concrete implementors underneath MessageChannel.
    arrow(d, 1260, 390, 1130, 490, LINE)
    arrow(d, 1440, 390, 1410, 490, LINE)
    arrow(d, 1620, 390, 1690, 490, LINE)
    box(d, 1000, 490, 260, 100, "EmailChannel", "concrete", ACCENT)
    box(d, 1280, 490, 260, 100, "SmsChannel", "concrete, truncates", ACCENT)
    box(d, 1560, 490, 260, 100, "PushChannel", "concrete, drops body", ACCENT)

    box(d, 400, 650, 480, 100, "NaiveOrderConfirmationSms", "«the trap»", ROSE)
    d.text((400, 780),
           "no shared implementor type -- every combination\nre-derives its own delivery logic",
           font=f(SANS, 22), fill=MUTED)

    d.text((W // 2, 900),
           "Notification never subclasses per channel -- it holds one, by composition.",
           font=f(SANS_B, 32), fill=TEXT, anchor="ma")
    d.text((W // 2, 956),
           "That one field is the entire bridge between two hierarchies that never meet again.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "BRIDGE", font=f(SANS_B, 128), fill=TEXT)
    ...
    # The naive N x M explosion vs. one abstraction, any implementor.
    pill(d, 150, 556, 700, 170, RED, "OrderConfirmationSms",
         "one class per combination", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "new Notification(channel)",
         "any content, any channel", tag="AFTER")
    ...
    d.text((W // 2, 758), "Two hierarchies. One field. Independent variation.",
