#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Proxy with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Proxy with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # A call from outside versus a call on this -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "this.render(sku)",
         "the shopper gets the image", tag="SKIPPED")
    pill(d, 1070, 556, 700, 170, GREEN, "catalogue.render(sku)",
         "the shopper is refused", tag="CHECKED")
    ...
    d.text((W // 2, 744), "Spring writes the proxy.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Calls must go through it.",
