# Proxy with Spring Pattern — Project Specification
...
[`proxy-with-spring-pattern-explained.md`](proxy-with-spring-pattern-explained.md); the problem it
...
10 production classes under `com.jk.explore.proxyspring`:
...
1. **Loads are counted,** never timed.
2. **The refusal is asserted** with `assertThrows` on the proxy.
3. **The skipped checks are asserted,** so a Spring change that fixes them fails the test.
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `ProxyBeanTest`.
...
| `proxy-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/proxy-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Proxy with Spring pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Observer with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py proxy-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py proxy-with-spring
python3 ../../docs/make_specs.py proxy-with-spring
```
