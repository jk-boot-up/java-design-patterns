# Proxy Pattern — Project Specification
...
[`proxy-pattern-explained.md`](proxy-pattern-explained.md); the problem it
...
- Not an image library. Loading is simulated and counted so the saving is visible.
- Not an authorisation framework. The role check is deliberately trivial; real systems need policies, not an enum comparison.
- Not a dynamic-proxy tutorial. `java.lang.reflect.Proxy` and the frameworks built on it are mentioned in the written notes and not used, because the hand-written version is what makes the mechanism visible.
...
8 production classes under `com.jk.explore.proxy`:
...
1. **Both proxies implement `ProductImage`.** A proxy with its own interface is a wrapper, and the client would have to know which it holds.
2. **The real image is constructed at most once**, however many times the lazy proxy is asked, with a load counter proving it.
3. **Denial happens before any load.** A protection proxy that fetches the image and then refuses to show it has leaked the expensive work and, on a metered backend, the fact of the request.
...
- `./gradlew build` passes. 17 test methods across `HighResolutionProductImageTest`, `LazyProductImageTest`, `NaiveAdminImageViewerTest`, `NaiveProductListingTest`, `ProductImageDemoTest`, `RestrictedProductImageTest`.
...
| `proxy-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/proxy-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Proxy pattern in
...
| Runtime | ~7:43 over 14 scenes, 114 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Proxy Pattern in Java - Lazy and Guarded Images"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Strategy.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py proxy`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py proxy
python3 ../../docs/make_specs.py proxy
```
