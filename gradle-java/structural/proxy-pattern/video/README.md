# Proxy Pattern — Teaching Video

A narrated, slide-based video that teaches the Proxy pattern using this
...
| File | What it is |
| --- | --- |
| `proxy-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `proxy-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `proxy-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
The final mux is deliberate too: the file is written with `+faststart` so
the index sits at the front and the poster paints the moment the file is
opened, and the AAC priming edit list is dropped so the video track starts
at 0.000 alongside the audio rather than 21 ms late. Without either,
players show black at 0:00. If you change the ffmpeg calls, check with
`ffprobe -show_entries stream=start_time` that both streams still start at
zero.

The narration is also cleaned up before it is encoded, and that matters more
than it sounds. Every voice macOS ships by default is the compact 22 kHz
tier, and resampling it straight to the 48 kHz the AAC track needs leaves an
audible hiss. The `CLEANUP` filter chain in `build_video.sh` resamples with a long filter so the
...
| 2 | The Scenario | A product listing where loading an image is expensive and not everyone may see one |
| 3 | Where Those Checks End Up | Why eager loading and inline role checks land in every caller |
| 4 | The Naive Approach | `NaiveProductListing`'s eager loop and `NaiveAdminImageViewer`'s copy-pasted check |
| 5 | Why That Hurts | Wasted loads, duplicated access rules, and the screen that forgets one |
| 6 | The Pattern | The formal definition, in plain language |
| 7 | The Bouncer on the Door | How to remember it forever |
| 8 | The Four Roles | Subject, real subject, proxies, client |
| 9 | The Subject | `ProductImage` and `HighResolutionProductImage` side by side |
| 10 | The Virtual Proxy | `LazyProductImage` — build it late, build it once, keep cheap questions cheap |
| 11 | The Protection Proxy | Why `RestrictedProductImage` takes a `ProductImage`, and why the check comes first |
| 12 | Running It | The real program output, and what the load counts prove |
| 13 | Wrap Up | When to use it, and the one sentence to remember |
...
**Upload `proxy-pattern-explained.mp4`, with `poster.png` as the thumbnail
...
`RestrictedProductImage`. Uploading `proxy-pattern-explained.srt` gives
...
> Learn the Proxy design pattern in Java 21 by building a product listing
...
> and at what actually separates Proxy from Decorator. No prior
> design-pattern knowledge needed. Full source code and written notes are
> in the repository.
