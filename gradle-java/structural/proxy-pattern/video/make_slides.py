#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Proxy pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Proxy Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("wrap it"):
...
def kind_diagram(scene, img, d):
    ...
    # Subject interface at top, the shared shape the real thing and every
    # stand-in implement.
    d.text((790, 250), "SUBJECT", font=f(SANS_B, 24), fill=GREEN)
    box(d, 660, 290, 400, 100, "ProductImage", "«interface»", GREEN)

    d.text((140, 250), "CLIENT", font=f(SANS_B, 24), fill=CLIENT)
    box(d, 80, 490, 380, 100, "ProductImageDemo", "holds only a ProductImage", CLIENT)
    arrow(d, 270, 490, 720, 395, CLIENT, 4)

    d.text((1400, 250), "REAL SUBJECT", font=f(SANS_B, 24), fill=ACCENT)
    box(d, 1240, 490, 460, 100, "HighResolutionProductImage", "expensive to construct", ACCENT)
    arrow(d, 1360, 490, 1000, 395, ACCENT, 4)

    # The two proxies, both implementing ProductImage and both holding one.
    box(d, 300, 700, 460, 100, "LazyProductImage", "«virtual», builds on first use", ROSE)
    arrow(d, 530, 700, 750, 400, ROSE)

    box(d, 1000, 700, 560, 100, "RestrictedProductImage",
        "«protection», checks role first", ROSE)
    arrow(d, 1280, 700, 970, 400, ROSE)

    # The seam: a proxy holds a ProductImage, so one proxy can stand in front of
    # another exactly as easily as it stands in front of the real subject.
    arrow(d, 1000, 750, 775, 750, ROSE, 3)
    d.text((880, 648), "wraps a ProductImage", font=f(SANS_B, 22), fill=ROSE,
           anchor="ma")

    box(d, 1600, 700, 280, 100, "NaiveProductListing", "«the trap»", AMBER)

    d.text((W // 2, 900),
           "Every proxy implements exactly ProductImage, and returns exactly what the real image would return.",
           font=f(SANS_B, 30), fill=TEXT, anchor="ma")
    d.text((W // 2, 954),
           "Nothing is added. Only when the call happens, and whether it happens at all, is decided.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "PROXY", font=f(SANS_B, 128), fill=TEXT)
    ...
    # Eagerly building the real image vs. a stand-in that decides when, and
    # whether, the expensive call happens at all.
    pill(d, 150, 556, 700, 170, RED, "new HighResolutionImage()",
         "loaded whether it is seen or not", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "new LazyProductImage(sku)",
         "loaded on first render, once", tag="AFTER")
    ...
    d.text((W // 2, 758), "Same interface. It controls the door.",
