# Adapter Pattern — Project Specification
...
[`adapter-pattern-explained.md`](adapter-pattern-explained.md); the problem it
...
- Not a real carrier integration. `AcmeShippingSdk` is a stand-in for a third-party jar, and is deliberately awkward in the way real ones are.
- Not a units library. The conversions are arithmetic in one method because that is where the lesson is.
...
8 production classes under `com.jk.explore.adapter`:
...
1. **The unit conversion appears exactly once**, inside the adapter. Two copies of a pounds-to-kilograms factor is the bug the pattern prevents.
2. **`CheckoutService` never mentions Acme.** It depends on `ShippingRateProvider` and cannot tell which implementation it holds.
3. **The adapter and the flat-rate provider are interchangeable**, demonstrated by running checkout against both.
...
- `./gradlew build` passes. 11 test methods across `AcmeShippingAdapterTest`, `AcmeShippingSdkTest`, `CheckoutServiceTest`, `FlatRateShippingProviderTest`, `NaiveShippingCodeTest`, `ShippingDemoTest`.
...
| `adapter-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/adapter-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Adapter pattern in
...
| Runtime | ~7:07 over 14 scenes, 102 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Adapter Pattern in Java - Wrapping a Shipping SDK"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Bridge.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py adapter`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py adapter
python3 ../../docs/make_specs.py adapter
```
