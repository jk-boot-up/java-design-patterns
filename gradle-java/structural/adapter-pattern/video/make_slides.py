#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Adapter pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Adapter Pattern  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # Client on the left, calling only the target interface.
    d.text((140, 250), "CLIENT", font=f(SANS_B, 24), fill=CLIENT)
    box(d, 80, 290, 340, 100, "CheckoutService", "calls only the target", CLIENT)

    d.text((620, 250), "TARGET", font=f(SANS_B, 24), fill=GREEN)
    box(d, 560, 290, 400, 100, "ShippingRateProvider", "«interface»", GREEN)

    arrow(d, 420, 340, 555, 340, CLIENT, 4)

    # Two implementations of the target: the adapter, and a native one.
    arrow(d, 700, 390, 590, 490, LINE)
    arrow(d, 820, 390, 1000, 490, LINE)
    box(d, 380, 490, 420, 100, "AcmeShippingAdapter", "«adapter»", ACCENT)
    box(d, 850, 490, 460, 100, "FlatRateShippingProvider", "native, no adapting", GREEN)

    d.text((1440, 250), "ADAPTEE", font=f(SANS_B, 24), fill=AMBER)
    box(d, 1300, 290, 480, 100, "AcmeShippingSdk", "«adaptee», third-party", AMBER)

    # The seam: the adapter's field pointing at the adaptee it wraps.
    arrow(d, 720, 490, 1420, 395, AMBER, 4)
    d.text((1080, 430), "sdk", font=f(SANS_B, 24), fill=AMBER, anchor="ma")

    box(d, 100, 650, 480, 100, "NaiveCheckoutService", "«the trap»", ROSE)
    d.text((100, 780),
           "calls AcmeShippingSdk directly -- the pounds/cents\nconversion is duplicated at every call site",
           font=f(SANS, 22), fill=MUTED)

    d.text((W // 2, 900),
           "CheckoutService never imports AcmeShippingSdk -- only ShippingRateProvider.",
           font=f(SANS_B, 32), fill=TEXT, anchor="ma")
    d.text((W // 2, 956),
           "One adapter class is the entire seam between two shapes that were never designed together.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "ADAPTER", font=f(SANS_B, 128), fill=TEXT)
    ...
    # Coupled to the third-party SDK directly vs. one adapter, one seam.
    pill(d, 150, 556, 700, 170, RED, "sdk.fetchCostInCents(...)",
         "coupled at every call site", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "provider.quoteRate(zip, kg)",
         "adapted or native, no difference", tag="AFTER")
    ...
    d.text((W // 2, 758), "One class translates. Nothing else has to.",
