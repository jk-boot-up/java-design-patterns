#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Facade pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Facade Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("one simple"):
...
def kind_console(scene, img, d):
    ...
    for ln in lines:
        ...
        if ln.startswith("$"):
            col = MUTED
        elif ln.startswith("Inventory"):
            col = GREEN
        elif ln.startswith("Payment"):
            col = AMBER
        elif ln.startswith("Shipping"):
            col = ROSE
        elif ln.startswith("Notification"):
            col = CYAN
        elif ln.startswith("Order placed"):
            col = CLIENT
        elif ln.startswith(" "):
            col = MUTED
...
def kind_diagram(scene, img, d):
    ...
    box(d, 760, 250, 400, 110, "FacadeDemo", "the Client", CLIENT)
    arrow(d, 960, 360, 960, 430, LINE)
    d.text((980, 380), "placeOrder(request)", font=f(SANS, 24), fill=MUTED)

    d.rounded_rectangle([120, 430, W - 120, 800], radius=18,
                        outline=ACCENT, width=3)
    d.text((150, 448), "FACADE", font=f(SANS_B, 22), fill=ACCENT)

    box(d, 760, 480, 400, 110, "OrderFacade", "one simple method", ACCENT)

    subs = [
        ("InventoryService", "reserveStock", GREEN),
        ("PaymentService", "charge", AMBER),
        ("ShippingService", "scheduleShipment", ROSE),
        ("NotificationService", "sendOrderConfirmation", CYAN),
    ]
    centres = [175 + 190 + 400 * i for i in range(4)]

    # vertical stub down from the facade, then a horizontal bus, then a
    # straight drop into each subsystem -- no crossing lines
    bus_y = 625
    d.line([(960, 590), (960, bus_y)], fill=LINE, width=2)
    d.line([(centres[0], bus_y), (centres[-1], bus_y)], fill=LINE, width=2)

    for (name, meth, col), cx in zip(subs, centres):
        arrow(d, cx, bus_y, cx, 660, LINE, 2)
        box(d, cx - 190, 660, 380, 110, name, meth, col)

    d.text((W // 2, 850),
           "The facade knows the subsystems.  The subsystems never know the facade.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")
    d.text((W // 2, 906),
           "That one-way arrow is what keeps every service independently reusable.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "FACADE", font=f(SANS_B, 128), fill=TEXT)
    ...
    # The four subsystem calls OrderFacade.placeOrder makes, in order.
    pill(d, 150, 556, 700, 170, RED, "reserve, charge, ship, notify",
         "every caller repeats them", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "facade.placeOrder(r)",
         "one call, done right", tag="AFTER")
    ...
    d.text((W // 2, 758), "One door in front of many",
