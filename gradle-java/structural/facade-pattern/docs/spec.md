# Facade Pattern — Project Specification
...
[`facade-pattern-explained.md`](facade-pattern-explained.md); the problem it
...
- Not a transaction manager. A real place-order needs compensation when step three fails after step two charged the card; the written notes raise this and the code does not solve it.
- Not a service layer template. A facade that grows business rules of its own has stopped being a facade, and the notes say where that line is.
...
8 production classes under `com.jk.explore.facade`:
...
1. **The facade adds no business rules.** It sequences calls and moves data between them; a discount calculation appearing here means the responsibility has drifted.
2. **The subsystems remain usable on their own.** Nothing is made package-private to force callers through the facade — the facade is a convenience, not a wall.
3. **The ordering constraint is enforced in one place**: stock is reserved before the card is charged, and that fact lives in the facade rather than in every caller.
...
| `facade-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/facade-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Facade pattern in
...
| Runtime | ~7:16 over 15 scenes, 106 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Facade Pattern in Java - One Call to Place an Order"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Flyweight.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py facade`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py facade
python3 ../../docs/make_specs.py facade
```
