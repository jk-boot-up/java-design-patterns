# Decorator Pattern — Project Specification
...
[`decorator-pattern-explained.md`](decorator-pattern-explained.md); the problem it
...
- Not a pricing engine. Real checkout pricing has tax interactions and ordering rules this deliberately does not model.
- Not a claim that decorators are free. The written notes cover the cost: a deep stack is hard to debug, and the object you hold is no longer the one you created.
...
10 production classes under `com.jk.explore.decorator`:
...
1. **A decorator is itself a `PricedItem`.** That is what lets decorators wrap decorators, and it is the whole mechanism.
2. **Each fee is defined in exactly one class.** If the gift-wrap fee appears twice, the pattern has been applied to the shape but not to the duplication.
3. **Decorators stack in any order**, with a test that builds the same set of extras two ways and asserts the price agrees.
...
- `./gradlew build` passes. 12 test methods across `DecoratorStackingTest`, `ExpressHandlingDecoratorTest`, `GiftWrapDecoratorTest`, `InsuranceDecoratorTest`, `NaiveProductClassesTest`, `PricingDemoTest`, `ProductTest`.
...
| `decorator-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/decorator-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Decorator pattern in
...
| Runtime | ~6:56 over 14 scenes, 101 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Decorator Pattern in Java - Gift Wrap and Insurance"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Facade.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py decorator`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py decorator
python3 ../../docs/make_specs.py decorator
```
