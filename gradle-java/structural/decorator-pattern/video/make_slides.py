#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Decorator pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Decorator Pattern  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # Component interface at top, the shared shape everything implements.
    d.text((760, 250), "COMPONENT", font=f(SANS_B, 24), fill=GREEN)
    box(d, 660, 290, 400, 100, "PricedItem", "«interface»", GREEN)

    d.text((140, 250), "CONCRETE COMPONENT", font=f(SANS_B, 24), fill=CLIENT)
    box(d, 80, 490, 380, 100, "Product", "plain, no extras", CLIENT)
    arrow(d, 270, 490, 720, 395, CLIENT, 4)

    d.text((1440, 250), "ABSTRACT DECORATOR", font=f(SANS_B, 24), fill=ACCENT)
    box(d, 1240, 490, 460, 100, "ProductDecorator", "«decorator», holds wrapped", ACCENT)
    arrow(d, 1300, 490, 900, 395, ACCENT, 4)

    # The seam: the decorator's field pointing back at PricedItem (itself or another).
    arrow(d, 1350, 490, 1000, 340, ACCENT, 3)
    d.text((1150, 420), "wrapped", font=f(SANS_B, 22), fill=ACCENT, anchor="ma")

    # Three concrete decorators, each extending the abstract decorator.
    labels = [
        ("GiftWrapDecorator", "+ $3.50 flat fee", 80),
        ("InsuranceDecorator", "+ 2% of wrapped.cost()", 560),
        ("ExpressHandlingDecorator", "+ $9.99 flat fee", 1090),
    ]
    for label, sub, x in labels:
        box(d, x, 700, 420, 100, label, sub, ROSE)
        arrow(d, x + 210, 700, 1440, 590, LINE)

    box(d, 1590, 700, 260, 100, "NaiveGiftWrapped\nInsuredProduct".replace("\n", " "),
        "«the trap»", AMBER)

    d.text((W // 2, 900),
           "Every decorator implements exactly PricedItem -- cost() and description(), nothing more.",
           font=f(SANS_B, 30), fill=TEXT, anchor="ma")
    d.text((W // 2, 954),
           "That is what lets GiftWrapDecorator wrap Product, or wrap InsuranceDecorator, or be wrapped itself.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "DECORATOR", font=f(SANS_B, 128), fill=TEXT)
    ...
    # A hardcoded combination class vs. stacked decorators, no new class needed.
    pill(d, 150, 556, 700, 170, RED, "NaiveGiftWrappedInsured...",
         "one class per combination", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "new Insurance(new GiftWrap(p))",
         "stack any combination, any order", tag="AFTER")
    ...
    d.text((W // 2, 758), "Wrap it, don't subclass it.",
