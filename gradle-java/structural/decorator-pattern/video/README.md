# Decorator Pattern — Teaching Video

A narrated, slide-based video that teaches the Decorator pattern using this
...
| File | What it is |
| --- | --- |
| `decorator-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `decorator-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `decorator-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Scenario | Checkout pricing with optional, combinable extras |
| 3 | One Class Per Combination | Why the naive fix already needs three classes for two features |
| 4 | The Naive Approach | `NaiveGiftWrappedInsuredProduct`'s duplicated fee calculations |
| 5 | Why That Hurts | Combinatorial class growth and duplicated fee logic |
| 6 | The Pattern | The formal definition, in plain language |
| 7 | Dressing for Weather | How to remember it forever |
| 8 | The Four Roles | Component, concrete component, decorator, concrete decorators |
| 9 | The Component | `PricedItem` and `Product` side by side |
| 10 | A Concrete Decorator | `InsuranceDecorator` — delegate, then add |
| 11 | Stacking Order | Why gift-wrap-then-insure differs from insure-then-gift-wrap |
| 12 | Running It | The real program output, every stacking combination |
...
**Upload `decorator-pattern-explained.mp4`, with `poster.png` as the
...
`ProductDecorator`. Uploading `decorator-pattern-explained.srt` gives
