# Flyweight Pattern — Project Specification
...
[`flyweight-pattern-explained.md`](flyweight-pattern-explained.md); the problem it
...
- Not a general memory-optimisation guide. Flyweight is worth reaching for when profiling has shown duplicated immutable state, and the written notes are blunt that applying it speculatively costs clarity for nothing.
- Not a cache. The factory's map is a pool of shared values with a fixed key space, not an eviction policy.
...
6 production classes under `com.jk.explore.flyweight`:
...
1. **`BadgeStyle` is immutable.** A shared mutable object is not an optimisation, it is a defect affecting every listing at once.
2. **The factory returns the identical instance** for a repeated type, asserted with `assertSame` rather than `assertEquals` — equality would pass even if nothing were shared.
3. **No extrinsic state is stored on the flyweight.** The moment a `BadgeStyle` knows which listing it belongs to, it cannot be shared.
...
- `./gradlew build` passes. 15 test methods across `BadgeDemoTest`, `BadgeStyleFactoryTest`, `BadgeStyleTest`, `CatalogBadgeTest`, `NaiveListingBadgeTest`.
...
| `flyweight-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/flyweight-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Flyweight pattern in
...
| Runtime | ~7:48 over 15 scenes, 111 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Flyweight Pattern in Java - Sharing Badge Styles"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Proxy.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py flyweight`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py flyweight
python3 ../../docs/make_specs.py flyweight
```
