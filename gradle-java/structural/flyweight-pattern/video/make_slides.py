#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Flyweight pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Flyweight Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("stop paying"):
...
def kind_console(scene, img, d):
    ...
    for ln in lines:
        ...
        if ln.startswith("$"):
            ...
            col = RED
        elif ln.startswith("Naive"):
            col = ROSE
        elif ln.startswith("Flyweight"):
            col = GREEN
        elif ln.startswith("Savings"):
            col = CLIENT
        elif ln.startswith(" "):
            col = MUTED
...
def kind_diagram(scene, img, d):
    ...
    box(d, 760, 250, 400, 110, "BadgeDemo", "the Client", CLIENT)
    arrow(d, 960, 360, 960, 430, LINE)
    d.text((980, 380), "styleFor(SALE)", font=f(SANS, 24), fill=MUTED)

    d.rounded_rectangle([120, 430, W - 120, 800], radius=18,
                        outline=ACCENT, width=3)
    d.text((150, 448), "FLYWEIGHT FACTORY", font=f(SANS_B, 22), fill=ACCENT)

    box(d, 760, 480, 400, 110, "BadgeStyleFactory", "cache, keyed by type", ACCENT)

    subs = [
        ("BadgeStyle", "NEW  (shared)", GREEN),
        ("BadgeStyle", "SALE  (shared)", AMBER),
        ("BadgeStyle", "BESTSELLER  (shared)", ROSE),
        ("BadgeStyle", "LOW_STOCK  (shared)", CYAN),
    ]
    centres = [175 + 190 + 400 * i for i in range(4)]

    # vertical stub down from the factory, then a horizontal bus, then a
    # straight drop into each flyweight -- no crossing lines
    bus_y = 625
    d.line([(960, 590), (960, bus_y)], fill=LINE, width=2)
    d.line([(centres[0], bus_y), (centres[-1], bus_y)], fill=LINE, width=2)

    for (name, sub, col), cx in zip(subs, centres):
        arrow(d, cx, bus_y, cx, 660, LINE, 2)
        box(d, cx - 190, 660, 380, 110, name, sub, col)

    d.text((W // 2, 850),
           "One hundred thousand CatalogBadge contexts. As few as four BadgeStyle instances.",
           font=f(SANS_B, 32), fill=TEXT, anchor="ma")
    d.text((W // 2, 906),
           "The factory hands out the same shared instance to every caller asking for the same type.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "FLYWEIGHT", font=f(SANS_B, 128), fill=TEXT)
    ...
    # The naive per-listing allocation vs. the one shared cache lookup.
    pill(d, 150, 556, 700, 170, RED, "new BadgeStyle(...)",
         "once per listing", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "styleFor(SALE)",
         "once per type, shared", tag="AFTER")
    ...
    d.text((W // 2, 758), "Stop paying for the same data twice",
