# Flyweight Pattern — Teaching Video

A narrated, slide-based video that teaches the Flyweight pattern using this
...
| File | What it is |
| --- | --- |
| `flyweight-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `flyweight-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `flyweight-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Scenario | One hundred thousand listings, four badge designs |
| 3 | What a Badge Design Actually Is | Intrinsic fields, named concretely |
| 4 | The Naive Approach | One full badge object built per listing |
| 5 | Why That Hurts | The memory arithmetic, and why it's structural, not a bug |
| 6 | The Pattern | The formal definition, in plain language |
| 7 | The Rubber Stamp Analogy | How to remember it forever |
| 8 | The Four Roles | Flyweight, factory, context, client — and the shared instance |
| 9 | The Flyweight | `BadgeStyle` — intrinsic fields, extrinsic parameters |
| 10 | The Factory | `BadgeStyleFactory.styleFor` and `computeIfAbsent` |
| 11 | What It Gives You | Sharing, correctness, separation |
| 12 | The Client | `CatalogBadge` — owns little, borrows the rest |
| 13 | Running It | The real program output, proving the sharing |
...
**Upload `flyweight-pattern-explained.mp4`, with `poster.png` as the
...
`BadgeStyleFactory`. Uploading `flyweight-pattern-explained.srt` gives
