#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Front Controller video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Front Controller  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("FRONT CONTROLLER", 152, TEXT)
    fitted("ONE DOOR", 288, ACCENT)
    ...
    # A forgotten check versus one door -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "/orders, not signed in",
         "200, ada's orders", tag="NO CHECK")
    pill(d, 1070, 556, 700, 170, GREEN, "/orders, not signed in",
         "401, please sign in", tag="CHECKED ONCE")
    ...
    d.text((W // 2, 744), "Every request comes here first.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Shared work, done once.",
