# Front Controller Pattern — Project Specification
...
[`front-controller-pattern-explained.md`](front-controller-pattern-explained.md); the problem it
...
9 production classes under `com.jk.explore.frontcontroller`:
...
1. **The forgotten check is a real hole:** the naive orders handler serves an unsigned visitor.
2. **Refused requests are logged.**
3. **A failure's message never reaches the customer.**
4. **A buggy filter brings every route down,** and a test says so.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `FrontControllerTest`.
...
| `front-controller-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/front-controller-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Front Controller pattern in
...
| Runtime | ~5:06 over 14 scenes, 78 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Gateway.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py front-controller`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py front-controller
python3 ../../docs/make_specs.py front-controller
```
