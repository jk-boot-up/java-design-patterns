# Identity Map Pattern — Project Specification
...
[`identity-map-pattern-explained.md`](identity-map-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.identitymap`:
...
1. **Identity is proven with `==`, not `equals`.** `CustomerSessionTest` uses `assertSame`.
2. **The lost change is proven.** `PlainCustomerMapperTest` saves two objects for one row and reads the table.
3. **The saving is counted.** A second find costs zero operations, read from the toy database's own counter.
4. **Staleness is proven.** The table is written directly and the session still returns the old value.
...
- `./gradlew build` passes. 13 test methods across `CustomerSessionTest`, `DemoRunsTest`, `PlainCustomerMapperTest`, `ToyDatabaseTest`.
...
| `identity-map-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/identity-map-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Identity Map pattern in
...
| Runtime | ~5:49 over 15 scenes, 83 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Unit of Work.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py identity-map`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py identity-map
python3 ../../docs/make_specs.py identity-map
```
