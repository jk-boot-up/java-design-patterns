#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Identity Map video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Identity Map  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("IDENTITY", 152, TEXT)
    fitted("MAP", 288, ACCENT)
    ...
    # Two objects versus one -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "find(7) == find(7)  // false",
         "two objects, one row", tag="NO MAP")
    pill(d, 1070, 556, 700, 170, GREEN, "find(7) == find(7)  // true",
         "one object per row", tag="IDENTITY MAP")
    ...
    d.text((W // 2, 744), "Same row, same object.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Or two of them will disagree.",
