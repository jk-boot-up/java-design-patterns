# Repository with Spring Data Pattern — Project Specification
...
[`repository-with-spring-data-pattern-explained.md`](repository-with-spring-data-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.repositoryspringdata`:
...
1. **There is no implementing class.** `SpringDataRepositoryTest` asserts the injected object is a generated proxy.
2. **The generated query is counted.** One statement, from Hibernate's statistics.
3. **The leak is real in both directions.** A change is written with no save inside a transaction, and lost outside one.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `SpringDataRepositoryTest`.
...
| `repository-with-spring-data-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/repository-with-spring-data-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Repository with Spring Data pattern in
...
| Runtime | ~5:33 over 14 scenes, 81 subtitle cues |  |
...
> `python3 ../../docs/make_youtube_docs.py repository-with-spring-data`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py repository-with-spring-data
python3 ../../docs/make_specs.py repository-with-spring-data
```
