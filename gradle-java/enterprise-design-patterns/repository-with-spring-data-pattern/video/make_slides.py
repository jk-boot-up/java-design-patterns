#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Repository with Spring Data video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Repository with Spring Data  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("REPOSITORY", 152, TEXT)
    fitted("SPRING DATA", 288, ACCENT)
    ...
    # Hand-written implementations versus an interface alone -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "class InMemoryCustomerRepository",
         "two implementations, by hand", tag="THE PARTNER")
    pill(d, 1070, 556, 700, 170, GREEN, "interface CustomerRepository",
         "no implementation at all", tag="SPRING DATA")
    ...
    d.text((W // 2, 744), "An interface, and nothing behind it.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Except a leak.",
