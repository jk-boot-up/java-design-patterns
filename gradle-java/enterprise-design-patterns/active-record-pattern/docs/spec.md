# Active Record Pattern — Project Specification
...
[`active-record-pattern-explained.md`](active-record-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.activerecord`:
...
1. **Table operations are counted,** never timed.
2. **The renamed column is asserted to break loading.**
3. **Five orders cause five operations,** and the pure rule causes none.
4. **Saving twice updates one row.**
...
- `./gradlew build` passes. 8 test methods across `ActiveRecordTest`, `DemoRunsTest`.
...
| `active-record-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/active-record-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Active Record pattern in
...
| Runtime | ~4:49 over 14 scenes, 73 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Optimistic Offline Lock.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py active-record`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py active-record
python3 ../../docs/make_specs.py active-record
```
