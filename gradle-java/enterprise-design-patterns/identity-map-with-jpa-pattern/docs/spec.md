# Identity Map with JPA Pattern — Project Specification
...
[`identity-map-with-jpa-pattern-explained.md`](identity-map-with-jpa-pattern-explained.md); the problem it
...
2 production classes under `com.jk.explore.identitymapjpa`:
...
1. **Identity is proven with `assertSame`.** `PersistenceContextTest`.
2. **SQL counts come from Hibernate's statistics.** One statement for two finds.
3. **The detached failure is real.** A change made after the context closed is absent from a new context.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `PersistenceContextTest`.
...
| `identity-map-with-jpa-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/identity-map-with-jpa-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Identity Map with JPA pattern in
...
| Runtime | ~5:55 over 16 scenes, 87 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Unit of Work with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py identity-map-with-jpa`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py identity-map-with-jpa
python3 ../../docs/make_specs.py identity-map-with-jpa
```
