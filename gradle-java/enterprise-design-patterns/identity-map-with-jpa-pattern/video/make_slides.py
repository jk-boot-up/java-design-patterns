#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Identity Map with JPA video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Identity Map with JPA  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("IDENTITY MAP", 152, TEXT)
    fitted("WITH JPA", 288, ACCENT)
    ...
    # One context versus two -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "one.find(7) == two.find(7)",
         "two contexts, two objects", tag="TWO CONTEXTS")
    pill(d, 1070, 556, 700, 170, GREEN, "em.find(7) == em.find(7)",
         "one context, one object", tag="ONE CONTEXT")
    ...
    d.text((W // 2, 744), "The persistence context is a map.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "You did not write it.",
