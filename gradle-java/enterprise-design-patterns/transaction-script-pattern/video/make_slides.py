#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Transaction Script video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Transaction Script  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("TRANSACTION SCRIPT", 152, TEXT)
    fitted("ONE PROCEDURE", 288, ACCENT)
    ...
    # A script at its best and a year later -- real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "a year later",
         "7 decisions, 128 paths", tag="GROWN")
    pill(d, 1070, 556, 700, 170, GREEN, "month end",
         "one purpose, 12 lines", tag="RIGHT FOR A SCRIPT")
    ...
    d.text((W // 2, 744), "One request, one procedure.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Simple until it is not.",
