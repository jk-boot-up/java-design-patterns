# Transaction Script Pattern — Project Specification
...
[`transaction-script-pattern-explained.md`](transaction-script-pattern-explained.md); the problem it
...
2 production classes under `com.jk.explore.transactionscript`:
...
1. **The rollback is asserted:** stock and orders return to what they were.
2. **The drift is asserted:** 50.40 against 56.00.
3. **Decisions are counted from the real source:** 3 and 7.
4. **The grown script's rules all apply.**
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `ScriptTest`.
...
| `transaction-script-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/transaction-script-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Transaction Script pattern in
...
| Runtime | ~4:59 over 14 scenes, 77 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Transaction Script"*,
   18 characters. The suffix after the dash names the worked e-commerce scenario, so the title says what the viewer will actually watch rather than only which pattern it is about.
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Active Record.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py transaction-script`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py transaction-script
python3 ../../docs/make_specs.py transaction-script
```
