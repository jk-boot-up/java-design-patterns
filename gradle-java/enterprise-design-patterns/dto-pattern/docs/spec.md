# DTO Pattern — Project Specification
...
[`dto-pattern-explained.md`](dto-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.dto`:
...
1. **The leak is measured.** `DtoTest` asserts the password hash is in the JSON and that the payload exceeds 5000 characters.
2. **The coupling is shown.** Renaming a private field turns the client's key lookup into null.
3. **The DTO is measured.** Its JSON is exactly the three fields and loads nothing.
4. **A DTO is not a model.** The domain object refuses a bad email and the DTO carries it.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `DtoTest`.
...
| `dto-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/dto-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the DTO pattern in
...
| Runtime | ~5:31 over 15 scenes, 82 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Identity Map with JPA.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py dto`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py dto
python3 ../../docs/make_specs.py dto
```
