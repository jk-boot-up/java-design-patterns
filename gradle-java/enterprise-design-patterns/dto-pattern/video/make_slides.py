#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the DTO video."""
...
def kind_poster(scene, img, d):
    ...
    # The domain object versus a DTO -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "return customer; // 5297 chars",
         "password hash included", tag="THE DOMAIN OBJECT")
    pill(d, 1070, 556, 700, 170, GREEN, "return new CustomerDto(...);",
         "three fields, 46 chars", tag="A DTO")
    ...
    d.text((W // 2, 744), "Decide what leaves.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Do not let the object decide.",
