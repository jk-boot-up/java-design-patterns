# Unit of Work with Spring Pattern — Project Specification
...
[`unit-of-work-with-spring-pattern-explained.md`](unit-of-work-with-spring-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.unitofworkspring`:
...
1. **What is committed is read back.** `TransactionalTest` reads the tables, not the code's intent.
2. **The checked exception is real.** Half an order is asserted committed under `@Transactional`.
3. **The flush is counted.** Hibernate's statistics show zero, zero, then one.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `TransactionalTest`.
...
| `unit-of-work-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/unit-of-work-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Unit of Work with Spring pattern in
...
| Runtime | ~5:59 over 15 scenes, 84 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Lazy Load with Hibernate.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py unit-of-work-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py unit-of-work-with-spring
python3 ../../docs/make_specs.py unit-of-work-with-spring
```
