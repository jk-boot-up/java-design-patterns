#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Unit of Work with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Unit of Work with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # Nothing versus one annotation -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "place(); // each step commits",
         "half an order stays", tag="NO ANNOTATION")
    pill(d, 1070, 556, 700, 170, GREEN, "@Transactional void place()",
         "all of it, or none", tag="ONE ANNOTATION")
    ...
    d.text((W // 2, 744), "The unit of work is one annotation.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "With rules nobody wrote down.",
