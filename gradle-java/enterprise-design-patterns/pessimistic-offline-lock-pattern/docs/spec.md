# Pessimistic Offline Lock Pattern — Project Specification
...
[`pessimistic-offline-lock-pattern-explained.md`](pessimistic-offline-lock-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.pessimisticlock`:
...
1. **Expiry uses a clock the demo controls,** never a real wait.
2. **A write without the lock is refused,** including the old owner's after expiry.
3. **A deadlock is shown, and fixed-order locking avoids it.**
4. **Only the holder can release.**
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `PessimisticLockTest`.
...
| `pessimistic-offline-lock-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/pessimistic-offline-lock-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Pessimistic Offline Lock pattern in
...
| Runtime | ~5:09 over 14 scenes, 72 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Front Controller.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py pessimistic-offline-lock`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py pessimistic-offline-lock
python3 ../../docs/make_specs.py pessimistic-offline-lock
```
