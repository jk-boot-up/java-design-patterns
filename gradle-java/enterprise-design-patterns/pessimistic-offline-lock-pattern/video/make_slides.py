#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Pessimistic Offline Lock video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Pessimistic Offline Lock  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("PESSIMISTIC LOCK", 152, TEXT)
    fitted("ASK BEFORE YOU EDIT", 288, ACCENT)
    ...
    # A held lock versus a deadlock -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "A needs B's lock, B needs A's",
         "neither can move", tag="DEADLOCK")
    pill(d, 1070, 556, 700, 170, GREEN, "locks in a fixed order",
         "A took both, B waited", tag="ORDERED")
    ...
    d.text((W // 2, 744), "Prevent the clash.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Pay in waiting.",
