#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Lazy Load with Hibernate video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Lazy Load with Hibernate  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("LAZY LOAD", 152, TEXT)
    fitted("HIBERNATE", 288, ACCENT)
    ...
    # The failure and the way out -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "order.customer().name()",
         "no session: throws", tag="LazyInitialization")
    pill(d, 1070, 556, 700, 170, GREEN, "join fetch o.customer",
         "one statement", tag="A FIX")
    ...
    d.text((W // 2, 744), "The exception everybody has met.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Explained from its mechanism.",
