# Lazy Load with Hibernate Pattern — Project Specification
...
[`lazy-load-with-hibernate-pattern-explained.md`](lazy-load-with-hibernate-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.lazyloadhibernate`:
...
1. **The exception is real.** `LazyLoadHibernateTest` asserts `LazyInitializationException` with 'no session'.
2. **The proxy is shown.** It is not a `Customer`, is uninitialised, and knows its id without loading.
3. **Every count is Hibernate's.** Six, twenty-one, one, and eighty rows.
4. **Dependencies are explained.** `docs/dependencies.md` says what to install, what it costs, and that skipping loses nothing.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `LazyLoadHibernateTest`.
...
| `lazy-load-with-hibernate-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/lazy-load-with-hibernate-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Lazy Load with Hibernate pattern in
...
| Runtime | ~6:07 over 15 scenes, 86 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"LazyInitializationException Explained in Hibernate"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Repository with Spring Data.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py lazy-load-with-hibernate`. They are the one part of
...
integrated, -3.64 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py lazy-load-with-hibernate
python3 ../../docs/make_specs.py lazy-load-with-hibernate
```
