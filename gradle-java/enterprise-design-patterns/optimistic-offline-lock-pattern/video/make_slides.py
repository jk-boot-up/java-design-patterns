#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Optimistic Offline Lock video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Optimistic Offline Lock  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("OPTIMISTIC LOCK", 152, TEXT)
    fitted("CHECK AT SAVE TIME", 288, ACCENT)
    ...
    # No lock versus a version -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "no lock: last write wins",
         "clerk A's price vanished", tag="LOST UPDATE")
    pill(d, 1070, 556, 700, 170, GREEN, "a version on the row",
         "clerk B's stale save refused", tag="REFUSED")
    ...
    d.text((W // 2, 744), "Assume no clash.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Check the version when saving.",
