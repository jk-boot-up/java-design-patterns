# Optimistic Offline Lock Pattern — Project Specification
...
[`optimistic-offline-lock-pattern-explained.md`](optimistic-offline-lock-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.optimisticlock`:
...
1. **The lost update is real:** the last-write-wins row loses the price.
2. **A stale save is refused and the first write survives.**
3. **Ten writers on one row lose nothing and attempt nineteen saves.**
4. **Different fields still conflict,** because the version is per row.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `OptimisticLockTest`.
...
| `optimistic-offline-lock-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/optimistic-offline-lock-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Optimistic Offline Lock pattern in
...
| Runtime | ~4:59 over 14 scenes, 72 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Pessimistic Offline Lock.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py optimistic-offline-lock`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py optimistic-offline-lock
python3 ../../docs/make_specs.py optimistic-offline-lock
```
