# Gateway Pattern — Project Specification
...
[`gateway-pattern-explained.md`](gateway-pattern-explained.md); the problem it
...
8 production classes under `com.jk.explore.gateway`:
...
1. **Network calls are counted:** the fake makes none.
2. **Every Acme code is translated,** and an unknown code is not swallowed.
3. **One timeout is retried once,** two are reported as unavailable.
4. **The naive gift card really omits the currency.**
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `GatewayTest`.
...
| `gateway-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/gateway-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Gateway pattern in
...
| Runtime | ~5:12 over 14 scenes, 76 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Cache-Aside.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py gateway`. They are the one part of
...
integrated, -3.45 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py gateway
python3 ../../docs/make_specs.py gateway
```
