#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Gateway video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Gateway  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("GATEWAY", 152, TEXT)
    fitted("ONE DOOR OUT", 288, ACCENT)
    ...
    # Direct calls versus one door -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "3 places call the client",
         "1 forgot the currency", tag="DIRECT")
    pill(d, 1070, 556, 700, 170, GREEN, "checkout asks the gateway",
         "approved or declined", tag="ONE DOOR")
    ...
    d.text((W // 2, 744), "The shop speaks its own language.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "One class speaks the provider's.",
