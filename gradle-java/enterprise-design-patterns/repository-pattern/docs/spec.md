# Repository Pattern — Project Specification
...
[`repository-pattern-explained.md`](repository-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.repository`:
...
1. **The drift is real.** `SqlInTheServiceTest` shows three answers to one question.
2. **The swap is proven.** The same service gives the same answer on both stores, and reflection shows it names only the interface.
3. **The leak is counted.** Seven operations for six customers, from the toy database's counter.
4. **The rename is silent.** Every list is empty and nothing throws.
...
- `./gradlew build` passes. 13 test methods across `DemoRunsTest`, `RepositoryTest`, `SqlInTheServiceTest`, `ToyDatabaseTest`.
...
| `repository-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/repository-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Repository pattern in
...
| Runtime | ~5:45 over 15 scenes, 81 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Service Layer.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py repository`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py repository
python3 ../../docs/make_specs.py repository
```
