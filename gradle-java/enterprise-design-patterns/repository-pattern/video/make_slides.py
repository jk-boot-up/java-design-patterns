#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Repository video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Repository  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # A query in the service versus a collection to ask -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "db.selectWhere(city, ...)",
         "the service knows the column", tag="SQL IN THE SERVICE")
    pill(d, 1070, 556, 700, 170, GREEN, "customers.findByCity(city)",
         "the service asks for customers", tag="REPOSITORY")
    ...
    d.text((W // 2, 744), "Query the collection,",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "not the table.",
