#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Service Layer video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Service Layer  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # Logic copied per door versus one shared operation -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "web.placeOrder(); cli.placeOrder();",
         "two copies of the logic", tag="COPIED")
    pill(d, 1070, 556, 700, 170, GREEN, "service.placeOrder(request);",
         "one operation, every door", tag="SERVICE LAYER")
    ...
    d.text((W // 2, 744), "Two doors, one placeOrder.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Or the copies will drift.",
