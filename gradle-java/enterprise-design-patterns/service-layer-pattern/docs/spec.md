# Service Layer Pattern — Project Specification
...
[`service-layer-pattern-explained.md`](service-layer-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.servicelayer`:
...
1. **The drift is real.** `ServiceLayerTest` shows the copied door charging 6000 pence for a refused order.
2. **Both doors agree.** With one `placeOrder` the two doors give the same answer.
3. **A refusal rolls back.** Nothing is written or emailed.
4. **The rules live in the domain.** Tests call `Order.from` and `Product.reserve` directly.
...
- `./gradlew build` passes. 13 test methods across `DemoRunsTest`, `ServiceLayerTest`, `ToyDatabaseTest`.
...
| `service-layer-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/service-layer-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Service Layer pattern in
...
| Runtime | ~5:38 over 15 scenes, 84 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: DTO.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py service-layer`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py service-layer
python3 ../../docs/make_specs.py service-layer
```
