# Unit of Work Pattern — Project Specification
...
[`unit-of-work-pattern-explained.md`](unit-of-work-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.unitofwork`:
...
1. **The wreckage is real.** `PlacementTest` fails the sixth write and reads the tables.
2. **Nothing touches the database until commit.** `UnitOfWorkTest` asserts zero operations after registration.
3. **Failure rolls back completely.** The seventh write is rejected and every table is read back.
4. **The ordering cost is real.** Committing in registration order breaks the foreign key.
...
- `./gradlew build` passes. 15 test methods across `DemoRunsTest`, `PlacementTest`, `ToyDatabaseTest`, `UnitOfWorkTest`.
...
| `unit-of-work-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/unit-of-work-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Unit of Work pattern in
...
| Runtime | ~5:46 over 15 scenes, 82 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Lazy Load.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py unit-of-work`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py unit-of-work
python3 ../../docs/make_specs.py unit-of-work
```
