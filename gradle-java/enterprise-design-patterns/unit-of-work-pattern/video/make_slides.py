#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Unit of Work video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Unit of Work  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("UNIT OF", 152, TEXT)
    fitted("WORK", 288, ACCENT)
    ...
    # Write as you go versus write once -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "table.update(product);",
         "each write, as it happens", tag="SAVE AS YOU GO")
    pill(d, 1070, 556, 700, 170, GREEN, "work.commit();",
         "all of it, or none", tag="UNIT OF WORK")
    ...
    d.text((W // 2, 744), "Half an order is not an order.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Write it all once, or not at all.",
