package com.jk.explore.unitofwork.toydb;
...
public final class Table {
    ...
    public void insert(int id, Row row) {
        database.write("INSERT " + name + " id=" + id, () -> {
            database.checkParent(name, row);
            rows.put(id, row.copy());
        });
    }

    Map<Integer, Row> snapshot() {
        Map<Integer, Row> copy = new TreeMap<>();
        rows.forEach((k, v) -> copy.put(k, v.copy()));
        return copy;
    }

    void restore(Map<Integer, Row> snapshot) {
        rows.clear();
