# Data Mapper Pattern — Project Specification
...
[`data-mapper-pattern-explained.md`](data-mapper-pattern-explained.md); the problem it
...
- Not an argument against Active Record. It is shown working and named as the right answer for a simple application.
- Not object-graph loading. How far a mapper should load belongs to the Lazy Load project.
- Not a framework tutorial. The build files hold no framework; JPA is named as where you have already met the pattern.
...
1 production classes under `com.jk.explore.datamapper`:
...
1. **Active Record is shown working first.** `ActiveRecordCustomerTest` proves save, find and update in three counted operations.
2. **The customer holds no persistence state.** `CustomerTest` inspects the class by reflection and constructs one with no database.
3. **Every count comes from the counter.** The demo prints the toy database's own operation log.
4. **The silent field is proven.** `CustomerMapperTest` shows a careless mapper succeeds and loses the postcode.
...
- `./gradlew build` passes. 15 test methods across `ActiveRecordCustomerTest`, `CustomerMapperTest`, `CustomerTest`, `DemoRunsTest`, `ToyDatabaseTest`.
...
| `data-mapper-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/data-mapper-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Data Mapper pattern in
...
| Runtime | ~6:06 over 14 scenes, 87 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Identity Map.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py data-mapper`. They are the one part of
...
Last verified: all eleven items pass. The delivered MP4 measures -16.03 LUFS
...
```bash
python3 ../../docs/make_youtube_docs.py data-mapper
python3 ../../docs/make_specs.py data-mapper
```
