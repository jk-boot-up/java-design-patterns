#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Data Mapper video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Data Mapper  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("DATA", 152, TEXT)
    fitted("MAPPER", 288, ACCENT)
    ...
    # Active Record versus a mapper -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "customer.save();",
         "the object knows its table", tag="ACTIVE RECORD")
    pill(d, 1070, 556, 700, 170, GREEN, "mapper.insert(customer);",
         "the object knows nothing", tag="DATA MAPPER")
    ...
    d.text((W // 2, 744), "Should an object know how it is stored?",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Give that job to a mapper.",
