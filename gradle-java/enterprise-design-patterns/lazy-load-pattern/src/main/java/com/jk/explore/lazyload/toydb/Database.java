package com.jk.explore.lazyload.toydb;
...
public final class Database {
    ...
    private int failReadAfter = -1;

    /** The read after the next {@code readsFromNow - 1} reads will fail. */
    public void failReadNumber(int readsFromNow) {
        this.failReadAfter = readsFromNow - 1;
    }
    ...
    void record(String operation) {
        if (failReadAfter == 0) {
            ...
            throw new IllegalStateException("the database failed to read: " + operation);
        }
        if (failReadAfter > 0) {
            failReadAfter--;
        }
