#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Lazy Load video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Lazy Load  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("LAZY", 152, TEXT)
    fitted("LOAD", 288, ACCENT)
    ...
    # Everything now versus each part when asked -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "load(order) // 37 objects",
         "one order, everything reachable", tag="EAGER")
    pill(d, 1070, 556, 700, 170, GREEN, "customer.name() // on demand",
         "load the rest when asked", tag="LAZY LOAD")
    ...
    d.text((W // 2, 744), "One order should not load the shop.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "But every access is now a query.",
