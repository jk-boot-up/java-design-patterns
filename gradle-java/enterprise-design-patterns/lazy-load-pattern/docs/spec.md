# Lazy Load Pattern — Project Specification
...
[`lazy-load-pattern-explained.md`](lazy-load-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.lazyload`:
...
1. **The eager count is exact.** `LazyLoadTest` asserts 37 objects and 26 operations.
2. **N+1 is counted.** Twenty-one queries lazily and two batched, from the toy database's counter.
3. **A lazy failure is at the point of use.** The proxy is created, the session closed, and the exception thrown only on access.
4. **Each variant loads once.** Zero before use, one after the first, one after the second.
...
- `./gradlew build` passes. 14 test methods across `DemoRunsTest`, `LazyLoadTest`, `ToyDatabaseTest`.
...
| `lazy-load-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/lazy-load-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Lazy Load pattern in
...
| Runtime | ~5:31 over 14 scenes, 82 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Repository.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py lazy-load`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py lazy-load
python3 ../../docs/make_specs.py lazy-load
```
