#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Hexagonal Architecture with Spring Boot video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Hexagonal Architecture with Spring Boot  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # A core with no framework versus one that reaches for it -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "use case with @Transactional",
         "10 rule violations", tag="FRAMEWORK IN THE CORE")
    pill(d, 1070, 556, 700, 170, GREEN, "use case, plain Java",
         "0 rule violations", tag="PLAIN CORE")
    ...
    d.text((W // 2, 744), "The core stays plain.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Spring wires around it.",
