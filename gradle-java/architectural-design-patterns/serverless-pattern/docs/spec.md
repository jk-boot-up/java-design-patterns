# Serverless Functions Pattern — Project Specification
...
[`serverless-pattern-explained.md`](serverless-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.serverless`:
...
1. **The clock is counted,** so every number is exact.
2. **Concurrent calls each need an instance.**
3. **An idle instance is dropped after the timeout.**
4. **Instance memory resets, and the outside store does not.**
5. **Work over the limit is reported as stopped.**
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `ServerlessTest`.
...
| `serverless-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/serverless-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Serverless Functions pattern in
...
| Runtime | ~4:47 over 14 scenes, 71 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Serverless Functions"*,
   20 characters. The suffix after the dash names the worked e-commerce scenario, so the title says what the viewer will actually watch rather than only which pattern it is about.
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Blue-Green and Canary.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py serverless`. They are the one part of
...
integrated, -3.54 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py serverless
python3 ../../docs/make_specs.py serverless
```
