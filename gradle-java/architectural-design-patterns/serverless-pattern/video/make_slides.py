#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Serverless video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Serverless  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("SERVER-", 152, TEXT)
    fitted("LESS", 288, ACCENT)
    ...
    # A machine always on versus paying per call -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "always-on server",
         "bill 200 for 3 orders", tag="PAID WHILE IDLE")
    pill(d, 1070, 556, 700, 170, GREEN, "function per order",
         "bill 3 for 3 orders", tag="PAY PER CALL")
    ...
    d.text((W // 2, 744), "Run only when called.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Pay only for calls.",
