# Layered Architecture with Spring Boot Pattern — Project Specification
...
[`layered-architecture-with-spring-boot-pattern-explained.md`](layered-architecture-with-spring-boot-pattern-explained.md); the problem it
...
2 production classes under `com.jk.explore.layeredspring`:
...
1. **All traffic is real HTTP.**
2. **The rollback is asserted** by reading the stock.
3. **The rule is asserted to fail only on the shortcut.**
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `LayeredTest`.
...
| `layered-architecture-with-spring-boot-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/layered-architecture-with-spring-boot-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Layered Architecture with Spring Boot pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: MVC with Spring MVC.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py layered-architecture-with-spring-boot`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py layered-architecture-with-spring-boot
python3 ../../docs/make_specs.py layered-architecture-with-spring-boot
```
