#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Layered Architecture with Spring Boot video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Layered Architecture with Spring Boot  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("LAYERED ARCHITECTURE", 152, TEXT)
    fitted("WITH SPRING BOOT", 288, ACCENT)
    ...
    # A controller that skips a layer -- a real line from the project.
    pill(d, 150, 556, 700, 170, RED, "controller reads the repository",
         "cost price leaks in the JSON", tag="SKIPPED A LAYER")
    pill(d, 1070, 556, 700, 170, GREEN, "controller, service, repository",
         "response hides the cost", tag="LAYERED")
    ...
    d.text((W // 2, 744), "Spring names the layers.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "A test enforces them.",
