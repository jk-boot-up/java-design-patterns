#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the MVC video."""
...
def kind_quote(scene, img, d):
    ...
    for ln in body:
        col = TEXT
        fnt = f(SANS, 42)
        if ln.startswith("If you have used"):
...
def kind_poster(scene, img, d):
    ...
    fitted("MVC", 152, TEXT)
    fitted("MODEL VIEW CONTROLLER", 288, ACCENT, cap=74)
    ...
    # Two views, one order, two totals -- both real lines from the project,
    # so the poster shows the actual contrast rather than a slogan.
    pill(d, 150, 556, 700, 170, RED, "email: £383.00",
         "rounds its own prices — screen says £382.50", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "email: £382.50",
         "reads the same model as the screen", tag="AFTER")
    ...
    d.text((W // 2, 744), "Two views should never disagree.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "One that can't calculate, can't.",
