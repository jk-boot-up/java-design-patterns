# Model View Controller Pattern — Project Specification
...
[`mvc-pattern-explained.md`](mvc-pattern-explained.md); the problem it
...
- Not a UI toolkit tutorial. Every view produces a String; there is no Swing, JavaFX or web framework anywhere in this project.
- Not a claim that this project's controller matches classic Smalltalk MVC's observation. The controller builds the model once and hands it to the views asked for, rather than views subscribing and being notified — the written notes say so plainly, because a synchronous console demo has no independent redraw to trigger.
- Not MVP or MVVM. Both are named and distinguished in one scene each; neither is built, because teaching either properly needs a UI toolkit with real data binding.
...
2 production classes under `com.jk.explore.mvc`:
...
1. **An ArchUnit rule asserts no view depends on infrastructure, and a second test proves it can fail.** `ArchitectureRuleCatchesTheShortcutTest` widens the rule to `naive.view` and asserts the failure names `RoundedEmailView` and `ProductTable` by name.
2. **Two real views are proven, by arithmetic, to always agree — and the naive bug is proven, by arithmetic, to be real.** `ViewsAgreementTest` asserts both: `twoRealViewsAlwaysAgree` and `theNaiveShortcutReallyDoesDisagree`, the latter computing the actual £383.00 rather than asserting it in prose.
3. **`OrderSummaryView` accepts exactly one parameter type.** No overload anywhere in the interface accepts a `Product`, a price, or a quantity a view could multiply for itself.
...
- `./gradlew build` passes. 19 test methods across `ArchitectureRuleCatchesTheShortcutTest`, `ArchitectureTest`, `CompositeOrderViewTest`, `DemoRunsTest`, `MoneyTest`, `PlaceOrderServiceTest`, `ViewsAgreementTest`.
...
| `mvc-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/mvc-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Model View Controller pattern in
...
| Runtime | ~12:42 over 15 scenes, 187 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Hexagonal Architecture.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py mvc`. They are the one part of
...
integrated, -4.02 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py mvc
python3 ../../docs/make_specs.py mvc
```
