#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Event-Driven Architecture video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Event-Driven Architecture  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # Calling and waiting versus writing to a log -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "call shipping, and wait",
         "shipping down: order lost", tag="ORDER LOST")
    pill(d, 1070, 556, 700, 170, GREEN, "write to a log",
         "shipping catches up later", tag="ORDER KEPT")
    ...
    d.text((W // 2, 744), "Write a fact.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Read it at your own pace.",
