# Event-Driven Architecture Pattern — Project Specification
...
[`event-driven-architecture-pattern-explained.md`](event-driven-architecture-pattern-explained.md); the problem it
...
7 production classes under `com.jk.explore.eda`:
...
2. **A down reader's lag is asserted exactly.**
3. **A new reader replays the whole log.**
4. **Stock is wrong before the reader polls, and right after.**
5. **A duplicate is shown to double-count without a check.**
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `EdaTest`.
...
| `event-driven-architecture-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/event-driven-architecture-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Event-Driven Architecture pattern in
...
| Runtime | ~4:43 over 14 scenes, 71 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Microkernel.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py event-driven-architecture`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py event-driven-architecture
python3 ../../docs/make_specs.py event-driven-architecture
```
