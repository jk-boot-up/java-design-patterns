# Hexagonal Architecture Pattern — Project Specification
...
[`hexagonal-architecture-pattern-explained.md`](hexagonal-architecture-pattern-explained.md); the problem it
...
- Not a claim that this project replaces Layered Architecture. It is one deliberate move on top of it: the same interface, moved into the core, so the direction of one import reverses.
- Not a real HTTP server or a real database. `HttpCheckoutAdapter` simulates a JSON body as a `Map`; every adapter in this project is in-memory, because the lesson is the shape of the dependency, not the infrastructure behind it.
- Not a claim that every application needs this. The written notes name the honest question directly: for a core that will only ever have one database and one caller, is either swap ever going to happen?
...
2 production classes under `com.jk.explore.hexagonal`:
...
1. **An ArchUnit rule asserts the core depends on no adapter, and a second test proves it can fail.** `ArchitectureRuleCatchesTheShortcutTest` widens the rule to `naive.core` and asserts the failure names `NaivePlaceOrderService` and the adapter it reached for.
2. **Both directions of the forced change are proven, not merely described.** `BothSidesAgreeTest` asserts the same core answers identically through `HttpCheckoutAdapter` and `CliCheckoutAdapter`, and that swapping `InMemoryOrderStore` for `AppendOnlyOrderStore` changes no result.
3. **`PlaceOrderService` imports nothing from `adapter`.** Every dependency it has is a type declared inside `core.port`.
...
- `./gradlew build` passes. 17 test methods across `ArchitectureRuleCatchesTheShortcutTest`, `ArchitectureTest`, `BothSidesAgreeTest`, `DemoRunsTest`, `MoneyTest`, `PlaceOrderServiceTest`.
...
| `hexagonal-architecture-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/hexagonal-architecture-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Hexagonal Architecture pattern in
...
| Runtime | ~10:11 over 14 scenes, 146 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Clean Architecture.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py hexagonal-architecture`. They are the one part of
...
integrated, -3.89 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py hexagonal-architecture
python3 ../../docs/make_specs.py hexagonal-architecture
```
