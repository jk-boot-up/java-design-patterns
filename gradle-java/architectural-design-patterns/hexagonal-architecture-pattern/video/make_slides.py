#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Hexagonal Architecture video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Hexagonal Architecture  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in body:
        col = TEXT
        fnt = f(SANS, 42)
        if ln.startswith("Same three methods"):
...
def kind_poster(scene, img, d):
    ...
    # The core naming an adapter, versus the adapter naming the core --
    # both real lines from the project, so the poster shows the actual
    # contrast rather than a slogan.
    pill(d, 150, 556, 700, 170, RED, "core -> InMemoryOrderStore",
         "the core names the adapter", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "adapter -> OrderStore",
         "the adapter names the core's port", tag="AFTER")
    ...
    d.text((W // 2, 744), "Who is allowed to name whom?",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "The core never names an adapter.",
