#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Microkernel video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Microkernel  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("MICRO-", 152, TEXT)
    fitted("KERNEL", 288, ACCENT)
    ...
    # Every feature inside versus plugins -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "edit the checkout",
         "for every new feature", tag="EDIT + RELEASE")
    pill(d, 1070, 556, 700, 170, GREEN, "register a plugin",
         "core unchanged", tag="PLUG IN")
    ...
    d.text((W // 2, 744), "A tiny core.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Everything else is a plugin.",
