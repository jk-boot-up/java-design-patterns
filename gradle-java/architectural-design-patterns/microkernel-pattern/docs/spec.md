# Microkernel Pattern — Project Specification
...
[`microkernel-pattern-explained.md`](microkernel-pattern-explained.md); the problem it
...
8 production classes under `com.jk.explore.microkernel`:
...
1. **Totals are integers in cents,** so results are exact.
2. **Register starts and unregister stops.**
3. **A throwing plugin is recorded and the rest run.**
4. **Plugin order is shown to change the total.**
...
- `./gradlew build` passes. 6 test methods across `DemoRunsTest`, `MicrokernelTest`.
...
| `microkernel-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/microkernel-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Microkernel pattern in
...
| Runtime | ~4:41 over 14 scenes, 70 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Pipe-and-Filter Architecture.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py microkernel`. They are the one part of
...
integrated, -3.67 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py microkernel
python3 ../../docs/make_specs.py microkernel
```
