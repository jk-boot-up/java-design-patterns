# Onion Architecture Pattern — Project Specification
...
[`onion-architecture-pattern-explained.md`](onion-architecture-pattern-explained.md); the problem it
...
2 production classes under `com.jk.explore.onion`:
...
2. **The checker reads real classes,** not a list written by hand.
3. **The onion classes have no outward reference.**
4. **Both storages give the same total and round-trip an order.**
5. **The pricing rule is checked with no storage.**
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `OnionTest`.
...
| `onion-architecture-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/onion-architecture-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Onion Architecture pattern in
...
| Runtime | ~5:17 over 14 scenes, 75 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Serverless Functions.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py onion-architecture`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py onion-architecture
python3 ../../docs/make_specs.py onion-architecture
```
