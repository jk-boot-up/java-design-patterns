#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Onion Architecture video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Onion Architecture  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # A core that reaches out versus one that does not -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "order runs SQL itself",
         "ring 0 refers to ring 3", tag="1 VIOLATION")
    pill(d, 1070, 556, 700, 170, GREEN, "storage is outside",
         "refers only inward", tag="0 VIOLATIONS")
    ...
    d.text((W // 2, 744), "Rules at the centre.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Dependencies point inward.",
