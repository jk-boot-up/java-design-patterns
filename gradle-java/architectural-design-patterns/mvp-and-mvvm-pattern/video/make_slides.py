#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the MVP and MVVM video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "MVP and MVVM  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("MVP AND", 152, TEXT)
    fitted("MVVM", 288, ACCENT)
    ...
    # Rules in the screen versus out of it -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "rules inside the screen",
         "needs a window to check", tag="1 WINDOW")
    pill(d, 1070, 556, 700, 170, GREEN, "rules in a presenter",
         "checked with no screen", tag="0 WINDOWS")
    ...
    d.text((W // 2, 744), "Rules out of the screen.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Tell it, or let it bind.",
