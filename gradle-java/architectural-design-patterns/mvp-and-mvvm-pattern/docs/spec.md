# MVP and MVVM Pattern — Project Specification
...
[`mvp-and-mvvm-pattern-explained.md`](mvp-and-mvvm-pattern-explained.md); the problem it
...
10 production classes under `com.jk.explore.mvpmvvm`:
...
1. **Money is integer cents,** formatted in one place.
2. **No test for the presenter or view model opens a window.**
3. **The view model holds no reference to any view.**
4. **A forgotten binding is shown to be silently wrong.**
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `MvpMvvmTest`.
...
| `mvp-and-mvvm-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/mvp-and-mvvm-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the MVP and MVVM pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Onion Architecture.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py mvp-and-mvvm`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py mvp-and-mvvm
python3 ../../docs/make_specs.py mvp-and-mvvm
```
