#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Layered Architecture video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Layered Architecture  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in body:
        col = TEXT
        fnt = f(SANS, 42)
        if ln.startswith("A diagram") or ln.startswith("A test"):
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson: four stacked layers on the left, each
    # arrow pointing down to the layer beneath it, and on the right the one
    # honest admission -- application still names infrastructure by package,
    # which is exactly what the next project in the category inverts.
    box(d, 70, 244, 560, 110, "presentation  ·  CheckoutScreen",
        "turns a request into a call, an answer into words", CLIENT,
        label_size=30, sub_size=20)
    arrow(d, 350, 354, 350, 380, CLIENT, 4)

    box(d, 70, 384, 560, 110, "application  ·  PlaceOrderService",
        "one use case, four steps, in order", AMBER,
        label_size=30, sub_size=20)
    arrow(d, 350, 494, 350, 520, AMBER, 4)

    box(d, 70, 524, 560, 110, "domain  ·  Order, Money, Product",
        "knows nothing above it, or below it", GREEN,
        label_size=30, sub_size=20)

    box(d, 70, 694, 560, 130, "infrastructure",
        "OrderTable · ProductTable · CardNetwork · EmailServer", ROSE,
        label_size=30, sub_size=20)

    d.text((350, 858), "the only downward chain that is allowed",
           font=f(SANS, 24), fill=MUTED, anchor="ma")

    # Right column: the honest admission this project makes about itself.
    d.text((1560, 248), "WHAT THIS PROJECT DOES NOT FIX",
           font=f(SANS_B, 28), fill=AMBER, anchor="ma")
    d.line([(1250, 294), (1870, 294)], fill=LINE, width=2)

    box(d, 1250, 324, 620, 130,
        'application imports "OrderTable"',
        "the interface is DEFINED in infrastructure, not up here", CYAN,
        label_size=26, sub_size=20)

    arrow(d, 1560, 458, 1560, 490, CYAN, 4)

    box(d, 1250, 494, 620, 130, "the dependency still points down",
        "domain + application still rely on infrastructure to exist", ROSE,
        label_size=26, sub_size=20)

    arrow(d, 1560, 628, 1560, 660, ROSE, 4)

    box(d, 1250, 664, 620, 150, "next project: Hexagonal Architecture",
        "moves the interface INTO the core -- storage implements it, "
        "the arrow reverses", GREEN,
        label_size=26, sub_size=19)

    d.text((W // 2, 970),
           "Layering organises a codebase. It does not, on its own, "
           "invert which way a dependency points.",
           font=f(SANS_B, 27), fill=AMBER, anchor="ma")
...
def kind_poster(scene, img, d):
    ...
    fitted("LAYERED", 152, TEXT)
    fitted("ARCHITECTURE", 288, ACCENT)
    ...
    # The shortcut, versus the rule that catches it -- both real lines from
    # the project, so the poster shows the actual contrast rather than a
    # slogan.
    pill(d, 150, 556, 700, 170, RED, "screen -> InMemoryOrderTable",
         "compiles. ships. nothing objects.", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "noClasses().should().dependOn...",
         "fails the build, names the class", tag="AFTER")
    ...
    d.text((W // 2, 744), "Four folders are not an architecture.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "A test that fails when one is skipped, is.",
