# MVC with Spring MVC Pattern — Project Specification
...
[`mvc-with-spring-mvc-pattern-explained.md`](mvc-with-spring-mvc-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.mvcspring`:
...
1. **All traffic is real HTTP,** and redirects are not followed so they can be seen.
2. **The model is tested without a server.**
3. **The divergent template is asserted,** so a fix fails the test.
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `MvcTest`.
...
| `mvc-with-spring-mvc-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/mvc-with-spring-mvc-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the MVC with Spring MVC pattern in
...
| Runtime | ~4:27 over 16 scenes, 65 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Hexagonal Architecture with Spring Boot.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py mvc-with-spring-mvc`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py mvc-with-spring-mvc
python3 ../../docs/make_specs.py mvc-with-spring-mvc
```
