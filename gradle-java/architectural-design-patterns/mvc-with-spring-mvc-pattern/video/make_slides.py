#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the MVC with Spring MVC video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "MVC with Spring MVC  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("MVC", 152, TEXT)
    fitted("WITH SPRING MVC", 288, ACCENT)
    ...
    # A sum in the template versus the model -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "sum done in the template",
         "32500 pence, no discount", tag="WRONG TOTAL")
    pill(d, 1070, 556, 700, 170, GREEN, "sum done in the model",
         "29250 pence, discount applied", tag="RIGHT TOTAL")
    ...
    d.text((W // 2, 744), "The model computes.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "The view only shows.",
