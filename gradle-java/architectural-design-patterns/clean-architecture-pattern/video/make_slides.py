#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Clean Architecture video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Clean Architecture  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in body:
        col = TEXT
        fnt = f(SANS, 42)
        if ln.startswith("The fourth difference"):
...
def kind_poster(scene, img, d):
    ...
    # Control flowing one way, dependency pointing the other -- both real
    # lines from the project, so the poster shows the actual contrast
    # rather than a slogan.
    pill(d, 150, 556, 700, 170, CYAN, "control: interactor -> gateway",
         "flows OUTWARD, at runtime", tag="CALL")
    pill(d, 1070, 556, 700, 170, GREEN, "dependency: gateway -> interface",
         "points INWARD, in source", tag="TYPE")
    ...
    d.text((W // 2, 744), "The call goes out. The arrow points in.",
           font=f(SANS_B, 46), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Letting those disagree is the whole trick.",
