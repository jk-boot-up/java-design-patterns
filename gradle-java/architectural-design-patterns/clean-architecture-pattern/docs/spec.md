# Clean Architecture Pattern — Project Specification
...
[`clean-architecture-pattern-explained.md`](clean-architecture-pattern-explained.md); the problem it
...
- Not a claim that this project is unrelated to Hexagonal Architecture. It is a close relative, named as one: the same inward-pointing rule, generalised into three rings instead of one core/adapter split.
- Not a dependency injection container. The whole object graph is wired by hand in `main()`, deliberately, because watching that wiring is where the inversion stops being a diagram — the container version is `clean-architecture-with-spring`, a separate project.
- Not a recommendation to build every feature this way. The written notes name this as the most over-applied pattern in the category and count the file cost of one feature to prove it.
...
2 production classes under `com.jk.explore.clean`:
...
1. **The concentric rule is checked with ArchUnit's own layered-architecture API, not a hand-rolled rule.** `Architectures.layeredArchitecture()` declares three named layers and one accessibility sentence per pair; `ArchitectureRuleCatchesTheShortcutTest` widens an equivalent rule to `naive.usecases` and asserts the failure names `NaivePlaceOrderInteractor`.
2. **Both halves of the forced change are proven working at once, not narrated.** `BothAddedAtOnceTest` exercises the original HTTP-and-map path and the new batch-and-file path in the same test run.
3. **`PlaceOrderInteractor` imports nothing from `adapters`.** Every dependency it has is a type declared inside its own package.
...
- `./gradlew build` passes. 16 test methods across `ArchitectureRuleCatchesTheShortcutTest`, `ArchitectureTest`, `BothAddedAtOnceTest`, `DemoRunsTest`, `MoneyTest`, `PlaceOrderInteractorTest`.
...
| `clean-architecture-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/clean-architecture-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Clean Architecture pattern in
...
| Runtime | ~10:19 over 15 scenes, 147 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Clean Architecture with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py clean-architecture`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py clean-architecture
python3 ../../docs/make_specs.py clean-architecture
```
