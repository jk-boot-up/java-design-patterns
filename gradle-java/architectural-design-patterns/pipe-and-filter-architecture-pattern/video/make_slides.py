#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Pipe and Filter Architecture video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Pipe and Filter Architecture  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("PIPE AND FILTER", 152, TEXT)
    ...
    # One big step versus stages -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "one step does all",
         "5 orders in 30 ticks", tag="5 DONE")
    pill(d, 1070, 556, 700, 170, GREEN, "three stages",
         "8 orders in 30 ticks", tag="8 DONE")
    ...
    d.text((W // 2, 744), "Stages run together.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Size the slow one.",
