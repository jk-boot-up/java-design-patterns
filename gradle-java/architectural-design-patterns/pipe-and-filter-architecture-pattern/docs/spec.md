# Pipe-and-Filter Architecture Pattern — Project Specification
...
[`pipe-and-filter-architecture-pattern-explained.md`](pipe-and-filter-architecture-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.pipefilterarch`:
...
1. **Time is counted in ticks,** so every number is exact.
2. **Stages overlap,** so more orders finish than with one step.
3. **The longest line is in front of the slowest stage.**
4. **A limit keeps a line short and gives the same output.**
5. **A crash loses the orders in flight, and they are counted.**
...
- `./gradlew build` passes. 6 test methods across `DemoRunsTest`, `PipeFilterArchTest`.
...
| `pipe-and-filter-architecture-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/pipe-and-filter-architecture-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Pipe-and-Filter Architecture pattern in
...
| Runtime | ~4:58 over 14 scenes, 73 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: MVP and MVVM.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py pipe-and-filter-architecture`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py pipe-and-filter-architecture
python3 ../../docs/make_specs.py pipe-and-filter-architecture
```
