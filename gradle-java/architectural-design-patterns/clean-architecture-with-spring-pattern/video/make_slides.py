#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Clean Architecture with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Clean Architecture + Spring  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in body:
        col = TEXT
        fnt = f(SANS, 42)
        if ln.startswith("At its centre"):
...
def kind_poster(scene, img, d):
    ...
    fitted("CLEAN + SPRING", 152, TEXT, cap=92)
    fitted("WHO WIRES THE GRAPH?", 288, ACCENT, cap=68)
    ...
    # A deleted argument versus a deleted @Bean method -- both real lines
    # from the project, so the poster shows the actual contrast rather
    # than a slogan.
    pill(d, 150, 556, 700, 170, RED, "new Interactor(a, b, c)",
         "fails to COMPILE", tag="BY HAND")
    pill(d, 1070, 556, 700, 170, GREEN, "@Bean missing -> context.refresh()",
         "fails at STARTUP", tag="BY CONTAINER")
    ...
    d.text((W // 2, 744), "Hand-wiring fails at compile time.",
           font=f(SANS_B, 46), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Container wiring fails at startup.",
