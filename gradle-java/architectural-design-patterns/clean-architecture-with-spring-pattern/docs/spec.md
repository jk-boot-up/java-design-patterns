# Clean Architecture with Spring Pattern — Project Specification
...
[`clean-architecture-with-spring-pattern-explained.md`](clean-architecture-with-spring-pattern-explained.md); the problem it
...
- Not a re-teaching of Clean Architecture. Every entity, use case and adapter is `clean-architecture-pattern`'s file, unchanged; that project's own explained document is the authority on all of them.
- Not a Spring tutorial. This project uses exactly one dependency, `spring-boot-starter`, and answers exactly one question — what changes when a container does the wiring — before it stops.
- Not a web application. There is no web starter, no HTTP server, anywhere in this project.
...
1 production classes under `com.jk.explore.cleanspring`:
...
1. **The contrast is proven, not narrated.** `WiringContrastTest` builds a working context from `AppConfig` and asserts `BrokenAppConfig` throws `UnsatisfiedDependencyException` naming the missing `NotificationGateway` type.
2. **Spring appears in no file copied from §66.** A dedicated ArchUnit test asserts no class in `entities`, `usecases` or `adapters` depends on any `org.springframework` package.
3. **`docs/dependencies.md` answers the standing five questions** — what Spring is, why this project uses it, what to install with the version pinned, what it costs, and that skipping this project loses none of the architecture.
...
- `./gradlew build` passes. 11 test methods across `ArchitectureRuleCatchesTheShortcutTest`, `ArchitectureTest`, `DemoRunsTest`, `MoneyTest`, `PlaceOrderInteractorTest`, `WiringContrastTest`.
...
| `clean-architecture-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/clean-architecture-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Clean Architecture with Spring pattern in
...
| Runtime | ~9:09 over 14 scenes, 134 subtitle cues |  |
...
> `python3 ../../docs/make_youtube_docs.py clean-architecture-with-spring`. They are the one part of
...
integrated, -3.98 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py clean-architecture-with-spring
python3 ../../docs/make_specs.py clean-architecture-with-spring
```
