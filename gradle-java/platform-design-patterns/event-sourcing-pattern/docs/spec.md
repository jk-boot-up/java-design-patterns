# Event Sourcing Pattern — Project Specification
...
[`event-sourcing-pattern-explained.md`](event-sourcing-pattern-explained.md); the problem it
...
- Not a database, and not durability. The event store is an `ArrayList`. Ordering and durability are assumed, not demonstrated, and the README says so.
- Not a concurrency lesson. There is one thread and no optimistic concurrency check, so the question of two writers appending to one stream at once does not arise here.
- Not a performance measurement. The cost figures in Act 6 are events examined, not milliseconds — a count the test can assert exactly, where a timing could not.
- Not CQRS. The two are separate decisions, and Act 9 exists to prove it by building each one without the other.
- Not a recommendation. The project argues that most systems should keep the total, and the closing question is how to tell which kind you have.
...
13 production classes under `com.jk.explore.eventsourcing`:
...
1. **There is no balance field.** `EventSourcedLoyaltyAccounts` holds no stored total; every number it returns is a fold over the customer's events, worked out on the call.
2. **Every answer explains itself.** `explain` prints one line per event with the running total, including the expiry — the line support needs when a customer asks where their points went.
3. **A past question needs no forethought.** `balanceOn` answers what the balance was on an earlier date without any history table having been designed in advance.
4. **The repair is a read, not a write.** The duplicate-award fix changes how the log is interpreted; a test asserts the log is byte-identical before and after, because the shop really did award twice and the log was never wrong.
5. **Replay cost is stated as a number.** `SnapshotTest` asserts 5,000 events examined for one balance, 5,001 after one more order, and 1 with a snapshot — the same answer either way.
6. **A stale snapshot is silently wrong.** A snapshot taken under the buggy interpretation reports 90 where the repaired fold gives 45, with nothing thrown and nothing logged; the cure asserted is to discard every snapshot and refold.
7. **Erasure fights the pattern.** Deleting a customer's events destroys the history that explained them *and* leaves a snapshot still answering with their balance — both asserted, so the difficulty is shown rather than described.
8. **Old events never gain a field.** Awards written before order ids were recorded make the duplicate hunt return an empty list on a stream that plainly contains two identical awards.
9. **CQRS and event sourcing are separated.** `CqrsDistinctionTest` builds each without the other, including a read model that falls behind its row, so the two are shown to be independent decisions.
10. **Every quoted number is the program's.** `DemoRunsTest` captures the demo's output and asserts the figures the README, the notes and the video narration quote, and that two runs are byte-identical.
...
- `./gradlew build` passes. 31 test methods across `BugInvestigationTest`, `CqrsDistinctionTest`, `CurrentStateLoyaltyAccountsTest`, `DemoRunsTest`, `ErasureAndEventVersioningTest`, `EventSourcedLoyaltyAccountsTest`, `SnapshotTest`.
...
| `event-sourcing-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/event-sourcing-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Event Sourcing pattern in
...
| Runtime | ~17:00 over 16 scenes, 251 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Event Sourcing in Java - Why Is This Balance 140?"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Strangler Fig.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py event-sourcing`. They are the one part of
...
integrated, -3.86 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py event-sourcing
python3 ../../docs/make_specs.py event-sourcing
```
