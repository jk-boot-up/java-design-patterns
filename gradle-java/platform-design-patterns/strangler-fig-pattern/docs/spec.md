# Strangler Fig Pattern — Project Specification
...
[`strangler-fig-pattern-explained.md`](strangler-fig-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.stranglerfig`:
...
1. **Shadow reads catch real differences.** `StranglerFigTest` finds the VAT rounding and the exactly-fifty-pounds delivery difference on a fixed-seed stream.
2. **Rollback is per capability.** One switch flips and pricing stays moved.
3. **The stall is named and modelled.** The cost figures are labelled as assumptions.
4. **The verdict is stated aloud.** Finish it, or do not start.
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `StranglerFigTest`.
...
| `strangler-fig-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/strangler-fig-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Strangler Fig pattern in
...
| Runtime | ~7:11 over 16 scenes, 102 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Layered Architecture.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py strangler-fig`. They are the one part of
...
Last verified: all eleven items pass. The delivered MP4 measures -15.98 LUFS
...
```bash
python3 ../../docs/make_youtube_docs.py strangler-fig
python3 ../../docs/make_specs.py strangler-fig
```
