#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Strangler Fig video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Strangler Fig  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("STRANGLER", 152, TEXT)
    fitted("FIG", 288, ACCENT)
    ...
    # One switch versus a switch per capability -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "cutover(ALL_FOUR)",
         "one switch, one Monday", tag="BIG BANG")
    pill(d, 1070, 556, 700, 170, GREEN, "route(PRICING, NEW)",
         "one capability at a time", tag="STRANGLER FIG")
    ...
    d.text((W // 2, 744), "No cutover weekend.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "But finish it.",
