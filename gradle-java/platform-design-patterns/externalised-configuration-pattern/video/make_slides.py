#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Externalised Configuration video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Externalised Configuration  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson. The left column is the path one quote
    # takes, top to bottom, and it ends at a box that stores text and has no
    # opinion about it. The middle column holds the pieces that had to be
    # built because of that. The right column is the invoice: the four guards
    # the value lost on its way out of the source file, and their replacements.
    box(d, 70, 244, 500, 118, "ConfiguredCheckout",
        "«caller»   asks on every quote, never in the constructor", CLIENT,
        label_size=34, sub_size=21)

    d.text((320, 370), "money(setting)", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    arrow(d, 320, 402, 320, 428, CLIENT, 4)

    box(d, 70, 432, 500, 118, "SettingsReader",
        "«the boundary»   the one place that knows outside exists", AMBER,
        label_size=34, sub_size=21)

    d.text((320, 558), "lookup(key)", font=f(SANS_B, 24), fill=AMBER, anchor="ma")
    arrow(d, 320, 590, 320, 616, AMBER, 4)

    box(d, 70, 620, 500, 118, "ConfigSource",
        "«the source»   stores text. It has no opinion about it.", GREEN,
        label_size=34, sub_size=21)

    box(d, 70, 758, 500, 80, 'delivery.freeOver = "35"',
        "a string. Not money, and not checked.", GREEN,
        label_size=27, sub_size=20)

    # The middle column: the starting point we are replacing, the two guards
    # that are objects, and the record that carries provenance back out.
    box(d, 640, 244, 540, 118, "HardCodedCheckout",
        "«where we started»   correct, and a release away from £35", RED,
        label_size=34, sub_size=21)

    d.text((910, 384), "there is nothing wrong with it.",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    box(d, 640, 432, 540, 118, "MoneySetting",
        "«the schema»   key, default, lowest, highest", CYAN,
        label_size=34, sub_size=21)
    arrow_left(d, 636, 491, 578, CYAN, 4)

    box(d, 640, 620, 540, 118, "ChangeLog",
        "«the trail»   append only. who, when, and what it was", ROSE,
        label_size=34, sub_size=21)
    arrow_left(d, 636, 679, 578, ROSE, 4)

    box(d, 640, 758, 540, 80, "SettingValue",
        "the amount, and where it came from", ACCENT,
        label_size=27, sub_size=20)

    # The right column: the invoice, which is the half of this pattern that
    # most write-ups leave out.
    d.text((1560, 248), "WHAT THE VALUE LEFT BEHIND",
           font=f(SANS_B, 28), fill=AMBER, anchor="ma")
    d.line([(1250, 294), (1870, 294)], fill=LINE, width=2)

    guards = [
        ('the compiler refusing "fifty"', "a typed setting that parses, or rejects", CYAN),
        ("a reviewer querying -1", "a declared range it must fall inside", CYAN),
        ("version control's history", "an audit trail of every write", ROSE),
        ("a revert and a redeploy", "a rollback as fast as the change", GREEN),
    ]
    y = 324
    for label, sub, colour in guards:
        box(d, 1250, y, 620, 94, label, "->   " + sub, colour,
            label_size=27, sub_size=21)
        y += 106

    box(d, 1250, 758, 620, 80, "ReleasePipeline",
        "«the price tag»   2h 15m of work, live Monday 10:45", MUTED,
        label_size=27, sub_size=20)

    d.text((W // 2, 900),
           "One line moved the value outside.  Everything else on this slide is "
           "what it cost to let it.",
           font=f(SANS_B, 28), fill=AMBER, anchor="ma")
...
def kind_poster(scene, img, d):
    ...
    fitted("EXTERNALISED", 152, TEXT)
    fitted("CONFIGURATION", 288, ACCENT)
    ...
    # Where the number lives, before and after. Both are real lines from the
    # project, so the poster shows the actual change rather than a slogan, and
    # the verdicts are times rather than opinions.
    pill(d, 150, 556, 700, 170, RED, "= Money.pounds(50)",
         "correct — and 2 days from £35", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "settings.money(setting)",
         "4 seconds from £35", tag="AFTER")
    ...
    d.text((W // 2, 744), "Changing one number should not need a release.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Move it out — then rebuild its four guards.",
