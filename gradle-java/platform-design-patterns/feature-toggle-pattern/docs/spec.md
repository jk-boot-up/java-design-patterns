# Feature Toggle Pattern — Project Specification
...
[`feature-toggle-pattern-explained.md`](feature-toggle-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.featuretoggle`:
...
1. **Rollout is by customer number modulo 100,** so counts are exact.
2. **A switch takes effect with no new checkout.**
3. **Unknown toggles and an unreachable table mean off.**
4. **Stale toggles are found by age and settled state.**
...
- `./gradlew build` passes. 6 test methods across `DemoRunsTest`, `FeatureToggleTest`.
...
| `feature-toggle-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/feature-toggle-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Feature Toggle pattern in
...
| Runtime | ~4:44 over 14 scenes, 72 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Service Mesh.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py feature-toggle`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py feature-toggle
python3 ../../docs/make_specs.py feature-toggle
```
