#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Feature Toggle video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Feature Toggle  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("FEATURE", 152, TEXT)
    fitted("TOGGLE", 288, ACCENT)
    ...
    # Releasing by deploying versus by a switch -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "deploy to release",
         "deploy again to undo", tag="2 DEPLOYS")
    pill(d, 1070, 556, 700, 170, GREEN, "flip a switch",
         "flip it back", tag="0 DEPLOYS")
    ...
    d.text((W // 2, 744), "Ship it switched off.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Turn it on with a setting.",
