# Blue-Green and Canary Pattern — Project Specification
...
[`blue-green-and-canary-pattern-explained.md`](blue-green-and-canary-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.bluegreen`:
...
1. **Routing is a fixed rule on the request number,** so every run is the same.
2. **A buggy release fails on big orders, one in ten.**
3. **Going back needs only a setting.**
4. **The gate halts a bad release after one step, and promotes a good one.**
...
- `./gradlew build` passes. 6 test methods across `BlueGreenTest`, `DemoRunsTest`.
...
| `blue-green-and-canary-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/blue-green-and-canary-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Blue-Green and Canary pattern in
...
| Runtime | ~5:03 over 14 scenes, 75 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Feature Toggle.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py blue-green-and-canary`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py blue-green-and-canary
python3 ../../docs/make_specs.py blue-green-and-canary
```
