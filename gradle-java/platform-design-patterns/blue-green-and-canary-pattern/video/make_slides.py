#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Blue-Green and Canary video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Blue-Green and Canary  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("BLUE-GREEN", 152, TEXT)
    fitted("AND CANARY", 288, ACCENT)
    ...
    # Replacing in place versus going live beside -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "replace in place",
         "10 of 100 fail", tag="DOWNTIME")
    pill(d, 1070, 556, 700, 170, GREEN, "run beside, then switch",
         "0 of 100 fail", tag="NO DOWNTIME")
    ...
    d.text((W // 2, 744), "Run it beside the old one.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Switch, or grow a share.",
