#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Backends for Frontends video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Backends for Frontends  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in body:
        ...
        elif ln.startswith(("each ", "own ", "shape ", "so half")):
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson. The left column is the pattern, read
    # bottom-up: two backends, both live, both pointing at one interface, over
    # one shop. The two arrows converging on ClientBackend are the whole
    # argument, so they are the only arrows drawn in the accent colour and the
    # only ones that meet. The middle column is the two designs this replaces,
    # plus the ruler that decides what "waste" means. The right column is the
    # invoice.
    box(d, 70, 244, 500, 118, "ClientBackend",
        "«the interface»   client() and productScreen(sku)", ACCENT,
        label_size=34, sub_size=21)

    # Both arrowheads land on the same edge. This is the pattern: not one
    # implementation chosen from two, but two running at once.
    arrow_up(d, 190, 428, 370, ACCENT, 4)
    arrow_up(d, 450, 428, 370, ACCENT, 4)

    box(d, 70, 432, 240, 118, "MobileBff",
        "6 fields · 196 bytes", CLIENT, label_size=30, sub_size=21)
    box(d, 330, 432, 240, 118, "WebBff",
        "15 fields · 1391 bytes", GREEN, label_size=30, sub_size=21)

    d.text((320, 566), "both live, at the same time, on purpose",
           font=f(SANS_B, 23), fill=AMBER, anchor="ma")

    box(d, 70, 620, 500, 118, "Shop",
        "«the data»   catalog · pricing · inventory · reviews", MUTED,
        label_size=34, sub_size=21)

    box(d, 70, 758, 500, 80, "4 internal calls, not 5",
        "no recommendations: this screen has no strip", CLIENT,
        label_size=27, sub_size=20)

    # The middle column: the two designs the pattern replaces, and the ruler.
    # Nothing points at the top two boxes, which is the point -- they are not
    # participants.
    box(d, 640, 244, 540, 118, "ChattyPhone",
        "«design one»   5 device calls · 29 fields · 6 drawn", RED,
        label_size=34, sub_size=21)

    box(d, 640, 432, 540, 118, "SharedApi",
        "«design two»   1 call · 1755 bytes · 87% unused", RED,
        label_size=34, sub_size=21)

    d.text((910, 566), "?fields= fixes the size. Not whose queue you are in.",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # The ruler points up and left, at the two backends rather than at the shop,
    # because Screens measures what each backend sends and nothing writes to it.
    box(d, 640, 620, 540, 118, "Screens",
        "«the ruler»   PHONE = 6 names · WEB = 15 names", CYAN,
        label_size=34, sub_size=21)
    arrow_diag_up(d, 640, 650, 580, 512, CYAN, 4)
    d.text((610, 600), "measured against", font=f(SANS, 20), fill=CYAN, anchor="ra")

    box(d, 640, 758, 540, 80, "CallLog.Origin  ·  DEVICE | INTERNAL",
        "the two round trips that do not cost the same", CYAN,
        label_size=27, sub_size=20)

    # The right column: the invoice, which is the half of this pattern that
    # most write-ups leave out.
    d.text((1560, 248), "WHAT THE PATTERN COSTS",
           font=f(SANS_B, 28), fill=AMBER, anchor="ma")
    d.line([(1250, 294), (1870, 294)], fill=LINE, width=2)

    bill = [
        ("one rule copied into two backends", "Save £12.00 on one screen, nothing on the other", RED),
        ("four shared jobs, done twice", "8 copies. A gateway in front does 4.", RED),
        ("one backend per client", "6 clients, and only 3 disagree", RED),
        ("not one of the three throws", "the witness is a customer with both screens open", AMBER),
    ]
    y = 324
    ...
    box(d, 1250, 758, 620, 80, "Shop returns fixed data",
        "«why every figure here repeats»   nothing random", MUTED,
        label_size=27, sub_size=20)

    d.text((W // 2, 900),
           "Fourteen small classes.  The one that matters is an interface with "
...
def kind_poster(scene, img, d):
    ...
    # The title is two long words rather than the short ones the rest of the
    # series gets, so each line is fitted to the available width instead of
    # being set at a fixed size. At the ~360 px wide thumbnail YouTube serves
    # in search results this is the only text guaranteed to be readable, and it
    # is what tells someone whether the video is for them.
    ...
    fitted("BACKENDS FOR", 152, TEXT)
    fitted("FRONTENDS", 288, ACCENT)

    d.rectangle([110, 462, 620, 472], fill=RULE)

    # One box around the whole idea - the two ways of recording the same work
    # and the sentence that explains them - so the tagline reads as part of the
    # pattern rather than as a caption floating under it.
    d.rounded_rectangle([110, 516, W - 110, 866], radius=26,
                        outline=ACCENT, width=4)

    # The same product screen, fetched two ways. Both request lines are real
    # and both verdicts are the program's actual output rather than opinions.
    pill(d, 150, 556, 700, 170, RED, "GET /api/products/4417",
         "1755 bytes · 87% unused", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "GET /phone/product/4417",
         "196 bytes · 6 fields, all drawn", tag="AFTER")
    ...
    d.text((W // 2, 744), "One endpoint for every screen fits none of them.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "A backend per screen does — and here is what it costs.",
