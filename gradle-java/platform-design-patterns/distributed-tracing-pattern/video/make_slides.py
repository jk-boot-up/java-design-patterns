#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Distributed Tracing video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Distributed Tracing  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson. The left column is the path one unit of
    # work takes, top to bottom, and it ends at the single string field that
    # the whole pattern rests on. The middle column is what is derived from
    # that field, reading upwards -- so the arrows point back at the left
    # column, because nothing in the middle is recorded, it is all worked out
    # afterwards. The right column is the invoice.
    box(d, 70, 244, 500, 118, "ProductPage",
        "«caller»   opens a span around each unit of work", CLIENT,
        label_size=34, sub_size=21)

    d.text((320, 370), "start(context, name)", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    arrow(d, 320, 402, 320, 428, CLIENT, 4)

    box(d, 70, 432, 500, 118, "Tracer.Scope",
        "«the boundary»   the closing brace records the span", AMBER,
        label_size=34, sub_size=21)

    d.text((320, 558), "close()", font=f(SANS_B, 24), fill=AMBER, anchor="ma")
    arrow(d, 320, 590, 320, 616, AMBER, 4)

    box(d, 70, 620, 500, 118, "Span",
        "«the record»   name, start, duration, and its parent", GREEN,
        label_size=34, sub_size=21)

    box(d, 70, 758, 500, 80, 'parentSpanId = "span-5"',
        "one string. This is the whole pattern.", GREEN,
        label_size=27, sub_size=20)

    # The middle column: what the parent field replaces, and the two things
    # derived from it. Both arrows point left, because Trace and Waterfall
    # read spans and nothing writes to them.
    box(d, 640, 244, 540, 118, "InterleavedLog",
        "«where we started»   four correct logs, and no answer", RED,
        label_size=34, sub_size=21)

    d.text((910, 384), "connected to nothing. It is what this replaces.",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    box(d, 640, 432, 540, 118, "Trace",
        "«the arithmetic»   roots, children, selfTime", CYAN,
        label_size=34, sub_size=21)
    arrow_left(d, 636, 491, 578, CYAN, 4)

    # Deliberately not ROSE here. The right-hand column is the bill and is drawn
    # in red, so a red box on the left would read as another failure rather than
    # as the thing the pattern produces.
    box(d, 640, 620, 540, 118, "Waterfall",
        "«the drawing»   derived. Trace has never heard of it.", ACCENT,
        label_size=34, sub_size=21)
    arrow_left(d, 636, 679, 578, ACCENT, 4)

    box(d, 640, 758, 540, 80, "ranking-model  340ms  =  37%",
        "computed afterwards, by subtraction", ACCENT,
        label_size=27, sub_size=20)
    ...
    bill = [
        ("a service that opens no span", "one root, no orphans, wrong answer", RED),
        ("a context lost across a thread", "two roots, 400ms belonging to nobody", RED),
        ("a trace sampled away at the door", "gone before anybody knew it mattered", RED),
        ("not one of the three throws", "so nothing tells you the answer moved", AMBER),
    ]
    y = 324
    ...
    box(d, 1250, 758, 620, 80, "Clock.Scripted",
        "«why every figure here repeats»   no wall clock", MUTED,
        label_size=27, sub_size=20)

    d.text((W // 2, 900),
           "Six small classes.  The one that matters is a single string field on "
           "Span.",
           font=f(SANS_B, 28), fill=AMBER, anchor="ma")
...
def kind_poster(scene, img, d):
    ...
    fitted("DISTRIBUTED", 152, TEXT)
    fitted("TRACING", 288, ACCENT)
    ...
    # What each approach records about the same piece of work, and what each one
    # can then answer. Both samples are real lines, and both verdicts are the
    # program's actual output rather than opinions.
    pill(d, 150, 556, 700, 170, RED, 'log("quote complete")',
         "180ms? or 220ms?", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'start(parent, "pricing")',
         "340ms — 37% of the page", tag="AFTER")
    ...
    d.text((W // 2, 744), "Four correct logs cannot say where the time went.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "One parent field can — and here is what it costs.",
