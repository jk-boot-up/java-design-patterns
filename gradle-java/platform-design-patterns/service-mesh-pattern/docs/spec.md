# Service Mesh Pattern — Project Specification
...
[`service-mesh-pattern-explained.md`](service-mesh-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.servicemesh`:
...
1. **The payment service refuses a set number of calls,** so every run is the same.
2. **One mesh policy gives every caller the same behaviour.**
3. **A denied caller never reaches the payment service.**
4. **Retries multiply the calls the payment service receives, and this is counted.**
...
- `./gradlew build` passes. 6 test methods across `DemoRunsTest`, `ServiceMeshTest`.
...
| `service-mesh-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/service-mesh-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Service Mesh pattern in
...
| Runtime | ~4:59 over 14 scenes, 71 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Consumer-Driven Contract.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py service-mesh`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py service-mesh
python3 ../../docs/make_specs.py service-mesh
```
