#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Service Mesh video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Service Mesh  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # Three retry behaviours versus one policy -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "each service retries alone",
         "3 services, 3 behaviours", tag="3 BEHAVIOURS")
    pill(d, 1070, 556, 700, 170, GREEN, "proxies retry for all",
         "one policy, set once", tag="ONE POLICY")
    ...
    d.text((W // 2, 744), "A proxy beside each service.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "One policy for every call.",
