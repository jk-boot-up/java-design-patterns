#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Sidecar on Kubernetes video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Sidecar on Kubernetes  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("SIDECAR ON", 152, TEXT)
    fitted("KUBERNETES", 288, ACCENT)
    ...
    # Shared by configuration versus shared by definition -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "network_mode: service:checkout",
         "a line you can forget", tag="COMPOSE")
    pill(d, 1070, 556, 700, 170, GREEN, "containers: [checkout, proxy]",
         "one Pod, one network", tag="A POD")
    ...
    d.text((W // 2, 744), "Two containers, one Pod.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "One network, one fate.",
