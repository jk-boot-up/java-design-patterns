# Sidecar on Kubernetes Pattern — Project Specification
...
[`sidecar-on-kubernetes-pattern-explained.md`](sidecar-on-kubernetes-pattern-explained.md); the problem it
...
10 production classes under `com.jk.explore.sidecarkubernetes`:
...
1. **A crash does not take a neighbour with it.** `PodTest` asserts the checkout is untouched and only the proxy's restart count rises.
2. **Sharing by definition is proven.** The Compose model loses the line and the Pod cannot.
3. **Injection leaves the authored manifest unchanged.** And injecting twice adds nothing.
4. **Kubernetes is explained first.** `docs/dependencies.md` says what it is, what it costs, and that skipping loses nothing.
...
- `./gradlew build` passes. 10 test methods across `DemoRunsTest`, `PodTest`.
...
| `sidecar-on-kubernetes-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/sidecar-on-kubernetes-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Sidecar on Kubernetes pattern in
...
| Runtime | ~7:11 over 15 scenes, 101 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Event Sourcing.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py sidecar-on-kubernetes`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py sidecar-on-kubernetes
python3 ../../docs/make_specs.py sidecar-on-kubernetes
```
