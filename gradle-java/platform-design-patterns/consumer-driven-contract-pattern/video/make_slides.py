#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Consumer-Driven Contract video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Consumer-Driven Contract  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("CONSUMER-DRIVEN", 152, TEXT)
    fitted("CONTRACT", 288, ACCENT)
    ...
    # Found in production versus found before release -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "rename, and release",
         "checkout fails in production", tag="FOUND BY A CUSTOMER")
    pill(d, 1070, 556, 700, 170, GREEN, "verify against contracts",
         "build fails, names checkout", tag="FOUND BEFORE RELEASE")
    ...
    d.text((W // 2, 744), "Consumers write it down.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Providers check before release.",
