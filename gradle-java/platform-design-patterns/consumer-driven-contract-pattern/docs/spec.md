# Consumer-Driven Contract Pattern — Project Specification
...
[`consumer-driven-contract-pattern-explained.md`](consumer-driven-contract-pattern-explained.md); the problem it
...
7 production classes under `com.jk.explore.contract`:
...
1. **A contract lists only the fields a consumer reads.**
2. **The verifier checks the provider's real answer.**
3. **A problem names the consumer and the field.**
4. **An added field passes.**
5. **A change of meaning passes, and the demo says so.**
...
- `./gradlew build` passes. 7 test methods across `ContractTest`, `DemoRunsTest`.
...
| `consumer-driven-contract-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/consumer-driven-contract-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Consumer-Driven Contract pattern in
...
| Runtime | ~4:43 over 14 scenes, 73 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Multiton.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py consumer-driven-contract`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py consumer-driven-contract
python3 ../../docs/make_specs.py consumer-driven-contract
```
