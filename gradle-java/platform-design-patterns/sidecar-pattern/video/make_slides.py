#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Sidecar video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Sidecar  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in body:
        ...
        elif ln.startswith(("separate process", "beside ·", "Does this concern",
                            "Must it work", "Yes to either", "No to both")):
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson. The left column is the pattern, read top
    # down: one interface, the service that is left almost empty under it, the
    # proxy it hands everything to, and -- underneath all four proxies -- one
    # configuration object. The middle column is the arrangement being replaced
    # and the gateway that does the counting. The right column is the bill,
    # which is the half of this pattern most write-ups leave out.
    box(d, 70, 244, 500, 118, "TakesPayments",
        "«the interface»   name() and pay(payment)", ACCENT,
        label_size=34, sub_size=21)

    arrow_up(d, 320, 428, 370, ACCENT, 4)

    box(d, 70, 432, 500, 118, "ServiceBehindASidecar",
        "pay() is one line:  return sidecar.send(payment)", GREEN,
        label_size=32, sub_size=21)

    arrow(d, 320, 556, 320, 616, GREEN, 4)
    d.text((346, 572), "localhost, nothing else",
           font=f(SANS, 20), fill=GREEN, anchor="la")

    box(d, 70, 620, 500, 118, "Sidecar",
        "«the proxy, next door»   retry loop · 1ms hop", CYAN,
        label_size=34, sub_size=21)

    box(d, 70, 758, 500, 80, "SidecarConfig",
        "one object, read by four proxies — not four copies", AMBER,
        label_size=27, sub_size=20)

    # The middle column: the four services as they were, and the two classes
    # that do the counting. Nothing points at the top two boxes, which is the
    # point -- after the pattern lands, they are not participants any more.
    box(d, 640, 244, 540, 118, "Checkout · Refunds · Payouts",
        "«the old way»   4 fields and a retry loop, each", RED,
        label_size=32, sub_size=21)

    box(d, 640, 432, 540, 118, "SubscriptionBillingService",
        "no applyPolicyReview() — the absence IS the fault", RED,
        label_size=32, sub_size=21)

    d.text((910, 568), "4 services × 4 decisions = 16 copies, in 4 places",
           font=f(SANS_B, 22), fill=AMBER, anchor="ma")

    box(d, 640, 620, 540, 118, "PaymentGateway",
        "«fails on purpose»   300ms wobble · 12 attempts", CYAN,
        label_size=34, sub_size=21)

    box(d, 640, 758, 540, 80, "CallLog",
        "every count here is the gateway's, never a service's", CYAN,
        label_size=27, sub_size=20)

    # The right column: what the pattern charges. Three of these four are
    # prices rather than mistakes, and the fourth is the misuse to avoid.
    d.text((1560, 248), "WHAT THE PATTERN COSTS",
           font=f(SANS_B, 28), fill=AMBER, anchor="ma")
    d.line([(1250, 294), (1870, 294)], fill=LINE, width=2)

    bill = [
        ("twice as many processes", "4 -> 8, each wanting memory and a patch", RED),
        ("a second thing that can be down", "proxy down: 0 attempts reached the gateway", RED),
        ("one millisecond, on every call", "600ms -> 603ms. On a 2ms call, fifty per cent.", RED),
        ("network facts only", "a shop rule in a proxy config is lost, not broken", AMBER),
    ]
    y = 324
    ...
    d.text((W // 2, 900),
           "Nineteen small classes.  The one that matters is a process "
           "boundary you cannot see in the code.",
           font=f(SANS_B, 28), fill=AMBER, anchor="ma")
...
def kind_poster(scene, img, d):
    ...
    fitted("THE SIDECAR", 152, TEXT)
    fitted("PATTERN", 288, ACCENT)
    ...
    # 16 copies and one missed update on the left, one shared config object and
    # a clean night on the right.
    pill(d, 150, 556, 700, 170, RED, "retryPolicy × 4 services",
         "16 copies · 3 updated, 1 missed", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "one proxy beside each",
         "1 place to edit · 12 of 12, 0 refused", tag="AFTER")
    ...
    d.text((W // 2, 744), "Four correct services. One incident nobody can be blamed for.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Move the concern next door — and here is what it costs.",
