# Event Bus Pattern — Teaching Video

A narrated, slide-based video that teaches Event Bus, and argues for when not to use it,
...
| File | What it is |
| --- | --- |
| `event-bus-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. |
| `event-bus-pattern-explained.m4a` | Audio-only version. |
| `event-bus-pattern-explained.srt` | Subtitles. |
...
| # | Scene | Covers |
| --- | --- | --- |
| 1 | Event Bus | Title card and the definition |
...
`python3 ../../docs/make_narration.py --force event-bus`, then
...
Upload `event-bus-pattern-explained.mp4`, with
