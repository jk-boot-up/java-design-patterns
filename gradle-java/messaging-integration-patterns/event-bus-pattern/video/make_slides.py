#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Event Bus video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Event Bus  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("EVENT BUS", 152, TEXT)
    fitted("ONE MEETING PLACE", 288, ACCENT)
    ...
    # A web of references versus a bus -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "5 components, each knowing the rest",
         "20 references", tag="A WEB")
    pill(d, 1070, 556, 700, 170, GREEN, "5 components, one bus",
         "5 references", tag="A HUB")
    ...
    d.text((W // 2, 744), "Post, and subscribe.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Hold no other reference.",
