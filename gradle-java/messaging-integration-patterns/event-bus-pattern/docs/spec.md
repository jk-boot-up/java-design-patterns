# Event Bus Pattern — Project Specification
...
[`event-bus-pattern-explained.md`](event-bus-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.eventbus`:
...
1. **A supertype subscriber hears every subtype.**
2. **A failing subscriber does not stop the others or the poster.**
3. **A dead event is posted once and never loops.**
4. **An uncancelled subscription is held, and a cancelled one is not.**
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `EventBusTest`.
...
| `event-bus-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/event-bus-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Event Bus pattern in
...
| Runtime | ~4:56 over 14 scenes, 76 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Event-Driven Architecture.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py event-bus`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py event-bus
python3 ../../docs/make_specs.py event-bus
```
