# Content-Based Router Pattern — Project Specification
...
[`content-based-router-pattern-explained.md`](content-based-router-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.contentrouter`:
...
1. **Every order goes to exactly one channel** when there is a fallback.
2. **Rule order changes the answer,** asserted.
3. **Without a fallback, an unmatched order is dropped and counted.**
4. **A renamed field value makes a rule miss,** asserted.
...
- `./gradlew build` passes. 7 test methods across `ContentRouterTest`, `DemoRunsTest`.
...
| `content-based-router-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/content-based-router-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Content-Based Router pattern in
...
| Runtime | ~5:15 over 14 scenes, 79 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Splitter and Aggregator.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py content-based-router`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py content-based-router
python3 ../../docs/make_specs.py content-based-router
```
