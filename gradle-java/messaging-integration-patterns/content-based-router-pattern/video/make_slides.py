#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Content-Based Router video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Content-Based Router  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("CONTENT-BASED", 152, TEXT)
    fitted("ROUTER", 288, ACCENT)
    ...
    # No fallback versus a fallback -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "no rule matches, no fallback",
         "order dropped, no sign", tag="LOST")
    pill(d, 1070, 556, 700, 170, GREEN, "no rule matches, a fallback",
         "manual review", tag="CAUGHT")
    ...
    d.text((W // 2, 744), "Look inside, then route.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Always have a fallback.",
