#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Dead Letter Channel video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Dead Letter Channel  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("DEAD LETTER", 152, TEXT)
    fitted("CHANNEL", 288, ACCENT)
    ...
    # A poison message blocking versus moved aside -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "no dead letter channel",
         "orders 3 and 4 stuck", tag="BLOCKED")
    pill(d, 1070, 556, 700, 170, GREEN, "after 3 attempts",
         "moved aside, 3 and 4 through", tag="MOVED ON")
    ...
    d.text((W // 2, 744), "Try, then move it aside.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Then look at it.",
