# Dead Letter Channel Pattern — Project Specification
...
[`dead-letter-channel-pattern-explained.md`](dead-letter-channel-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.deadletter`:
...
1. **Attempts are counted:** 11 for a blocked line, and 3 per dead letter.
2. **A transient failure that recovers is not a dead letter.**
3. **Replay after a fix handles the message, last.**
4. **Replay before the fix returns it to the dead letters.**
...
- `./gradlew build` passes. 8 test methods across `DeadLetterTest`, `DemoRunsTest`.
...
| `dead-letter-channel-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/dead-letter-channel-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Dead Letter Channel pattern in
...
| Runtime | ~4:54 over 14 scenes, 78 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Event Bus.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py dead-letter-channel`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py dead-letter-channel
python3 ../../docs/make_specs.py dead-letter-channel
```
