#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Splitter and Aggregator video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Splitter and Aggregator  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("SPLITTER AND", 152, TEXT)
    fitted("AGGREGATOR", 288, ACCENT)
    ...
    # A missing part with a timeout -- a real line from the project.
    pill(d, 150, 556, 700, 170, RED, "part 2 never arrives",
         "no timeout: waits for ever", tag="STUCK")
    pill(d, 1070, 556, 700, 170, GREEN, "after 30 minutes",
         "2 of 3 lines, part 2 missing", tag="GIVES UP")
    ...
    d.text((W // 2, 744), "Split by line, gather by id.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Put it back in order.",
