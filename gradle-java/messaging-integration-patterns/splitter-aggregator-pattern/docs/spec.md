# Splitter and Aggregator Pattern — Project Specification
...
[`splitter-aggregator-pattern-explained.md`](splitter-aggregator-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.splitteraggregator`:
...
1. **Parts in any order come back in line order.**
2. **Two orders interleaved do not mix.**
3. **A duplicate part is counted once.**
4. **A partial result appears exactly at the timeout,** not a minute before.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `SplitterAggregatorTest`.
...
| `splitter-aggregator-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/splitter-aggregator-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Splitter and Aggregator pattern in
...
| Runtime | ~4:53 over 14 scenes, 74 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Dead Letter Channel.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py splitter-aggregator`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py splitter-aggregator
python3 ../../docs/make_specs.py splitter-aggregator
```
