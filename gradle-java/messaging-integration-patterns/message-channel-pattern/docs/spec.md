# Message Channel Pattern — Project Specification
...
[`message-channel-pattern-explained.md`](message-channel-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.messagechannel`:
...
1. **No threads or clocks:** every run is the same.
2. **Messages arrive in the order sent.**
3. **A wrong-type message is refused and nothing is queued.**
4. **A full channel refuses and loses nothing already accepted.**
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `MessageChannelTest`.
...
| `message-channel-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/message-channel-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Message Channel pattern in
...
| Runtime | ~4:41 over 14 scenes, 72 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Content-Based Router.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py message-channel`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py message-channel
python3 ../../docs/make_specs.py message-channel
```
