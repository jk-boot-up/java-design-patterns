#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Message Channel video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Message Channel  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("MESSAGE CHANNEL", 152, TEXT)
    fitted("A QUEUE BETWEEN THEM", 288, ACCENT)
    ...
    # A direct call versus a channel -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "warehouse down, direct call",
         "3 checkouts failed", tag="FAILED")
    pill(d, 1070, 556, 700, 170, GREEN, "warehouse down, a channel",
         "0 failed, 3 waiting", tag="CARRIED ON")
    ...
    d.text((W // 2, 744), "Send, and carry on.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "The receiver takes it later.",
