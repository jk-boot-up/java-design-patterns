# Actor Pattern — Project Specification
...
[`actor-pattern-explained.md`](actor-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.actorpattern`:
...
1. **The lost update is forced** by a rendezvous, and asserted.
2. **Four thousand reservations from four threads lose nothing.**
3. **The actor exposes no public read of its stock.**
4. **A bad message restarts the actor, and the next one is handled.**
5. **Two actors that wait on each other never answer.**
...
- `./gradlew build` passes. 8 test methods across `ActorTest`, `DemoRunsTest`.
...
| `actor-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/actor-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Actor pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Two-Phase Termination.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py actor`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py actor
python3 ../../docs/make_specs.py actor
```
