#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Actor video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Actor  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("ACTOR", 152, TEXT)
    fitted("ONE OWNER, MANY MESSAGES", 288, ACCENT)
    ...
    # Shared state versus an actor -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "shared map, 2 threads",
         "one reservation lost", tag="LOST UPDATE")
    pill(d, 1070, 556, 700, 170, GREEN, "an actor, 4 threads",
         "4000 sent, stock 0", tag="NOTHING LOST")
    ...
    d.text((W // 2, 744), "One owner, one thread.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Everyone else sends messages.",
