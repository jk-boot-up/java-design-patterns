#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Future/Promise video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Future/Promise  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("FUTURE", 152, TEXT)
    fitted("PROMISE", 288, ACCENT)
    ...
    # Sequential versus concurrent -- both real lines from the project, so
    # the poster shows the actual contrast rather than a slogan.
    pill(d, 150, 556, 700, 170, RED, "price(); stock(); rating();",
         "600ms, one after another", tag="SEQUENTIAL")
    pill(d, 1070, 556, 700, 170, GREEN, "submit(); submit(); submit();",
         "~200ms, all three at once", tag="CONCURRENT")
    ...
    d.text((W // 2, 744), "Three lookups, none depending on the others.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Submit first. Ask for results last.",
