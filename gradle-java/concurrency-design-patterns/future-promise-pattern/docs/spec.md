# Future/Promise Pattern — Project Specification
...
[`future-promise-pattern-explained.md`](future-promise-pattern-explained.md); the problem it
...
- Not a `CompletableFuture` callback tutorial. `thenApply`, `thenCompose` and `thenCombine` are discussed as a named cost — readability collapsing past a few chained steps — not built up as a technique this project teaches to use.
- Not a re-teaching of the bounded queue or the worker pool. This project reuses §46's harness unchanged and assumes the reader has met either §46 or §47 already.
- Not an argument against `Future.cancel`. It is a real, useful signal — the point is narrower: it is a request a task must itself check for, never a guarantee.
...
1 production classes under `com.jk.explore.futurepromise`:
...
1. **Concurrency is proven, not inferred from a short elapsed time.** `ConcurrentProductPageTest` parks all three lookups on separate gates, confirms via a shared `CountDownLatch` that all three have started before any is released, and only then opens them — proving true overlap rather than merely a fast sequence.
2. **The reader/writer split is proven ordered, not merely fast.** `FutureAndPromiseTest` parks the writer's own work behind a `Gate` a second thread opens, so the writer's side effect can only be observed true after the gate opens — proving the reader's result cannot have arrived before the writer's work actually ran.
3. **The hang is proven to need no forcing at all.** `UnboundedWaitTest` parks a task on a `Gate` the test never opens; timing out is not a probability, it is the only possible outcome, on every run.
4. **Cancellation ignoring interruption is proven against a real race, not merely exercised.** `CooperativeCancellationTest` waits on a `CountDownLatch` confirming the task has actually started before calling `cancel(true)` — cancelling before the task starts would let the executor skip it entirely, proving nothing about a running task ignoring interruption.
5. **Nothing sleeps to wait for a race.** No test calls `Thread.sleep`; `LOOKUP_MILLIS` is the one named, measured subject reused from §46's convention, `CooperativeCancellation`'s three short sleeps are the anti-pattern being demonstrated, and the rescue timeout in act five ends a genuine hang rather than substituting for a `Gate`.
...
- `./gradlew build` passes. 10 test methods across `AsyncFailureTest`, `ConcurrentProductPageTest`, `CooperativeCancellationTest`, `DemoRunsTest`, `FutureAndPromiseTest`, `HarnessSelfTest`, `SequentialProductPageTest`, `UnboundedWaitTest`.
...
| `future-promise-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/future-promise-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Future/Promise pattern in
...
| Runtime | ~8:18 over 15 scenes, 120 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Read–Write Lock.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py future-promise`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py future-promise
python3 ../../docs/make_specs.py future-promise
```
