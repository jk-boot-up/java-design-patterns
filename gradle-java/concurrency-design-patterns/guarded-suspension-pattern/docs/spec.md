# Guarded Suspension Pattern — Project Specification
...
[`guarded-suspension-pattern-explained.md`](guarded-suspension-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.guardedsuspension`:
...
1. **Every result is exact:** thread states are polled to WAITING, and repeated runs are identical.
2. **The if guard hands one picker nothing;** the while guard does not.
3. **A picker that waits without looking first sleeps though an order is there.**
4. **notifyAll wakes every waiter,** counted.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `GuardedSuspensionTest`.
...
| `guarded-suspension-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/guarded-suspension-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Guarded Suspension pattern in
...
| Runtime | ~4:52 over 14 scenes, 75 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Thread-Local Storage.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py guarded-suspension`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py guarded-suspension
python3 ../../docs/make_specs.py guarded-suspension
```
