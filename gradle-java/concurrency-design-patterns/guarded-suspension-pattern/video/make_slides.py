#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Guarded Suspension video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Guarded Suspension  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("GUARDED", 152, TEXT)
    fitted("SUSPENSION", 288, ACCENT)
    ...
    # Asking again and again versus sleeping -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "asks and asks",
         "over a million checks, no order", tag="BUSY FOR NOTHING")
    pill(d, 1070, 556, 700, 170, GREEN, "waits",
         "thread WAITING, no processor", tag="SLEEPING")
    ...
    d.text((W // 2, 744), "Sleep until it is true.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Then check again.",
