#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Balking video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Balking  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("BALKING", 152, TEXT)
    fitted("RETURN AT ONCE", 288, ACCENT)
    ...
    # A careless balk versus a version counter -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "edit during a save, marked clean",
         "3 x MUG-BLUE never saved", tag="LOST")
    pill(d, 1070, 556, 700, 170, GREEN, "a version counter",
         "3 x MUG-BLUE saved next", tag="KEPT")
    ...
    d.text((W // 2, 744), "Wrong state: walk away.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "But do not lose the change.",
