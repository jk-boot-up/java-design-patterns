# Balking Pattern — Project Specification
...
[`balking-pattern-explained.md`](balking-pattern-explained.md); the problem it
...
7 production classes under `com.jk.explore.balkingpattern`:
...
1. **A save in progress is exact,** held at a gate inside the write.
2. **A second call balks at once,** asserted over repeated runs.
3. **The careless draft loses an edit and the versioned one keeps it,** both asserted.
4. **A balked request is not queued.**
...
- `./gradlew build` passes. 8 test methods across `BalkingTest`, `DemoRunsTest`.
...
| `balking-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/balking-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Balking pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Guarded Suspension.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py balking`. They are the one part of
...
integrated, -3.51 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py balking
python3 ../../docs/make_specs.py balking
```
