#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Active Object video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Active Object  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # A blocking call versus a call that returns a future -- both real
    # lines from the project, so the poster shows the actual contrast.
    pill(d, 150, 556, 700, 170, RED, "int stock = reserve(1);",
         "the caller waits for the work", tag="BLOCKING")
    pill(d, 1070, 556, 700, 170, GREEN, "Future f = reserve(1);",
         "returns now, answer later", tag="ACTIVE OBJECT")
    ...
    d.text((W // 2, 744), "One thread owns the state.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "So there is no lock.",
