# Active Object Pattern — Project Specification
...
[`active-object-pattern-explained.md`](active-object-pattern-explained.md); the problem it
...
- Not a re-teaching of the four projects it is made of. It links to each and covers only what the assembly adds.
- Not an actor framework. Actors and event loops are named as where the idea went and taught no further.
...
1 production classes under `com.jk.explore.activeobject`:
...
1. **The call is proven to return first.** The worker is parked on a `Gate`, confirmed by a `CountDownLatch`, before `reserve` is called.
2. **No lock is credited for correctness.** Four callers are released together and the total must be exact with no lock in the class.
3. **The error's stack is the worker's.** The exception is created inside the message and the test checks the calling class is absent.
4. **The ceiling is measured with real work, not sleeping.** Each message spins for 50 microseconds, and more callers must not double the rate.
...
- `./gradlew build` passes. 11 test methods across `DemoRunsTest`, `HarnessSelfTest`, `InventoryActiveObjectTest`, `MailboxTest`, `MonitorInventoryTest`.
...
| `active-object-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/active-object-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Active Object pattern in
...
| Runtime | ~6:16 over 14 scenes, 97 subtitle cues |  |
...
> `python3 ../../docs/make_youtube_docs.py active-object`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py active-object
python3 ../../docs/make_specs.py active-object
```
