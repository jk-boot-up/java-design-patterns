#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Two-Phase Termination video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Two-Phase Termination  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("TWO-PHASE", 152, TEXT)
    fitted("TERMINATION", 288, ACCENT)
    ...
    # Pulling the plug versus asking -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "close the ledger mid-order",
         "ORD-1 half written", tag="HALF WRITTEN")
    pill(d, 1070, 556, 700, 170, GREEN, "ask, then wait",
         "ORD-1 finished, 3 lines", tag="FINISHED")
    ...
    d.text((W // 2, 744), "Ask it to stop.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Then wait, with a limit.",
