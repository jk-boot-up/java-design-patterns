# Two-Phase Termination Pattern — Project Specification
...
[`two-phase-termination-pattern-explained.md`](two-phase-termination-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.twophase`:
...
1. **The half-written order is real:** the ledger records it.
2. **A requested stop finishes the order in progress and starts no other.**
3. **A flag alone leaves a waiting worker WAITING,** and an interrupt ends it.
4. **A worker that ignores the request is alive after the limit.**
5. **Pending orders are counted.**
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `TwoPhaseTest`.
...
| `two-phase-termination-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/two-phase-termination-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Two-Phase Termination pattern in
...
| Runtime | ~5:13 over 14 scenes, 80 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Message Channel.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py two-phase-termination`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py two-phase-termination
python3 ../../docs/make_specs.py two-phase-termination
```
