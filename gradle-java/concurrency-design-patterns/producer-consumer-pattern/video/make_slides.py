#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Producer-Consumer video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Producer-Consumer  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("PRODUCER", 152, TEXT)
    fitted("CONSUMER", 288, ACCENT)
    ...
    # An unbounded queue versus a bounded one -- both real lines from the
    # project, so the poster shows the actual contrast rather than a
    # slogan.
    pill(d, 150, 556, 700, 170, RED, "new Thread(...).start()",
         "no limit, ever", tag="PER ORDER")
    pill(d, 1070, 556, 700, 170, GREEN, "new BoundedOrderQueue(3)",
         "a bound, chosen on purpose", tag="THE PATTERN")
    ...
    d.text((W // 2, 744), "Orders arrive faster than they are packed.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "An unbounded queue is not a safer fix.",
