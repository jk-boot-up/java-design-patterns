# Thread-Local Storage Pattern — Project Specification
...
[`thread-local-storage-pattern-explained.md`](thread-local-storage-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.threadlocalstorage`:
...
1. **Two threads are held with both contexts set** before either reads, so isolation is exact.
2. **A reused thread leaks unless the context is cleared,** asserted both ways.
3. **The clear happens even when the work throws.**
4. **A pool thread does not see the submitter's context.**
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `ThreadLocalTest`.
...
| `thread-local-storage-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/thread-local-storage-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Thread-Local Storage pattern in
...
| Runtime | ~5:00 over 14 scenes, 78 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Fork-Join.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py thread-local-storage`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py thread-local-storage
python3 ../../docs/make_specs.py thread-local-storage
```
