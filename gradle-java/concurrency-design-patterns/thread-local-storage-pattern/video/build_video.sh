#!/usr/bin/env bash
#
# Builds the Thread-Local Storage pattern teaching video.
#
...
#
# Outputs:
#   thread-local-storage-pattern-explained.mp4   1080p H.264 + AAC, ready for YouTube
#   thread-local-storage-pattern-explained.m4a   audio-only version (podcast / revision)
...
OUT_VIDEO=thread-local-storage-pattern-explained.mp4
OUT_AUDIO=thread-local-storage-pattern-explained.m4a
OUT_SUBS=thread-local-storage-pattern-explained.srt
