#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Thread-Local Storage video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Thread-Local Storage  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("THREAD-LOCAL", 152, TEXT)
    fitted("STORAGE", 288, ACCENT)
    ...
    # A leak through a reused thread -- a real line from the project.
    pill(d, 150, 556, 700, 170, RED, "request A never cleared",
         "B logged as ada", tag="LEAKED")
    pill(d, 1070, 556, 700, 170, GREEN, "clear in a finally block",
         "B logged as nobody", tag="CLEARED")
    ...
    d.text((W // 2, 744), "Each thread has its own copy.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Always clear it.",
