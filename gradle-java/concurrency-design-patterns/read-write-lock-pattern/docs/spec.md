# Read–Write Lock Pattern — Project Specification
...
[`read-write-lock-pattern-explained.md`](read-write-lock-pattern-explained.md); the problem it
...
- Not a tour of every `java.util.concurrent.locks` class. `StampedLock` and optimistic reads are outside this project.
- Not a re-teaching of the harness. `Gate`, `Rendezvous` and `StepExecutor` are reused unchanged from §46.
- Not an argument against `ReentrantReadWriteLock`. Act six says when it loses; it does not say it never wins.
...
1 production classes under `com.jk.explore.readwritelock`:
...
1. **The torn read is forced, not hoped for.** The writer signals a `CountDownLatch` after setting the amount and then parks on a `Gate` before setting the currency, so the reader is proven to land in the gap on every run.
2. **Writer barging is proven from a genuinely queued writer.** `WriterBargingTest` confirms the writer is waiting on the lock before the second reader's `tryLock()` is attempted.
3. **The upgrade deadlock is rescued by a timeout the test owns.** `UpgradeDeadlockTest` requests the write lock while holding the read lock and asserts the request never completes on its own.
4. **Nothing sleeps to wait for a race.** No test calls `Thread.sleep`; the throughput figures in acts two, three and six are real measurements and are never asserted as exact numbers.
...
- `./gradlew build` passes. 10 test methods across `DemoRunsTest`, `HarnessSelfTest`, `ReadWriteCatalogueTest`, `SingleLockCatalogueTest`, `SnapshotCatalogueTest`, `UnsynchronizedCatalogueTest`, `UpgradeDeadlockTest`, `WriterBargingTest`.
...
| `read-write-lock-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/read-write-lock-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Read–Write Lock pattern in
...
| Runtime | ~6:40 over 14 scenes, 101 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Read-Write Lock in Java - A Thousand Readers, One Writer"*,
   56 characters. The suffix after the dash names the worked e-commerce scenario, so the title says what the viewer will actually watch rather than only which pattern it is about.
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Monitor Object.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py read-write-lock`. They are the one part of
...
integrated, -3.76 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py read-write-lock
python3 ../../docs/make_specs.py read-write-lock
```
