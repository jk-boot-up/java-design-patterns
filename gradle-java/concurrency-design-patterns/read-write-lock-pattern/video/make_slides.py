#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Read-Write Lock video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Read-Write Lock  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("READ-WRITE", 152, TEXT)
    fitted("LOCK", 288, ACCENT)
    ...
    # One lock for everyone versus readers sharing -- both real lines from
    # the project, so the poster shows the actual contrast, not a slogan.
    pill(d, 150, 556, 700, 170, RED, "lock.lock();  // everyone",
         "readers queue behind readers", tag="ONE LOCK")
    pill(d, 1070, 556, 700, 170, GREEN, "readLock().lock();",
         "readers share, a writer is alone", tag="READ-WRITE")
    ...
    d.text((W // 2, 744), "Two reads can never conflict.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Only a write can.",
