#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Thread Pool video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Thread Pool  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("THREAD", 152, TEXT)
    fitted("POOL", 288, ACCENT)
    ...
    # A pool with one bound versus a pool with two -- both real lines from
    # the project, so the poster shows the actual contrast rather than a
    # slogan.
    pill(d, 150, 556, 700, 170, RED, "newFixedThreadPool(2)",
         "queue capacity: unbounded", tag="THE DEFAULT TRAP")
    pill(d, 1070, 556, 700, 170, GREEN, "new ThreadPoolExecutor(1, 1, ...)",
         "queue capacity: 3, chosen on purpose", tag="THE PATTERN")
    ...
    d.text((W // 2, 744), "A fixed worker count is only one of two bounds.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "The queue behind it needs one too.",
