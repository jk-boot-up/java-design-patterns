#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Active Object with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Active Object with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # One thread versus a call that skips the proxy -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "this.restock(5) // caller's thread",
         "two threads change one field", tag="BYPASSED")
    pill(d, 1070, 556, 700, 170, GREEN, "inventory.restock(5) // mailbox",
         "one thread owns the state", tag="THROUGH THE PROXY")
    ...
    d.text((W // 2, 796), "Every access goes through it.",
