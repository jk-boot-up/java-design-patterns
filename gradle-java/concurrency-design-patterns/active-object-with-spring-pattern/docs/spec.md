# Active Object with Spring Pattern — Project Specification
...
[`active-object-with-spring-pattern-explained.md`](active-object-with-spring-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.activeobjectspring`:
...
1. **No lock is credited.** `ActiveObjectBeanTest` restocks a plain field from four threads and loses nothing.
2. **The lost update is forced.** A gate holds the worker mid-change while a call on `this` changes the field.
3. **The stale read is forced.** A gate holds the worker, and a direct read says 0 while a message read says 5.
4. **Only the throughput ratio depends on timing,** and its bound is generous.
...
- `./gradlew build` passes. 8 test methods across `ActiveObjectBeanTest`, `DemoRunsTest`.
...
| `active-object-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/active-object-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Active Object with Spring pattern in
...
| Runtime | ~5:54 over 16 scenes, 84 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Singleton with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py active-object-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py active-object-with-spring
python3 ../../docs/make_specs.py active-object-with-spring
```
