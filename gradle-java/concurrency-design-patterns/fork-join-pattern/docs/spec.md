# Fork-Join Pattern — Project Specification
...
[`fork-join-pattern-explained.md`](fork-join-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.forkjoin`:
...
1. **The recursive sum equals the loop for every threshold.**
2. **Leaves and tasks match the shape worked out with no threads.**
3. **Concurrency is exact:** pieces held at a gate, 4 with four workers and 2 with two, over repeated runs.
4. **Nothing is timed.**
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `ForkJoinTest`.
...
| `fork-join-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/fork-join-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Fork-Join pattern in
...
| Runtime | ~5:18 over 14 scenes, 76 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Actor.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py fork-join`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py fork-join
python3 ../../docs/make_specs.py fork-join
```
