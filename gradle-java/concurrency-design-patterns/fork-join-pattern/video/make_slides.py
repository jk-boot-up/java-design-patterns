#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Fork-Join video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Fork-Join  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("FORK-JOIN", 152, TEXT)
    fitted("SPLIT, RUN, JOIN", 288, ACCENT)
    ...
    # Four pieces at once versus one big piece -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "costs 85, 5, 5, 5",
         "best speedup 1.18 times", tag="ONE BIG PIECE")
    pill(d, 1070, 556, 700, 170, GREEN, "16 pieces, 4 workers",
         "4 running at the same moment", tag="TOGETHER")
    ...
    d.text((W // 2, 744), "Split until small.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Join the answers.",
