#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Thread Pool with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Thread Pool with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # An unbounded default versus a bounded pool -- both real settings from the project.
    pill(d, 150, 556, 700, 170, RED, "queue-capacity = 2147483647",
         "the default: no bound", tag="THE DEFAULT")
    pill(d, 1070, 556, 700, 170, GREEN, "queue-capacity = 3",
         "a refusal you can read", tag="BOUNDED")
    ...
    d.text((W // 2, 744), "Name the pool.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Bound the queue.",
