# Thread Pool with Spring Pattern — Project Specification
...
[`thread-pool-with-spring-pattern-explained.md`](thread-pool-with-spring-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.threadpoolspring`:
...
1. **The defaults are asserted.** `AsyncPoolTest` reads core 8, max and queue `Integer.MAX_VALUE`.
2. **The backlog is counted.** A thousand orders wait behind eight stuck workers, none refused.
3. **Every wait is a latch or a gate.** No test sleeps.
4. **The failures are real.** `TaskRejectedException`, a `this` call on the caller's thread, and starvation.
...
- `./gradlew build` passes. 8 test methods across `AsyncPoolTest`, `DemoRunsTest`.
...
| `thread-pool-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/thread-pool-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Thread Pool with Spring pattern in
...
| Runtime | ~5:34 over 16 scenes, 82 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Future/Promise with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py thread-pool-with-spring`. They are the one part of
...
integrated, -3.56 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py thread-pool-with-spring
python3 ../../docs/make_specs.py thread-pool-with-spring
```
