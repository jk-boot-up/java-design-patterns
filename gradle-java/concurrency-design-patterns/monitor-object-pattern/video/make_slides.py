#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Monitor Object video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Monitor Object  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("MONITOR", 152, TEXT)
    fitted("OBJECT", 288, ACCENT)
    ...
    # The caller's lock versus the object's own -- both real lines from the
    # project, so the poster shows the actual contrast, not a slogan.
    pill(d, 150, 556, 700, 170, RED, "stock.lock().lock();",
         "the caller must remember", tag="CALLER LOCKS")
    pill(d, 1070, 556, 700, 170, GREEN, "stock.sellOne();",
         "the object locks itself", tag="MONITOR")
    ...
    d.text((W // 2, 744), "A lock you can forget is not a lock.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Let the object own it.",
