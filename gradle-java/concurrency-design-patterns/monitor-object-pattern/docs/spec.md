# Monitor Object Pattern — Project Specification
...
[`monitor-object-pattern-explained.md`](monitor-object-pattern-explained.md); the problem it
...
- Not a tour of `java.util.concurrent`. `AtomicInteger` is named as the simpler answer for one counter and not built up.
...
1 production classes under `com.jk.explore.monitorobject`:
...
1. **The lost update is forced, not hoped for.** A two-party `Rendezvous` runs between the read and the write, so both threads have read before either writes.
2. **`if` instead of `while` is proven from the wait queue.** The demo waits until the condition reports two waiters before adding the item.
3. **The nested-monitor deadlock is detected by the JVM.** `ThreadMXBean.findDeadlockedThreads()` reports it, and interrupting both threads breaks it.
4. **Nothing sleeps to wait for a race.** No test calls `Thread.sleep`.
...
- `./gradlew build` passes. 13 test methods across `CallerLockedStockTest`, `DemoRunsTest`, `HarnessSelfTest`, `IfInsteadOfWhileTest`, `MonitorHazardsTest`, `PlainStockTest`, `StockMonitorTest`, `VolatileStockTest`.
...
| `monitor-object-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/monitor-object-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Monitor Object pattern in
...
| Runtime | ~6:30 over 14 scenes, 98 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Active Object.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py monitor-object`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py monitor-object
python3 ../../docs/make_specs.py monitor-object
```
