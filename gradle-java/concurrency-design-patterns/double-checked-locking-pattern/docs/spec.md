# Double-Checked Locking Pattern — Project Specification
...
[`double-checked-locking-pattern-explained.md`](double-checked-locking-pattern-explained.md); the problem it
...
7 production classes under `com.jk.explore.doublechecked`:
...
1. **The race is forced every time by a rendezvous,** and asserted over repeated runs.
2. **Locks are counted,** never timed.
3. **The field must be volatile,** asserted by reflection.
4. **The holder builds nothing until first use.**
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `DoubleCheckedTest`.
...
| `double-checked-locking-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/double-checked-locking-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Double-Checked Locking pattern in
...
| Runtime | ~5:07 over 14 scenes, 74 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Balking.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py double-checked-locking`. They are the one part of
...
integrated, -3.57 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py double-checked-locking
python3 ../../docs/make_specs.py double-checked-locking
```
