#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Double-Checked Locking video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Double-Checked Locking  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("DOUBLE-CHECKED", 152, TEXT)
    fitted("LOCKING", 288, ACCENT)
    ...
    # Check-then-create versus check, lock, check -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "check, then create",
         "2 price lists built", tag="TWO BUILT")
    pill(d, 1070, 556, 700, 170, GREEN, "check, lock, check",
         "1 built, 1 lock in 1000", tag="BUILT ONCE")
    ...
    d.text((W // 2, 744), "Take the lock only to build.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Prefer the holder.",
