#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Future/Promise with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Future/Promise with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("FUTURE PROMISE", 152, TEXT)
    ...
    # A future that carries the error versus a void that loses it -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "@Async void notify(sku)",
         "the exception goes nowhere", tag="VOID")
    pill(d, 1070, 556, 700, 170, GREEN, "@Async CompletableFuture<..>",
         "the exception reaches you", tag="FUTURE")
    ...
    d.text((W // 2, 744), "Return a future.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Never void.",
