# Future/Promise with Spring Pattern — Project Specification
...
[`future-promise-with-spring-pattern-explained.md`](future-promise-with-spring-pattern-explained.md); the problem it
...
7 production classes under `com.jk.explore.futurepromisespring`:
...
1. **Concurrency is counted, not inferred.** `AsyncFutureTest` gates three lookups and reads the peak in flight: 3, and 1 on a pool of one.
2. **Every wait is a latch, a gate or a bounded spin.** No test sleeps.
3. **The losses are proven.** A void exception, a null thread-local, a timeout and a cancel that leave the task running.
4. **The fixes are proven.** A registered handler, a `TaskDecorator`, and a fallback at the failing step.
5. **Dependencies are explained.** `docs/dependencies.md` says what to install, what it costs, and that skipping loses nothing.
...
- `./gradlew build` passes. 8 test methods across `AsyncFutureTest`, `DemoRunsTest`.
...
| `future-promise-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/future-promise-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Future/Promise with Spring pattern in
...
| Runtime | ~6:15 over 16 scenes, 91 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Active Object with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py future-promise-with-spring`. They are the one part of
...
integrated, -3.58 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py future-promise-with-spring
python3 ../../docs/make_specs.py future-promise-with-spring
```
