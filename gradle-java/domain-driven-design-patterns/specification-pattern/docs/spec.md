# Specification Pattern — Project Specification
...
[`specification-pattern-explained.md`](specification-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.specification`:
...
1. **The drift is real:** the three naive copies return different lists.
2. **And, or and not agree with their logic** for every product on the shelf, and De Morgan's law holds.
3. **The explanation names only the failing parts.**
4. **The in-memory cost is counted:** every product is examined.
...
- `./gradlew build` passes. 10 test methods across `DemoRunsTest`, `NaiveVersionTest`, `SpecificationTest`.
...
| `specification-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/specification-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Specification pattern in
...
| Runtime | ~5:01 over 14 scenes, 75 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Anti-Corruption Layer.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py specification`. They are the one part of
...
integrated, -3.55 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py specification
python3 ../../docs/make_specs.py specification
```
