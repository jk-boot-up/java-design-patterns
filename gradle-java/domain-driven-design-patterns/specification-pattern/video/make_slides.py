#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Specification video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Specification  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("SPECIFICATION", 152, TEXT)
    fitted("A RULE WITH A NAME", 288, ACCENT)
    ...
    # Three copies versus one named rule -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "price <= 1000 here, < 1000 there",
         "promotion: 4 products", tag="DRIFTED")
    pill(d, 1070, 556, 700, 170, GREEN, "cheapAndAvailable()",
         "all three agree: 2", tag="ONE RULE")
    ...
    d.text((W // 2, 744), "Write the rule once.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Combine it, explain it.",
