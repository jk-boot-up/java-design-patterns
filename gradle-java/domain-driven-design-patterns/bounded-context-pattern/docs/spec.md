# Bounded Context Pattern — Project Specification
...
[`bounded-context-pattern-explained.md`](bounded-context-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.boundedcontext`:
...
1. **Three correct answers to one question** are asserted, including the ninety-day boundary.
2. **No context imports another's types.** A test scans the source.
3. **A rename reaches Shipping only when delivered.**
4. **An event for an unknown customer is ignored.**
...
- `./gradlew build` passes. 7 test methods across `BoundedContextTest`, `DemoRunsTest`.
...
| `bounded-context-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/bounded-context-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Bounded Context pattern in
...
| Runtime | ~5:01 over 14 scenes, 71 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Transaction Script.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py bounded-context`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py bounded-context
python3 ../../docs/make_specs.py bounded-context
```
