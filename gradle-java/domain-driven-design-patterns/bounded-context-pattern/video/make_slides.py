#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Bounded Context video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Bounded Context  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("BOUNDED CONTEXT", 152, TEXT)
    fitted("ONE WORD, ONE MEANING", 288, ACCENT)
    ...
    # Three answers to one question -- real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "one Customer class",
         "12 fields, 3 departments", tag="ONE CLASS FOR ALL")
    pill(d, 1070, 556, 700, 170, GREEN, "Buyer, Recipient, Contact",
         "4 fields each", tag="ONE MODEL EACH")
    ...
    d.text((W // 2, 744), "Draw a boundary.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Inside it, one meaning.",
