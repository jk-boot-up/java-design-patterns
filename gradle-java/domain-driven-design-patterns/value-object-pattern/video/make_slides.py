#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Value Object video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Value Object  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("VALUE OBJECT", 152, TEXT)
    fitted("MONEY THAT CANNOT LIE", 288, ACCENT)
    ...
    # A bare number versus a value -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "10 GBP + 10 USD",
         "= 20.0", tag="MEANINGLESS")
    pill(d, 1070, 556, 700, 170, GREEN, "GBP 10.00 + USD 10.00",
         "refused", tag="REFUSED")
    ...
    d.text((W // 2, 744), "Defined by what it holds.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Impossible to build wrong.",
