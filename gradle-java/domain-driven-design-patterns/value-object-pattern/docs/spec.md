# Value Object Pattern — Project Specification
...
[`value-object-pattern-explained.md`](value-object-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.valueobject`:
...
1. **The floating point error is real output.** `NaiveVersionTest` asserts it.
2. **Allocation is exact for every amount from -50 to 200 pence into up to nine parts.**
3. **Shares differ by at most one penny.**
4. **The verdict is stated aloud.**
...
- `./gradlew build` passes. 17 test methods across `DemoRunsTest`, `EmailAddressTest`, `MoneyTest`, `NaiveVersionTest`.
...
| `value-object-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/value-object-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Value Object pattern in
...
| Runtime | ~5:14 over 14 scenes, 75 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Aggregate.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py value-object`. They are the one part of
...
integrated, -4.06 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py value-object
python3 ../../docs/make_specs.py value-object
```
