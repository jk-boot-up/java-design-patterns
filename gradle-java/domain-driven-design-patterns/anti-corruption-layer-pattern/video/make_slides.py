#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Anti-Corruption Layer video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Anti-Corruption Layer  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("ANTI-CORRUPTION", 152, TEXT)
    fitted("LAYER", 288, ACCENT)
    ...
    # Four guesses versus one decision -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "status H: four features guess",
         "basket sells a held item", tag="FOUR GUESSES")
    pill(d, 1070, 556, 700, 170, GREEN, "status H: the layer decides",
         "ON_HOLD, cannot be bought", tag="ONE DECISION")
    ...
    d.text((W // 2, 744), "Translate once, at the border.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Keep their model out of yours.",
