# Anti-Corruption Layer Pattern — Project Specification
...
[`anti-corruption-layer-pattern-explained.md`](anti-corruption-layer-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.acl`:
...
1. **Only the adapter imports the legacy package.** A test scans the source.
2. **Bad data is refused with the sku named.**
3. **A new status is one decision.** The shortcut's four guesses are asserted.
4. **The dropped fields are listed.**
...
- `./gradlew build` passes. 8 test methods across `AclTest`, `DemoRunsTest`.
...
| `anti-corruption-layer-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/anti-corruption-layer-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Anti-Corruption Layer pattern in
...
| Runtime | ~5:11 over 14 scenes, 77 subtitle cues |  |
...
> `python3 ../../docs/make_youtube_docs.py anti-corruption-layer`. They are the one part of
...
integrated, -3.96 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py anti-corruption-layer
python3 ../../docs/make_specs.py anti-corruption-layer
```
