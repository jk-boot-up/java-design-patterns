# Aggregate Pattern — Project Specification
...
[`aggregate-pattern-explained.md`](aggregate-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.aggregate`:
...
1. **Every rule is asserted,** including the merged-line and total limits.
2. **The line list is read-only and lines have no public constructor.**
3. **The second save of a stale load is refused.**
4. **A boundary drawn too wide is shown to conflict,** and one per order is shown not to.
...
- `./gradlew build` passes. 13 test methods across `ConsistencyBoundaryTest`, `DemoRunsTest`, `OrderAggregateTest`.
...
| `aggregate-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/aggregate-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Aggregate pattern in
...
| Runtime | ~5:05 over 14 scenes, 77 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Domain Event.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py aggregate`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py aggregate
python3 ../../docs/make_specs.py aggregate
```
