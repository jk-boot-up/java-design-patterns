#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Aggregate video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Aggregate  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("AGGREGATE", 152, TEXT)
    fitted("ONE DOOR, EVERY RULE", 288, ACCENT)
    ...
    # A loose list versus a root -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "order.lines.add(-3 mugs)",
         "accepted", tag="NO RULES")
    pill(d, 1070, 556, 700, 170, GREEN, "order.addLine(mug, 0)",
         "refused", tag="ONE DOOR")
    ...
    d.text((W // 2, 744), "One root, one way in.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Every rule in one place.",
