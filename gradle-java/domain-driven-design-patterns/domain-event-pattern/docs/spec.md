# Domain Event Pattern — Project Specification
...
[`domain-event-pattern-explained.md`](domain-event-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.domainevent`:
...
1. **The half-done state is real:** the naive order is saved and stock reserved when email fails.
2. **A failing handler does not stop the others and is retried alone,** and nothing is delivered twice.
3. **Events keep the order they were raised in.**
4. **A missed relay loses nothing.**
...
- `./gradlew build` passes. 11 test methods across `DemoRunsTest`, `DomainEventTest`, `RelayTest`.
...
| `domain-event-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/domain-event-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Domain Event pattern in
...
| Runtime | ~5:13 over 14 scenes, 79 subtitle cues |  |
...
> `python3 ../../docs/make_youtube_docs.py domain-event`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py domain-event
python3 ../../docs/make_specs.py domain-event
```
