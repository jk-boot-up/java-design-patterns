#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Domain Event video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Domain Event  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("DOMAIN EVENT", 152, TEXT)
    fitted("SAY WHAT HAPPENED", 288, ACCENT)
    ...
    # The order calls everyone versus the order says what happened -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "order calls stock, email, analytics",
         "email down: half done", tag="HALF DONE")
    pill(d, 1070, 556, 700, 170, GREEN, "order records OrderPlaced",
         "email retried alone", tag="RETRIED")
    ...
    d.text((W // 2, 744), "A fact, in the past tense.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Whoever cares reacts.",
