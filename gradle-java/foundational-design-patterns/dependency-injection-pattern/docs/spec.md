# Dependency Injection Pattern — Project Specification
...
[`dependency-injection-pattern-explained.md`](dependency-injection-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.dependencyinjection`:
...
1. **The wiring is by hand first, and its length is stated.** `DependencyInjectionTest` counts the lines between the markers.
2. **A container is built here.** `MiniContainer` builds the same graph, and fails at start on a missing bean and on a cycle.
3. **Field injection's cost is real.** A `new` object throws `NullPointerException` until reflection fills it.
4. **The progression is stated explicitly.** In the explainer and aloud in the video.
...
- `./gradlew build` passes. 10 test methods across `DemoRunsTest`, `DependencyInjectionTest`.
...
| `dependency-injection-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/dependency-injection-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Dependency Injection pattern in
...
| Runtime | ~6:47 over 14 scenes, 102 subtitle cues |  |
...
> `python3 ../../docs/make_youtube_docs.py dependency-injection`. They are the one part of
...
integrated, -3.84 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py dependency-injection
python3 ../../docs/make_specs.py dependency-injection
```
