#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Dependency Injection video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Dependency Injection  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("DEPENDENCY", 152, TEXT)
    fitted("INJECTION", 288, ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, "Registry.get(Gateway.class)",
         "the class asks", tag="STOP ASKING")
    pill(d, 1070, 556, 700, 170, GREEN, "new CheckoutService(p, g, n)",
         "the class is given", tag="BE GIVEN")
    ...
    d.text((W // 2, 744), "The signature is the",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "dependency list.",
