#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Service Locator with Consul video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Service Locator with Consul  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("SERVICE LOCATOR", 152, TEXT)
    fitted("WITH CONSUL", 288, ACCENT)
    ...
    # Asked for versus given -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "Discovery.find(notifier)",
         "every class asks, by string", tag="ASK CONSUL")
    pill(d, 1070, 556, 700, 170, GREEN, "http://nginx/charge",
         "one address, nothing to ask", tag="BE GIVEN")
    ...
    d.text((W // 2, 744), "Discovery is a legitimate locator.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Being given is better still.",
