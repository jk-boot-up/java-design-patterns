# Service Locator with Consul Pattern — Project Specification
...
[`service-locator-with-consul-pattern-explained.md`](service-locator-with-consul-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.locatorconsul`:
...
1. **Nothing is simulated.** The tests run a real Consul agent and real HTTP servers, and a real nginx container when Docker is available.
2. **Tests skip, not fail, without the infrastructure.** `consul` on the PATH, and Docker with the nginx image.
3. **The stale cache is real.** A cached address is still returned after the instance stopped, and the call fails.
4. **Nothing is left running.** The agent is stopped and the container removed when a run ends.
...
- `./gradlew build` passes. 8 test methods across `ConsulLocatorTest`, `DemoRunsTest`, `NginxServerSideDiscoveryTest`.
...
| `service-locator-with-consul-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/service-locator-with-consul-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Service Locator with Consul pattern in
...
| Runtime | ~6:11 over 14 scenes, 87 subtitle cues |  |
...
> `python3 ../../docs/make_youtube_docs.py service-locator-with-consul`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py service-locator-with-consul
python3 ../../docs/make_specs.py service-locator-with-consul
```
