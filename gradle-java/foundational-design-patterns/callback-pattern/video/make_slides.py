#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Callback video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Callback  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("CALL", 152, TEXT)
    fitted("BACK", 288, ACCENT)
    ...
    # Asking again and again versus being called -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "keep asking",
         "4 looks found nothing", tag="WAITING")
    pill(d, 1070, 556, 700, 170, GREEN, "hand over the code",
         "called when it is ready", tag="CALLED BACK")
    ...
    d.text((W // 2, 744), "Hand over what to do.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Go on with other work.",
