#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Dependency Injection with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Dependency Injection with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("DI WITH", 152, TEXT)
    fitted("SPRING", 288, ACCENT)
    ...
    # Wired by hand versus by annotation -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "new CheckoutService(p, g, n)",
         "nine lines, by hand", tag="BY HAND")
    pill(d, 1070, 556, 700, 170, GREEN, "@Component class CheckoutService",
         "the constructor says the rest", tag="SPRING")
    ...
    d.text((W // 2, 744), "Spring did not add the idea.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "It removed the typing.",
