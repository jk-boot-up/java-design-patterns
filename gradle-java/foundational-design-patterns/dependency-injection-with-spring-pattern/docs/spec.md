# Dependency Injection with Spring Pattern — Project Specification
...
[`dependency-injection-with-spring-pattern-explained.md`](dependency-injection-with-spring-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.diwithspring`:
...
1. **The partner's constructors are untouched.** `SpringDiTest` asserts the constructor's parameter types.
2. **Spring builds the same graph.** The same charge and three messages as the hand wiring.
3. **The failures are Spring's own.** `UnsatisfiedDependencyException` and `BeanCurrentlyInCreationException`.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `SpringDiTest`.
...
| `dependency-injection-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/dependency-injection-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Dependency Injection with Spring pattern in
...
| Runtime | ~5:04 over 15 scenes, 74 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Registry with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py dependency-injection-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py dependency-injection-with-spring
python3 ../../docs/make_specs.py dependency-injection-with-spring
```
