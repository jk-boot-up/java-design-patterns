# Execute Around Pattern — Project Specification
...
[`execute-around-pattern-explained.md`](execute-around-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.executearound`:
...
1. **The pool counts what is still open.**
2. **The around method closes on success and on failure.**
3. **A failed transaction leaves the balance as it was.**
4. **Time is a fake clock,** so every number is exact.
5. **A connection used after the block fails.**
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `ExecuteAroundTest`.
...
| `execute-around-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/execute-around-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Execute Around pattern in
...
| Runtime | ~4:43 over 14 scenes, 68 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Callback.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py execute-around`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py execute-around
python3 ../../docs/make_specs.py execute-around
```
