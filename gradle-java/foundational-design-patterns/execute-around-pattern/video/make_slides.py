#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Execute Around video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Execute Around  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("EXECUTE", 152, TEXT)
    fitted("AROUND", 288, ACCENT)
    ...
    # Closing by hand versus closing in one place -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "close it yourself",
         "after a failure: 1 still open", tag="LEAK")
    pill(d, 1070, 556, 700, 170, GREEN, "hand in the work",
         "after a failure: 0 still open", tag="NO LEAK")
    ...
    d.text((W // 2, 744), "Set up, run, clean up.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "You only write the middle.",
