# Registry with Spring Pattern — Project Specification
...
[`registry-with-spring-pattern-explained.md`](registry-with-spring-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.registryspring`:
...
1. **The cache leak is proven with real Spring.** `SharedContextLeakTest` has a second test that passes only after the first.
2. **The fix is proven.** `DirtiesContextFixTest` sees a clean gateway after `@DirtiesContext`.
3. **Each test class has its own context.** So the project's own tests do not depend on class order.
...
- `./gradlew build` passes. 11 test methods across `DemoRunsTest`, `DirtiesContextFixTest`, `RegistryBehaviourTest`, `SharedContextLeakTest`.
...
| `registry-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/registry-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Registry with Spring pattern in
...
| Runtime | ~5:26 over 14 scenes, 77 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Object Pool with HikariCP.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py registry-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py registry-with-spring
python3 ../../docs/make_specs.py registry-with-spring
```
