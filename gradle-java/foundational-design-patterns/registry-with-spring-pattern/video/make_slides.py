#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Registry with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Registry with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    pill(d, 150, 556, 700, 170, RED, "context.getBean(Gateway.class)",
         "the class asks", tag="GETBEAN")
    pill(d, 1070, 556, 700, 170, GREEN, "new InjectedCheckout(p, g, n)",
         "the class is given", tag="INJECTED")
    ...
    d.text((W // 2, 744), "The registry done well",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "is one you rarely call.",
