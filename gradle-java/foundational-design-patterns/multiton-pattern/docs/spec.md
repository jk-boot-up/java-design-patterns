# Multiton Pattern — Project Specification
...
[`multiton-pattern-explained.md`](multiton-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.multiton`:
...
2. **The same key always returns the same object.**
3. **Unknown keys are refused.**
4. **A forced race is deterministic,** by a barrier.
5. **Tests can reset the instances.**
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `MultitonTest`.
...
| `multiton-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/multiton-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Multiton pattern in
...
| Runtime | ~4:35 over 14 scenes, 63 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Type Object.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py multiton`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py multiton
python3 ../../docs/make_specs.py multiton
```
