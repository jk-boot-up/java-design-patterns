#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Multiton video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Multiton  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("MULTI-", 152, TEXT)
    fitted("TON", 288, ACCENT)
    ...
    # Two copies versus one per key -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "new one each time",
         "stock 90 and 100", tag="TWO BELIEFS")
    pill(d, 1070, 556, 700, 170, GREEN, "one per region",
         "stock 90 everywhere", tag="ONE TRUTH")
    ...
    d.text((W // 2, 744), "One instance for each key.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Always the same one.",
