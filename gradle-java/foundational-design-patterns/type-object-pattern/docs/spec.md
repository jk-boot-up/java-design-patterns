# Type Object Pattern — Project Specification
...
[`type-object-pattern-explained.md`](type-object-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.typeobject`:
...
1. **Money and tax are integers,** so results are exact.
2. **The subclass and the type object give the same total.**
3. **A new type adds no class.**
4. **A changed type changes every product of it.**
5. **A missing field is taken from the parent type.**
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `TypeObjectTest`.
...
| `type-object-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/type-object-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Type Object pattern in
...
| Runtime | ~4:56 over 14 scenes, 70 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Fluent Interface.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py type-object`. They are the one part of
...
integrated, -3.65 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py type-object
python3 ../../docs/make_specs.py type-object
```
