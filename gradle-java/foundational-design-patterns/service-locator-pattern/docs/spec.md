# Service Locator Pattern — Project Specification
...
[`service-locator-pattern-explained.md`](service-locator-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.servicelocator`:
...
1. **The compiler-silence failure is runnable.** `ServiceLocatorTest` builds a checkout with a missing notifier, charges the customer, and asserts the failure came after the charge.
2. **The advances are real.** Lazy singleton, prototype, and a test swap are each asserted.
3. **The legitimate case is real.** `ServiceLoader` finds two providers listed in `META-INF/services`.
4. **The verdict is stated aloud.** Prefer the alternative for business logic.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `ServiceLocatorTest`.
...
| `service-locator-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/service-locator-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Service Locator pattern in
...
| Runtime | ~5:23 over 13 scenes, 79 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Dependency Injection.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py service-locator`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py service-locator
python3 ../../docs/make_specs.py service-locator
```
