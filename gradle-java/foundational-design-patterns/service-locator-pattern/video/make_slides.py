#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Service Locator video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Service Locator  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    pill(d, 150, 556, 700, 170, RED, "locator.find(Notifier.class)",
         "a class that asks", tag="THE LOCATOR")
    pill(d, 1070, 556, 700, 170, GREEN, "new Checkout(policy, gateway, n)",
         "a class that is given", tag="THE ALTERNATIVE")
    ...
    d.text((W // 2, 744), "The class asks,",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "so nobody knows what it needs.",
