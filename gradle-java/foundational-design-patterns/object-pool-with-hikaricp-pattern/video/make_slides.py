#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Object Pool with HikariCP video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Object Pool with HikariCP  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("OBJECT POOL", 152, TEXT)
    fitted("WITH HIKARICP", 288, ACCENT)
    ...
    # Hand-built versus mature -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "class ConnectionPool // by hand",
         "a reset you must remember", tag="THE PARTNER")
    pill(d, 1070, 556, 700, 170, GREEN, "dataSource.getConnection()",
         "reset, timeout, sizing built in", tag="HIKARICP")
    ...
    d.text((W // 2, 744), "Use the library that fixed",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "the hard parts.",
