# Object Pool with HikariCP Pattern — Project Specification
...
[`object-pool-with-hikaricp-pattern-explained.md`](object-pool-with-hikaricp-pattern-explained.md); the problem it
...
2 production classes under `com.jk.explore.hikaripool`:
...
1. **No test asserts a timing.** `HikariPoolTest` asserts counts, states and exception messages.
2. **The reset is real.** autoCommit and readOnly return to their defaults for the next borrower.
3. **The remaining leak is real.** A session variable set by one borrower is read by the next.
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `HikariPoolTest`.
...
| `object-pool-with-hikaricp-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/object-pool-with-hikaricp-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Object Pool with HikariCP pattern in
...
| Runtime | ~5:24 over 14 scenes, 78 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Service Locator with Consul.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py object-pool-with-hikaricp`. They are the one part of
...
integrated, -4.01 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py object-pool-with-hikaricp
python3 ../../docs/make_specs.py object-pool-with-hikaricp
```
