# Object Pool Pattern — Project Specification
...
[`object-pool-pattern-explained.md`](object-pool-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.objectpool`:
...
1. **No test asserts a timing.** `ObjectPoolTest` asserts connection counts, dirty state and timeouts.
2. **The benchmark's method is written down.** Warm-up, median, and allocation measured with and without escape.
3. **The dirty return is real.** The next borrower reads the previous card holder.
4. **The verdict is stated aloud.** Pool what is expensive outside the JVM, and nothing else.
...
- `./gradlew build` passes. 10 test methods across `DemoRunsTest`, `ObjectPoolTest`.
...
| `object-pool-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/object-pool-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Object Pool pattern in
...
| Runtime | ~6:10 over 15 scenes, 93 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Registry.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py object-pool`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py object-pool
python3 ../../docs/make_specs.py object-pool
```
