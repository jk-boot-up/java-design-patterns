#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Object Pool video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Object Pool  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # Pooling something expensive versus something cheap -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "pool.borrow() // 6.4 ns",
         "a three-field object", tag="CHEAP OBJECT")
    pill(d, 1070, 556, 700, 170, GREEN, "pool.borrow() // saves 200ms",
         "a network connection", tag="EXPENSIVE")
    ...
    d.text((W // 2, 744), "Pool what is expensive",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "outside the JVM.",
