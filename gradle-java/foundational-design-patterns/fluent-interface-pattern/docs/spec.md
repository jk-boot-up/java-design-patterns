# Fluent Interface Pattern — Project Specification
...
[`fluent-interface-pattern-explained.md`](fluent-interface-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.fluent`:
...
1. **The fluent and positional calls give the same answer.**
2. **Swapped booleans are shown to compile and change the answer.**
3. **A never-changing query can be reused.**
4. **Guided steps offer only what may come next,** shown by reflection.
5. **A bad value is shown to be found late.**
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `FluentTest`.
...
| `fluent-interface-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/fluent-interface-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Fluent Interface pattern in
...
| Runtime | ~4:57 over 14 scenes, 70 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Execute Around.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py fluent-interface`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py fluent-interface
python3 ../../docs/make_specs.py fluent-interface
```
