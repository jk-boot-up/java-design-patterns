#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Fluent Interface video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Fluent Interface  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("FLUENT", 152, TEXT)
    fitted("INTERFACE", 288, ACCENT)
    ...
    # Five arguments versus a sentence -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "find(mugs, 2500, true, true, 10)",
         "which boolean is which?", tag="UNCLEAR")
    pill(d, 1070, 556, 700, 170, GREEN, "search().category(mugs).inStock()",
         "every part names itself", tag="CLEAR")
    ...
    d.text((W // 2, 744), "Chain the calls.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Read it like a sentence.",
