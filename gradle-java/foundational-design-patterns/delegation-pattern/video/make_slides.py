#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Delegation video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Delegation  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("DELEGA-", 152, TEXT)
    fitted("TION", 288, ACCENT)
    ...
    # A class per combination versus a helper -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "subclass for each way",
         "3 features: 8 classes", tag="8 CLASSES")
    pill(d, 1070, 556, 700, 170, GREEN, "hold a helper",
         "3 features: 0 new classes", tag="0 CLASSES")
    ...
    d.text((W // 2, 744), "Hand the job on.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Swap the helper.",
