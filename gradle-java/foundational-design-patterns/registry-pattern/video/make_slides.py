#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Registry video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Registry  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # Passed down versus asked for -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "new Charger(gateway) x 6",
         "five classes only forward it", tag="PASS IT DOWN")
    pill(d, 1070, 556, 700, 170, GREEN, "Registry.get(Gateway.class)",
         "a global, with better manners", tag="REGISTRY")
    ...
    d.text((W // 2, 744), "The friction goes away.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "So does the honesty.",
