# Registry Pattern — Project Specification
...
[`registry-pattern-explained.md`](registry-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.registry`:
...
1. **The naive version is treated fairly.** The friction is shown and said to be real, and honest.
2. **Order dependence is runnable.** `RegistryTest` runs the same two tests in both orders and asserts different outcomes.
3. **Invisible dependencies are proven.** `new RegistryCheckout()` has a zero-argument constructor and fails on first use.
4. **The verdict is stated aloud.** Narrowly, and never for what tests must replace.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `RegistryTest`.
...
| `registry-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/registry-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Registry pattern in
...
| Runtime | ~5:38 over 14 scenes, 83 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Service Locator.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py registry`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py registry
python3 ../../docs/make_specs.py registry
```
