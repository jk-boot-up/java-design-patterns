# Null Object Pattern — Project Specification
...
[`null-object-pattern-explained.md`](null-object-pattern-explained.md); the problem it
...
1 production classes under `com.jk.explore.nullobject`:
...
1. **The forgotten check is a real `NullPointerException`.** `NullObjectTest` asserts it.
2. **The behaviour is identical.** The naive and null-object checkouts give the same total for every customer.
3. **The bill is demonstrated.** A forgiving directory charges 10000 pence instead of 9000 on an outage.
4. **The verdict is stated aloud.** The video and the explainer end with it.
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `NullObjectTest`.
...
| `null-object-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/null-object-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Null Object pattern in
...
| Runtime | ~5:44 over 14 scenes, 86 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Object Pool.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py null-object`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py null-object
python3 ../../docs/make_specs.py null-object
```
