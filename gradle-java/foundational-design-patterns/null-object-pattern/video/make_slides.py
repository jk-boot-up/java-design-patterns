#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Null Object video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Null Object  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # A null check versus a discount that does nothing -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "if (discount != null)",
         "seven times, and one forgotten", tag="NULL CHECKS")
    pill(d, 1070, 556, 700, 170, GREEN, "NoDiscount.apply(price)",
         "the price, unchanged", tag="NULL OBJECT")
    ...
    d.text((W // 2, 744), "Absence can be a normal state.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "It must never hide a failure.",
