# Prototype with Spring Pattern — Project Specification
...
[`prototype-with-spring-pattern-explained.md`](prototype-with-spring-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.prototypespring`:
...
1. **Identity is asserted with assertSame/assertNotSame.**
2. **Destruction is counted,** never timed.
3. **Nothing depends on timing.**
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `PrototypeBeanTest`.
...
| `prototype-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/prototype-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Prototype with Spring pattern in
...
| Runtime | ~4:30 over 16 scenes, 66 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Proxy with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py prototype-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py prototype-with-spring
python3 ../../docs/make_specs.py prototype-with-spring
```
