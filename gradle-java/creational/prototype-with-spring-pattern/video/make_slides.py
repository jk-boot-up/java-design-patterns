#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Prototype with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Prototype with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # Injected once versus asked each time -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "Listing injected into a singleton",
         "the same draft, forever", tag="SHARED")
    pill(d, 1070, 556, 700, 170, GREEN, "ObjectProvider getObject",
         "a fresh listing each call", tag="FRESH")
    ...
    d.text((W // 2, 744), "A new bean each time.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "But not a copy.",
