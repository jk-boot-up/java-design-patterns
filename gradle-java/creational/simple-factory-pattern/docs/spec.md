# Simple Factory Pattern — Project Specification
...
[`simple-factory-pattern-explained.md`](simple-factory-pattern-explained.md); the problem it
...
- Not a payment integration. `CreditCardPayment` prints; it does not authorise anything.
- Not a Gang of Four pattern. Simple Factory is not in the book — it is the everyday idiom that Factory Method and Abstract Factory both grow out of, which is why it comes first in the learning order.
- Not an argument against dependency injection. A DI container solves an overlapping problem, and the written notes say so.
...
11 production classes under `com.jk.explore.simplefactory`:
...
1. **The factory is the only place a concrete method is named.** Grepping for `new CreditCardPayment()` must find exactly one call site.
2. **The client never branches on the payment type.** If `CheckoutService` contains a `switch` over `PaymentType`, the decision has been duplicated, not centralised.
3. **An unknown type fails loudly**, with a message naming the value that was not recognised. Silently returning a default is how a customer pays by a method they did not choose.
...
- `./gradlew build` passes. 9 test methods across `CheckoutServiceTest`, `PaymentMethodFactoryTest`.
...
| `simple-factory-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/simple-factory-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Simple Factory pattern in
...
| Runtime | ~8:22 over 15 scenes, 121 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Simple Factory Pattern in Java - Payment Methods"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Static Factory.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py simple-factory`. They are the one part of
...
integrated, -3.70 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py simple-factory
python3 ../../docs/make_specs.py simple-factory
```
