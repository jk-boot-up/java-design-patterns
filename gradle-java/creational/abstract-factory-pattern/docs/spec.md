# Abstract Factory Pattern — Project Specification
...
[`abstract-factory-pattern-explained.md`](abstract-factory-pattern-explained.md); the problem it
...
that document is the authority. This project is the reference implementation of that standard — the other thirteen projects' build scripts are generated from this one — so a change here is a change to all fourteen and must be made there first.
...
- Not a production tax or currency library. `UkVatCalculator` hard-codes 20%; a real system reads rates from a table that changes by legislation.
- Not a survey of alternatives. Dependency injection containers, service locators and enum-based strategies all solve overlapping problems, and the written notes say so, but the project demonstrates one pattern.
- Not a reference for internationalisation. `RupeeFormatter` formats a number; it is not `java.text.NumberFormat` and does not pretend to be.
...
20 production classes under `com.jk.explore.abstractfactory`:
...
1. **The client takes a factory, never a market string.** `CheckoutService` must have no knowledge of which markets exist. If it can name "UK", the pattern has not been applied — it has only been moved.
2. **Concrete product types are never named outside their factory.** Grepping for `new UkVatCalculator()` must find exactly one call site.
3. **A factory returns one family or none.** There is no path through the code that yields a UK tax rule beside a dollar formatter.
...
- `./gradlew build` passes. 12 test methods across `CheckoutServiceTest`, `MarketFactoryTest`.
...
| `abstract-factory-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/abstract-factory-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Abstract Factory pattern in
...
| Runtime | ~10:19 over 16 scenes, 151 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Builder.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py abstract-factory`. They are the one part of
...
integrated, -3.85 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py abstract-factory
python3 ../../docs/make_specs.py abstract-factory
```
