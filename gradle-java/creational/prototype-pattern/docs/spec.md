# Prototype Pattern — Project Specification
...
[`prototype-pattern-explained.md`](prototype-pattern-explained.md); the problem it
...
- Not an endorsement of `Cloneable` and `Object.clone()`. The project implements its own `Prototype` interface, and the written notes explain why the built-in mechanism is best avoided.
- Not a serialization-based deep-copy utility. Those exist and have their place; they hide the decision this project is about.
...
6 production classes under `com.jk.explore.prototype`:
...
1. **Mutating a copy never affects its original.** This is the property the tests exist to defend; a shallow copy of the attribute map breaks it silently.
2. **Sharing is deliberate, not accidental.** Immutable values may be shared by reference, and the code says why at each point rather than deep-copying everything out of caution.
3. **The registry hands out copies, never its masters.** A caller that mutated a registry entry would corrupt every later copy.
...
- `./gradlew build` passes. 9 test methods across `ListingRegistryTest`, `ProductListingDemoTest`, `ProductListingTest`.
...
| `prototype-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/prototype-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Prototype pattern in
...
| Runtime | ~9:48 over 20 scenes, 140 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Prototype Pattern in Java - Cloning Product Listings"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Singleton.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py prototype`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py prototype
python3 ../../docs/make_specs.py prototype
```
