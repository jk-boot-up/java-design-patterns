#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Prototype Pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Prototype Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("It is also"):
...
def kind_code(scene, img, d):
    ...
    for ln in lines:
        ...
        if s.startswith("//") or s.startswith("..."):
            col = (100, 180, 130)
        elif s.startswith("—") or s.startswith("Effective Java"):
            col = (100, 180, 130)
            ...
            col = PRODUCT
            fnt = f(MONO_B, size)
        elif s.startswith(("ProductListing", "Prototype", "ListingRegistry", "ShippingProfile")):
            col = ACCENT
...
def kind_console(scene, img, d):
    ...
    for ln in lines:
        ...
        if ln.startswith("$"):
            ...
            col = RED
        elif ln.startswith(("Master:", "White variant:")):
            col = PINK
        elif "true" in ln:
            col = GREEN
        elif ln.startswith(("master.images()", "shippingProfile", "registry copies")):
            col = ACCENT
...
def kind_diagram(scene, img, d):
    """master.copy() -> deep-copy-or-share, field by field -> independent clone."""
    draw_title(d, scene["title"])

    box(d, 700, 226, 520, 100, "ProductListingDemo", "the caller", CLIENT)
    arrow(d, 960, 326, 960, 392, LINE)
    d.text((790, 340), "master.copy()", font=f(MONO, 24), fill=MUTED)

    # the one method
    d.rounded_rectangle([120, 392, W - 120, 540], radius=18, outline=ACCENT, width=3)
    d.text((150, 410), "ONE METHOD, NO BAGGAGE  —  Prototype<T> DEFINES ONLY copy()",
           font=f(SANS_B, 22), fill=ACCENT)

    box(d, 660, 444, 600, 70, "ProductListing implements Prototype<ProductListing>",
        "", ACCENT, label_size=22)

    arrow(d, 960, 540, 960, 606, LINE)

    # the field-by-field decision
    d.rounded_rectangle([120, 606, W - 120, 800], radius=18, outline=PINK, width=3)
    d.text((W // 2, 616), "FIELD BY FIELD  —  DEEP COPY WHAT'S MUTABLE, SHARE WHAT'S IMMUTABLE",
           font=f(SANS_B, 22), fill=PINK, anchor="ma")

    box(d, 220, 664, 460, 110, "images (List)", "new ArrayList<>(...) — deep copy", PINK, label_size=26)
    box(d, 730, 664, 460, 110, "attributes (Map)", "new LinkedHashMap<>(...) — deep copy", PINK, label_size=26)
    box(d, 1240, 664, 460, 110, "shippingProfile", "passed through — shared reference", GREEN, label_size=26)

    d.text((W // 2, 830), "constructor runs once, from copy() — the same logic protects every caller",
           font=f(SANS_B, 26), fill=TEXT, anchor="ma")
    arrow(d, 960, 862, 960, 902, LINE)

    box(d, 760, 902, 400, 100, "whiteVariant", "independent ProductListing", PRODUCT)

    d.text((W // 2, 1024),
           "mutating whiteVariant's images or attributes never reaches master",
           font=f(SANS_B, 30), fill=MUTED, anchor="ma")
...
def kind_poster(scene, img, d):
    ...
    d.rectangle([0, 0, W, 14], fill=PRODUCT)
    d.text((110, 88), "DESIGN PATTERNS  -  JAVA",
           font=f(SANS_B, 36), fill=PRODUCT)

    d.text((110, 156), "PROTOTYPE", font=f(SANS_B, 128), fill=TEXT)
    ...
    pill(d, 150, 556, 700, 170, RED, "new ProductListing(11 args)",
         "re-typed, every variant", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "master.copy()",
         "then tweak what differs", tag="AFTER")
    ...
    d.text((W // 2, 758), "Copy the one you already have",
