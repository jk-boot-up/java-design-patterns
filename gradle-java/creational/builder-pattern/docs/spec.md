# Builder Pattern — Project Specification
...
[`builder-pattern-explained.md`](builder-pattern-explained.md); the problem it
...
- Not Lombok's `@Builder`. Writing the builder by hand is the point; generating it hides exactly the decisions being taught.
- Not a validation framework. `build()` checks what it must; Bean Validation is a different tool for a different job.
- Not an argument that everything needs a builder. Two or three parameters do not.
...
6 production classes under `com.jk.explore.builder`:
...
1. **`PurchaseOrder` has no public constructor and no setters.** The builder is the only way to make one, and a built order cannot be changed.
2. **`build()` validates.** A missing customer or an empty item list is rejected there, so an invalid `PurchaseOrder` cannot exist.
3. **Collections are copied on the way in and unmodifiable on the way out**, so a caller holding the original list cannot mutate a built order.
...
- `./gradlew build` passes. 13 test methods across `PurchaseOrderDemoTest`, `PurchaseOrderPresetsTest`, `PurchaseOrderTest`.
...
| `builder-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/builder-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Builder pattern in
...
| Runtime | ~10:50 over 19 scenes, 165 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Builder Pattern in Java - Building a Purchase Order"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Prototype.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py builder`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py builder
python3 ../../docs/make_specs.py builder
```
