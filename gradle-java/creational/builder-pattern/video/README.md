# Builder Pattern — Teaching Video

A narrated, slide-based video that teaches the builder pattern using this
project's code. Aimed at beginners with no prior design-pattern knowledge,
though it assumes the static factory method session ([`../../static-factory-pattern`](../../static-factory-pattern))
or an equivalent has already landed.

## Output Files

| File | What it is |
| --- | --- |
| `builder-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `builder-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `builder-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Job | A purchase order, two required facts, five optional pieces |
| 3 | The Constructor With Nine Parameters | The unreadable call site, and the swappable booleans |
| 4 | Telescoping Constructors | Effective Java's own name for the next-worst idea |
| 5 | Why That Hurts | Five separate costs, none of them the type's fault |
| 6 | The Builder Pattern | The GoF definition *and* Effective Java Item 2 — one technique |
| 7 | A Made-to-Order Sandwich Counter | The analogy: build it up, check it only when you're done |
| 8 | The Shape of It | One door in, a chain of calls, one build() at the end |
| 9 | Required Facts, Optional Pieces | The Builder's own constructor versus its chainable methods |
| 10 | One Method, One Piece | `return this`, and why the chain reads the way it does |
| 11 | A Rule That Lives in One Place | `giftMessage` implying `giftWrapped` |
| 12 | Checked Only When You Say You're Done | Why `build()` is the only method that can validate |
| 13 | The Product Stops Watching the Builder | `List.copyOf` and the reused-builder proof |
| 14 | The Director, the Java Way | `PurchaseOrderPresets`, honestly compared to the GoF role |
| 15 | Running It | The real program output, including both rejected orders |
| 16 | Where It Stops | The extra object, the extra typing, and when not to bother |
| 17 | How It Relates to the Others | Builder next to static factory, simple factory, abstract factory |
...
**Upload `builder-pattern-explained.mp4`, with `poster.png` as the thumbnail
and the `.srt` as the captions.** The `.m4a` and the build scripts have no
role on YouTube.

### Adding the subtitles

YouTube auto-generates captions, but they mis-hear class names like
`PurchaseOrderPresets`. Uploading `builder-pattern-explained.srt` gives
...
> Learn the builder pattern in Java 21 — a Gang of Four pattern *and* item
> two of Effective Java, described from two angles. We start from a
> nine-parameter constructor that no one can read at the call site, watch the
> telescoping-constructor workaround make it worse, and rebuild the same
> purchase order as a chain of named calls that only gets checked for
> completeness when build() is called. We cover the Director role, honestly
> compared to how idiomatic Java actually writes it, and show builder
> composing with the static factory method rather than competing with it. No
