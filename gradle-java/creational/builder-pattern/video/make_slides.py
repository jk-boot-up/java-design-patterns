#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Builder Pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Builder Pattern  ·  Java 21",
...
def kind_code(scene, img, d):
    ...
    for ln in lines:
        ...
        if s.startswith("//") or s.startswith("..."):
            ...
            col = PRODUCT
            fnt = f(MONO_B, size)
        elif s.startswith(("PurchaseOrder", "Builder", "Address", "Money")):
            col = ACCENT
...
def kind_console(scene, img, d):
    ...
    for ln in lines:
        ...
        if ln.startswith("$"):
            ...
            col = RED
        elif ln.startswith("Hand-built"):
            col = PINK
        elif "->" in ln:
            col = ACCENT
        elif "items:" in ln:
            col = GREEN
        elif ln.startswith(("gift order", "standard order", "express order")):
            col = TEXT
...
def kind_diagram(scene, img, d):
    """The one door in: builder() -> chained calls -> build() -> product."""
    draw_title(d, scene["title"])

    box(d, 700, 226, 520, 100, "PurchaseOrderDemo", "the caller", CLIENT)
    arrow(d, 960, 326, 960, 392, LINE)
    d.text((760, 340), 'PurchaseOrder.builder("ORD-9001", "CUST-100")',
           font=f(MONO, 22), fill=MUTED)

    # the one door in
    d.rounded_rectangle([120, 392, W - 120, 560], radius=18, outline=ACCENT, width=3)
    d.text((150, 410), "ONE DOOR IN  —  PurchaseOrder's CONSTRUCTOR IS PRIVATE",
           font=f(SANS_B, 22), fill=ACCENT)

    box(d, 660, 444, 600, 90, "PurchaseOrder.Builder",
        "static nested  ·  every method returns this", ACCENT)

    arrow(d, 960, 560, 960, 626, LINE)

    # the chained calls, accumulating pieces
    d.rounded_rectangle([120, 626, W - 120, 796], radius=18, outline=PINK, width=3)
    d.text((W // 2, 636), "CHAINED, IN ANY ORDER  —  EACH RETURNS THE SAME BUILDER",
           font=f(SANS_B, 22), fill=PINK, anchor="ma")

    calls = ["addItem(mug)", "addItem(book)", "shippingAddress(home)",
             "giftMessage(...)", "couponCode(...)"]
    centres = [280, 620, 960, 1300, 1640]
    for name, cx in zip(calls, centres):
        box(d, cx - 160, 674, 320, 100, name, "", PINK, label_size=22)

    d.text((W // 2, 826), "build()  —  checks completeness, then constructs",
           font=f(SANS_B, 32), fill=TEXT, anchor="ma")
    arrow(d, 960, 862, 960, 902, LINE)

    box(d, 760, 902, 400, 100, "PurchaseOrder", "immutable  ·  private constructor", PRODUCT)

    d.text((W // 2, 1024),
           "the builder cannot be reached again once build() returns",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "BUILDER", font=f(SANS_B, 128), fill=TEXT)
    ...
    pill(d, 150, 556, 700, 170, RED, "new PurchaseOrder(9 args)",
         "unreadable, unordered", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, ".addItem(x).shippingAddress(a)",
         "one piece at a time", tag="AFTER")
    ...
    d.text((W // 2, 758), "Decide it a piece at a time",
