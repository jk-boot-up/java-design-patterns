# Factory Method Pattern — Project Specification
...
[`factory-method-pattern-explained.md`](factory-method-pattern-explained.md); the problem it
...
- Not a claim that inheritance is the right tool here in production — a constructor parameter often is. The written notes cover when the subclass hook earns its keep and when it does not.
- Not Template Method, though the two are close relatives and the explained document draws the line between them.
...
14 production classes under `com.jk.explore.factorymethod`:
...
1. **The base class never names a concrete courier.** It calls its own factory method and nothing else.
2. **Subclasses override the factory method and nothing else.** A subclass that also overrides the workflow has defeated the pattern.
3. **The workflow is identical for all four tiers**, demonstrably: the printed output differs only in the carrier and the timings.
...
- `./gradlew build` passes. 11 test methods across `CourierTest`, `DeliveryServiceTest`.
...
| `factory-method-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/factory-method-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Factory Method pattern in
...
| Runtime | ~8:25 over 15 scenes, 122 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Factory Method Pattern in Java - Delivery Tiers"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Abstract Factory.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py factory-method`. They are the one part of
...
integrated, -3.82 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py factory-method
python3 ../../docs/make_specs.py factory-method
```
