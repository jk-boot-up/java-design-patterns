#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Singleton with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Singleton with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # A shared bean versus a second container -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "two contexts, two counters",
         "ORD-000001 twice", tag="DUPLICATE")
    pill(d, 1070, 556, 700, 170, GREEN, "one container, one bean",
         "ORD-000001, 2, 3", tag="SHARED")
    ...
    d.text((W // 2, 744), "One instance per container.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Not one per program.",
