# Singleton with Spring Pattern — Project Specification
...
[`singleton-with-spring-pattern-explained.md`](singleton-with-spring-pattern-explained.md); the problem it
...
7 production classes under `com.jk.explore.singletonspring`:
...
1. **Sharing is asserted by identity,** not by matching numbers.
2. **The duplicate is forced** with a gate between read and write.
3. **Construction is counted** with a static counter, never timed.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `SingletonBeanTest`.
...
| `singleton-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/singleton-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Singleton with Spring pattern in
...
| Runtime | ~5:12 over 16 scenes, 74 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Prototype with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py singleton-with-spring`. They are the one part of
...
integrated, -3.66 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py singleton-with-spring
python3 ../../docs/make_specs.py singleton-with-spring
```
