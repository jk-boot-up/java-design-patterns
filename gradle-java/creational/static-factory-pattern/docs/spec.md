# Static Factory Pattern — Project Specification
...
[`static-factory-pattern-explained.md`](static-factory-pattern-explained.md); the problem it
...
- Not a Gang of Four pattern. This is the Effective Java idiom, included because it is the cheapest possible introduction to the idea that construction is a decision worth naming.
- Not a money library. `Money` exists so the arithmetic is readable; use `BigDecimal` and a real currency type in production.
- Not a promotions engine. Real discount stacking has rules about precedence and exclusivity that this deliberately does not model.
...
11 production classes under `com.jk.explore.staticfactory`:
...
1. **Constructors are not public.** If callers can still write `new PercentageDiscount(...)`, the named methods are decoration rather than the way in.
2. **Every factory method names its intent**, not its parameters. `Discount.percentage(10)` reads at the call site; `Discount.of(10, true)` does not.
3. **Factory methods return the abstraction**, so which implementation comes back stays a private decision — that freedom is the whole point of the idiom.
...
- `./gradlew build` passes. 23 test methods across `CheckoutServiceTest`, `DiscountTest`, `MoneyTest`.
...
| `static-factory-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/static-factory-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Static Factory pattern in
...
| Runtime | ~12:19 over 19 scenes, 178 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Static Factory Methods in Java - Named Constructors"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Factory Method.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py static-factory`. They are the one part of
...
integrated, -3.62 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py static-factory
python3 ../../docs/make_specs.py static-factory
```
