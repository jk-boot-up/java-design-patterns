...
| File | Format | Purpose |
| --- | --- | --- |
| `singleton-pattern-explained.mp4` | 1920×1080, H.264 High, 30 fps, yuv420p, AAC-LC 192 kbps stereo @ 48 kHz, `+faststart` | The video. Uploaded to YouTube as-is. |
| `singleton-pattern-explained.m4a` | AAC 192 kbps stereo | Audio-only, for revision on the move. |
| `singleton-pattern-explained.srt` | SubRip | Captions. Uploaded alongside the video. |
...
`Singleton Pattern  ·  Java 21`, and the identifiers the syntax-colouring
rules in `make_slides.py` look for (`OrderSequenceGenerator`,
`LegacyOrderSequenceGenerator`, `INSTANCE`, `AtomicLong` in place of
...
of the original spec apply, with `singleton-pattern-explained.mp4` /
...
| # | Kind | Scene |
| --- | --- | --- |
| 1 | poster | Singleton Pattern |
| 2 | bullets | The Job |
| 3 | code | An Instance Per Caller |
| 4 | code | The Classic Fix |
| 5 | code | Breaking It: Reflection |
| 6 | code | Breaking It: Serialization |
| 7 | quote | The Singleton Pattern |
| 8 | diagram | The Shape of It |
| 9 | code | One Constant, Three Guarantees |
| 10 | code | Why `AtomicLong` |
| 11 | console | Running It |
| 12 | bullets | Where It Stops |
| 13 | bullets | How It Relates to the Others |
| 14 | title | One Sentence to Keep |
| 15 | outro | Thanks for Watching |
...
→ say where it stops → one sentence to keep.** This project's version compresses
the analogy into the definition scene and spends the recovered time on the two
attacks, since watching a "private" constructor actually get called is the
scene most worth the extra minute.
