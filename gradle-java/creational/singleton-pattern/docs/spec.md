# Singleton Pattern — Project Specification
...
[`singleton-pattern-explained.md`](singleton-pattern-explained.md); the problem it
...
- Not an endorsement of singletons as a default. The pattern makes dependencies invisible and tests order-dependent; the written notes are explicit that a single instance passed by dependency injection is usually better.
- Not a distributed ID generator. One JVM, one counter. Real order numbers across several instances need a database sequence or a snowflake ID.
- Not a lazy-initialisation tutorial. Double-checked locking is discussed and then not used, because the enum does the job.
...
3 production classes under `com.jk.explore.singleton`:
...
1. **The singleton is an enum.** That is what makes a second instance impossible: reflection cannot invoke an enum constructor, and deserialization returns the existing constant rather than a copy.
2. **The counter is thread-safe**, and a test proves it by issuing numbers concurrently and asserting the set of results has no duplicates.
3. **The broken version stays in the project.** `LegacyOrderSequenceGenerator` exists to be demonstrated failing; deleting it would remove the reason the pattern is there.
...
- `./gradlew build` passes. 10 test methods across `LegacyOrderSequenceGeneratorTest`, `OrderSequenceGeneratorDemoTest`, `OrderSequenceGeneratorTest`.
...
| `singleton-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/singleton-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Singleton pattern in
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Singleton Pattern in Java - Order Number Sequencer"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Adapter.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py singleton`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py singleton
python3 ../../docs/make_specs.py singleton
```
