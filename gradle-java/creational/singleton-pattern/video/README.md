# Singleton Pattern — Teaching Video

A narrated, slide-based video that teaches the singleton pattern using this
project's code. Aimed at beginners with no prior design-pattern knowledge,
though having done the prototype-pattern session
([`../../prototype-pattern`](../../prototype-pattern)) first gives useful
contrast.
...
| File | What it is |
| --- | --- |
| `singleton-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `singleton-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `singleton-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Job | An order-number sequencer three services all have to share |
| 3 | An Instance Per Caller | The plain class, constructed twice, colliding on the same numbers |
| 4 | The Classic Fix | Private constructor, static field, `getInstance()` |
| 5 | Breaking It: Reflection | `setAccessible(true)` calls the "private" constructor anyway |
| 6 | Breaking It: Serialization | A round trip rebuilds a second instance, no reflection needed |
| 7 | The Singleton Pattern | The GoF definition, in plain words |
| 8 | The Shape of It | One enum constant, three JVM guarantees, contrasted with the legacy shape's two holes |
| 9 | One Constant, Three Guarantees | `OrderSequenceGenerator`, and why it needs none of the classic scaffolding |
| 10 | Why `AtomicLong` | Concurrency, once there really is only one instance reachable from every thread |
| 11 | Running It | The real program output, enum and legacy side by side |
| 12 | Where It Stops | Global mutable state, hidden dependencies, and when the guarantee is the wrong one |
| 13 | How It Relates to the Others | Singleton next to prototype, builder, and abstract factory |
| 14 | One Sentence to Keep | The takeaway |
...
the human-readable copy used for review; regenerate it with:

```bash
python3 - <<'PY'
import re
from scenes import SCENES
def spoken(t): return re.sub(r"\s+", " ", re.sub(r"\[\[[^\]]*\]\]", " ", t)).strip()
lines = ["# Narration Script — Singleton Pattern", "",
         "Generated from `scenes.py`. Do not hand-edit; edit the `narration` "
         "field of a scene instead and regenerate.", ""]
for i, s in enumerate(SCENES, start=1):
    lines += [f"## {i}. {s['title']}", "", spoken(s["narration"]), ""]
open("narration.md", "w").write("\n".join(lines))
PY
```

Note that currency and symbols are written out in words — "ten pounds"
rather than "£10" — and every class name is spoken as a person would say it
("order sequence generator", not "OrderSequenceGenerator"). Keep that habit,
or the narration will mispronounce them.
...
**Upload `singleton-pattern-explained.mp4`, with `poster.png` as the
...
`OrderSequenceGenerator`. Uploading `singleton-pattern-explained.srt` gives
...
> Learn the singleton pattern in Java 21 — the Gang of Four pattern for
> guaranteeing exactly one instance of a class exists. We start from an
> order-number sequencer built twice by mistake, fix it the textbook way
> with a private constructor and a static `getInstance()`, and then break
> that "fix" twice — once with reflection's `setAccessible(true)`, once with
> a plain serialization round trip. We finish with the shape Effective Java
