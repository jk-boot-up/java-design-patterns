#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Circuit Breaker with Resilience4j video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Circuit Breaker with Resilience4j  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("CIRCUIT BREAKER", 152, TEXT)
    fitted("WITH RESILIENCE4J", 288, ACCENT)
    ...
    # One annotation versus a call on this -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "this.fetch(sku)",
         "10 errors, breaker closed", tag="BYPASSED")
    pill(d, 1070, 556, 700, 170, GREEN, "@CircuitBreaker",
         "opens after failures", tag="PROTECTED")
    ...
    d.text((W // 2, 744), "An annotation and a few numbers.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Still only a proxy.",
