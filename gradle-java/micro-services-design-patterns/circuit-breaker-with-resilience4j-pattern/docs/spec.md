# Circuit Breaker with Resilience4j Pattern — Project Specification
...
[`circuit-breaker-with-resilience4j-pattern-explained.md`](circuit-breaker-with-resilience4j-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.circuitbreakerr4j`:
...
1. **Calls that reach the backend are counted,** never timed.
2. **The half-open probe is started by the test,** not by a wait duration.
3. **The bypass is asserted,** so a fix fails the test.
...
- `./gradlew build` passes. 7 test methods across `BreakerTest`, `DemoRunsTest`.
...
| `circuit-breaker-with-resilience4j-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/circuit-breaker-with-resilience4j-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Circuit Breaker with Resilience4j pattern in
...
| Runtime | ~5:13 over 16 scenes, 74 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Retry with Resilience4j.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py circuit-breaker-with-resilience4j`. They are the one part of
...
integrated, -3.73 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py circuit-breaker-with-resilience4j
python3 ../../docs/make_specs.py circuit-breaker-with-resilience4j
```
