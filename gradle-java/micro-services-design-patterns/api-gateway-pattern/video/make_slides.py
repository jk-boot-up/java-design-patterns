#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the API Gateway video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "API Gateway  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # Three bands, and the layout carries the lesson: one crossing of the slow
    # network at the top, one gateway in the middle, four cheap calls at the
    # bottom. The only wide gap in the picture is the expensive link.
    box(d, 70, 232, 560, 130, "MobileApp",
        "«client»   knows one address, makes one call", CLIENT,
        label_size=38, sub_size=24)

    arrow(d, 200, 366, 200, 448, ROSE, 5)
    d.text((250, 392), "the slow link  ·  ~200ms a crossing",
           font=f(SANS_B, 24), fill=ROSE, anchor="la")

    box(d, 70, 452, 560, 140, "ProductPageGateway",
        "«gateway»  checks the token once, joins four answers", AMBER,
        label_size=36, sub_size=24)

    arrow(d, 200, 596, 200, 636, GREEN, 4)
    box(d, 70, 640, 560, 110, "AuthService",
        "«token check»  asked once per request, at the edge", GREEN,
        label_size=32, sub_size=22)

    d.text((1465, 400), "the fast link  ·  ~10ms a call",
           font=f(SANS_B, 24), fill=GREEN, anchor="ma")

    # The four owners of the page's four facts. Recommendations is drawn in its
    # own colour because the whole pattern turns on it being the optional one.
    services = [
        ("CatalogService", "the name", CYAN),
        ("PricingService", "the price", CYAN),
        ("InventoryService", "in stock?", CYAN),
        ("RecommendationsService", "also bought  —  OPTIONAL", AMBER),
    ]
    # The spine leaves the gateway, so the fan-out reads as the gateway's work.
    ...
    for label, sub, colour in services:
        ...
        arrow_right(d, 1084, y + 48, 1154, colour, 3)
        y += 116

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 70, 790, 560, 110, "NaiveMobileApp",
        "«the trap»  four crossings, four token checks", RED,
        label_size=30, sub_size=22)

    d.text((W // 2, 960),
           "The gateway joins and forwards.  It prices nothing, discounts "
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 288), "API GATEWAY", font=f(SANS_B, 122), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, "4 calls,  4 checks",
         "800ms  —  and no page at all if one fails", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "1 call,  1 check",
         "240ms  —  and a page, minus one row", tag="AFTER")
    ...
    d.text((W // 2, 744), "Four calls is four chances to lose the page.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "One front door, and one place to decide.",
