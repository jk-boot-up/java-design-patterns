# API Gateway Pattern — Project Specification
...
[`api-gateway-pattern-explained.md`](api-gateway-pattern-explained.md); the problem it
...
- Not an HTTP server. Nothing binds a port; a remote call is a `RemoteCall` that advances a simulated clock and writes a line into a timeline.
- Not a proof that the page is correct. Both versions of the app return the same product page, so any test asserting the page's contents would pass on the naive version too and prove nothing.
- Not business logic. The gateway must not price, discount or decide — the moment it does, it is a second copy of the shop, and a test pins the price it publishes to Pricing's answer unmodified.
- Not a distributed system. One JVM, no mesh, no partitions, no capacity planning; what the project teaches is the pattern's shape.
...
18 production classes under `com.jk.explore.apigateway`:
...
1. **One crossing from the client.** A test counts the calls the phone makes and asserts one against the naive app's five.
2. **The token is checked once.** `CallLog.countFor("Auth")` is 1 for the gateway and 4 for the naive app.
3. **Parallel, and the clock proves it.** The gateway's page arrives in 240ms where the naive app's sequential calls take 800ms; both numbers are asserted against `SimulatedClock`, not measured.
4. **An optional service failing degrades the page.** With Recommendations down the page still carries name, price and stock, and says it has no recommendations rather than pretending there are none.
5. **An essential service failing fails honestly.** With Catalog down the gateway raises the failure rather than serving a page with a blank name.
6. **The gateway adds no logic.** The published price equals Pricing's answer; the test exists to stop a future feature being put in the wrong place.
...
- `./gradlew build` passes. 25 test methods across `NaiveMobileAppTest`, `ProductPageGatewayTest`, `SimulationHarnessTest`.
...
| `api-gateway-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/api-gateway-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the API Gateway pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Service Registry and Discovery.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py api-gateway`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py api-gateway
python3 ../../docs/make_specs.py api-gateway
```
