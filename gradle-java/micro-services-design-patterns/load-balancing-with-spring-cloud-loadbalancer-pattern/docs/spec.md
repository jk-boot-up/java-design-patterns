# Load Balancing with Spring Cloud LoadBalancer Pattern — Project Specification
...
[`load-balancing-with-spring-cloud-loadbalancer-pattern-explained.md`](load-balancing-with-spring-cloud-loadbalancer-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.loadbalancersc`:
...
1. **Work is counted as cost units,** never timed.
2. **Round robin's random start is respected:** nothing asserts which copy is first.
3. **Every demo line is the same on every run.**
...
- `./gradlew build` passes. 6 test methods across `DemoRunsTest`, `LoadBalancerTest`.
...
| `load-balancing-with-spring-cloud-loadbalancer-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/load-balancing-with-spring-cloud-loadbalancer-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Load Balancing with Spring Cloud LoadBalancer pattern in
...
| Runtime | ~4:43 over 16 scenes, 70 subtitle cues |  |
...
> `python3 ../../docs/make_youtube_docs.py load-balancing-with-spring-cloud-loadbalancer`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py load-balancing-with-spring-cloud-loadbalancer
python3 ../../docs/make_specs.py load-balancing-with-spring-cloud-loadbalancer
```
