#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Load Balancing with Spring Cloud LoadBalancer video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Load Balancing with Spring Cloud LoadBalancer  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("LOAD BALANCING", 152, TEXT)
    ...
    # Round robin versus least work -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "round robin",
         "work 4, 4, 24", tag="FAIR NOT FAST")
    pill(d, 1070, 556, 700, 170, GREEN, "least work so far",
         "work 6, 5, 6", tag="EVEN WORK")
    ...
    d.text((W // 2, 744), "A name in, an address out.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Choose per request.",
