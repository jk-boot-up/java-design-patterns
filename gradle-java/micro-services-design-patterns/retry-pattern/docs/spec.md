# Retry with Backoff Pattern — Project Specification
...
[`retry-pattern-explained.md`](retry-pattern-explained.md); the problem it
...
- Not Resilience4j. `Retrier` is about thirty lines; the value is in the policy decisions, not the library.
- Not a defence of retrying everything. Half the project is failures that must not be retried.
- Not idempotency in full — the duplicate charge is shown here and solved in projects 36 and 37.
...
13 production classes under `com.jk.explore.retry`:
...
1. **A transient failure is absorbed.** Two timeouts followed by a success produce one receipt and three attempts, and the caller sees no error.
2. **A permanent failure is not retried.** A declined card is attempted exactly once; the test counts gateway calls, so a future change to the policy cannot quietly retry it.
3. **The backoff is on the clock.** With a 100ms base and doubling, the third attempt starts at 300ms of waiting, asserted against `SimulatedClock` — nothing sleeps.
4. **Attempts are capped**, and the last failure is what the caller receives, not a wrapper that hides which call failed.
5. **The duplicate charge is demonstrated, not mentioned.** A call that succeeds and loses its reply is retried and charges twice; the test asserts two charges and the notes name idempotency as the fix.
...
- `./gradlew build` passes. 21 test methods across `DemoRunsTest`, `NaiveCheckoutServiceTest`, `RetryTest`.
...
| `retry-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/retry-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Retry with Backoff pattern in
...
| Runtime | ~16:12 over 16 scenes, 241 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Retry with Backoff in Java - A Flaky Payment Gateway"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Circuit Breaker.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py retry`. They are the one part of
...
integrated, -3.59 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py retry
python3 ../../docs/make_specs.py retry
```
