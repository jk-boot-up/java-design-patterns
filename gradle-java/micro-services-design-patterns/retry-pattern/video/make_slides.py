...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Retry with Backoff  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson: the caller's two arrows are drawn in the
    # order the code performs them, because that order *is* the safety
    # property. The retrier's two decisions sit under it, and the three
    # failures on the right say which decision each one exercises.
    box(d, 70, 232, 560, 130, "CheckoutService",
        "«caller»   builds the request, then retries", CLIENT,
        label_size=38, sub_size=24)

    arrow(d, 200, 366, 200, 448, GREEN, 5)
    d.text((250, 386), "PaymentRequest.forOrder(id, amount)",
           font=f(MONO_B, 26), fill=GREEN, anchor="la")
    d.text((250, 418), "one key, built BEFORE the first attempt",
           font=f(SANS, 22), fill=MUTED, anchor="la")

    box(d, 70, 452, 560, 140, "Retrier",
        "«the loop»  two decisions, forty lines", AMBER,
        label_size=38, sub_size=24)

    arrow(d, 200, 596, 200, 636, AMBER, 4)
    box(d, 70, 640, 560, 110, "RetryPolicy",
        "«how long»  100ms, doubling, plus jitter", AMBER,
        label_size=30, sub_size=22)

    d.text((1465, 372), "one checkout  ·  £449.99  ·  three ways to fail",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "and only one of them charges the card twice",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # The three failures, each labelled with what the retrier does about it
    # and what the shopper ends up paying. The lost reply is rose because it
    # is the one that looks identical to the first and costs real money.
    failures = [
        ("GatewayTimeoutException", "retry  ·  nothing was charged  —  £449.99", GREEN),
        ("CardDeclinedException", "rethrow at once  ·  one attempt, 50ms", CYAN),
        ("charged, then reply lost", "looks like a timeout  ·  £449.99 or £899.98", ROSE),
        ("PaymentGateway", "keeps one map: key -> receipt", MUTED),
    ]
    # The spine runs into the retrier, because these are things that happen
    # to it rather than things it goes looking for.
    d.line([(630, 522), (1084, 522)], fill=LINE, width=3)
    d.line([(1084, 522), (1084, 848)], fill=LINE, width=3)

    y = 452
    for label, sub, colour in failures:
        box(d, 1160, y, 690, 96, label, sub, colour,
            label_size=30, sub_size=22)
        arrow_left(d, 1154, y + 48, 1088, colour, 3)
        y += 116

    # The naive alternative, kept on screen so the comparison is part of the
    # picture rather than a claim made once and then dropped.
    box(d, 70, 790, 560, 110, "NaiveCheckoutService",
        "«no retrier»  builds the key inside the loop", ROSE,
        label_size=28, sub_size=22)

    d.text((W // 2, 960),
           "The retrier runs a Supplier and has no idea what it is retrying.  "
           "Only the caller can make the job safe to repeat.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 152), "RETRY", font=f(SANS_B, 122), fill=TEXT)
    d.text((110, 288), "WITH BACKOFF", font=f(SANS_B, 104), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, '"key-" + id + "-" + attempt',
         "the card is charged twice  —  £899.98", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, '"key-" + orderId',
         "the card is charged once  —  £449.99", tag="AFTER")
    ...
    d.text((W // 2, 744), "It throws nothing, logs nothing, and every test passes.",
           font=f(SANS_B, 46), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "One line decides where the money goes.",
