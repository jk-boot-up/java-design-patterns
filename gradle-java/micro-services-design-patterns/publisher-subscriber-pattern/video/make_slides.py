#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Publisher-Subscriber video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Publisher-Subscriber  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("PUBLISHER-", 152, TEXT)
    fitted("SUBSCRIBER", 288, ACCENT)
    ...
    # Calling each one versus publishing once -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "order service calls 3 by name",
         "a fourth means editing it", tag="COUPLED")
    pill(d, 1070, 556, 700, 170, GREEN, "order service publishes once",
         "loyalty joins, no change", tag="DECOUPLED")
    ...
    d.text((W // 2, 744), "Publish once.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Anyone may listen.",
