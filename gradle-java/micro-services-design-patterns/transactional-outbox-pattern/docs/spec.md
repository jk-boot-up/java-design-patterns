# Transactional Outbox Pattern — Project Specification
...
[`transactional-outbox-pattern-explained.md`](transactional-outbox-pattern-explained.md); the problem it
...
- Not Kafka or Debezium. `MessageBroker` is a map with subscribers, and the relay is a method somebody calls.
- Not exactly-once delivery. The relay can publish and die before marking the message sent, so delivery is at-least-once — which is why the next project exists.
- Not change-data-capture. The relay polls the outbox table, which is the simpler of the two implementations and the one worth understanding first.
...
15 production classes under `com.jk.explore.transactionaloutbox`:
...
1. **Both or neither.** A crash before the commit leaves no order and no message; the commit writes one order and one outbox message together, and the log line says so.
2. **No broker call inside the transaction.** `OrderService.placeOrder` touches the database only, so a slow or dead broker cannot lengthen or fail the write.
3. **The message outlives the process.** With the broker down the relay logs `LEFT-IN-TRAY` twice and delivers both messages when it returns; nothing is lost and nobody minds.
4. **Delivery is at-least-once, demonstrated.** `OutboxRelay.dieAfterPublishing()` opens the one gap the pattern cannot close, and a test asserts the same message delivered twice and two identical emails.
5. **The duplicate is recognisable.** The redelivered message carries the same id, which is exactly what the next pattern needs.
6. **The naive version passes every test**, including the one where the order exists and zero events and zero emails went out.
7. **The family resemblance is named.** The notes connect this to double-checked locking's bug in a bigger coat: two operations that must both happen, with no single mechanism covering them.
...
- `./gradlew build` passes. 18 test methods across `DemoRunsTest`, `NaiveOrderServiceTest`, `TransactionalOutboxTest`.
...
| `transactional-outbox-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/transactional-outbox-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Transactional Outbox pattern in
...
| Runtime | ~10:36 over 16 scenes, 155 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Transactional Outbox in Java - Never Lose an Event"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Idempotent Consumer.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py transactional-outbox`. They are the one part of
...
integrated, -4.00 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py transactional-outbox
python3 ../../docs/make_specs.py transactional-outbox
```
