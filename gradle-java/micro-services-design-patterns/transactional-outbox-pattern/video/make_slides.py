#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Transactional Outbox video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Transactional Outbox  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # Down the left is the two-line version and what is left after a crash.
    # The viewer has to have felt the loss before the right-hand column is
    # anything other than extra machinery.
    box(d, 70, 232, 560, 120, "NaiveOrderService",
        "«save, then publish»   two lines, and a gap", MUTED,
        label_size=30, sub_size=24)

    arrow(d, 200, 356, 200, 430, RED, 5)
    d.text((250, 370), "the process dies in between",
           font=f(MONO_B, 26), fill=RED, anchor="la")
    d.text((250, 402), "a deploy landing mid-request is enough",
           font=f(SANS, 21), fill=MUTED, anchor="la")

    box(d, 70, 434, 560, 130, "order saved, nobody told",
        "and no error, because nothing failed", AMBER,
        label_size=30, sub_size=22)

    d.text((80, 590), "nothing recorded that a message was owed",
           font=f(SANS_B, 26), fill=RED, anchor="la")
    d.text((80, 622), "so there is nothing anywhere to retry",
    ...
    d.text((1465, 372), "the message goes into the database, next to the order",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "and somebody comes round later and posts it",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # OrderService is green because that is where the guarantee is bought, in
    # one ordinary commit. The relay is the moving part you now have to run.
    work = [
        ("OrderService → Transaction", "save(order)  ·  save(message)  ·  commit()", GREEN),
        ("OutboxMessage", "an ordinary row  ·  and a stable message id", CYAN),
        ("OutboxRelay", "unsent  ·  publish  ·  markSent  —  no retry code", MUTED),
        ("MessageBroker", "may be down, and checkout does not notice", AMBER),
    ]
    ...
    d.text((W // 2, 960),
           "OrderService has no reference to the broker at all.  "
           "That absence is the pattern.",
...
def kind_poster(scene, img, d):
    ...
    # "OUTBOX" is the word a viewer will actually remember, so it gets the
    # thumbnail-sized type, and the full name of the pattern sits under it in
    # the accent colour for anybody searching for the term itself.
    d.text((110, 146), "OUTBOX", font=f(SANS_B, 172), fill=TEXT)
    d.text((110, 348), "SAVE IT, THEN SEND IT", font=f(SANS_B, 66), fill=ACCENT)

    d.rectangle([110, 462, 620, 472], fill=RULE)

    # One box around the whole idea — the two ways of announcing an order, and
    # the sentence that explains why one of them loses it.
    d.rounded_rectangle([110, 516, W - 110, 866], radius=26,
                        outline=ACCENT, width=4)

    # Both sets of numbers come out of the demo: act two, and acts three to five.
    pill(d, 150, 556, 700, 170, RED, 'save, then publish',
         "1 order saved  ·  0 emails  ·  nothing threw", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'two rows, one commit',
         "1 order saved  ·  1 email  ·  0 lost, ever", tag="AFTER")
    ...
    d.text((W // 2, 750), "A database write and a message send cannot be one act. So do not send it.",
           font=f(SANS_B, 36), fill=TEXT, anchor="ma")
    d.text((W // 2, 800), "Never lost, sometimes twice.",
