# API Composition Pattern — Project Specification
...
[`api-composition-pattern-explained.md`](api-composition-pattern-explained.md); the problem it
...
- Not a threading exercise. `Fanout` is a loop over a simulated clock, because the pattern's lesson is the dependency shape and the classification, not `CompletableFuture` syntax.
- Not a claim that parallelism fixes latency. It removes the addition and leaves the maximum.
- Not CQRS. Composition on demand is what CQRS replaces when these two costs can no longer be lived with.
...
16 production classes under `com.jk.explore.apicomposition`:
...
1. **The sequential version works and is kept.** Its test class passes, including `itBuildsTheRightPage`, so the comparison is about time and availability rather than correctness.
2. **Parallel calls cost the maximum.** 210ms sequential against 150ms composed, asserted on the simulated clock, with a test checking that both parallel branches share a start time.
3. **The dependency shape is honest.** Catalog cannot start until Orders has named the skus, so the fan-out is one call and then two together, not a flat three.
4. **Required and optional are explicit.** Orders required, Catalog and Shipping optional: `Branch.value()` rethrows and `Branch.valueOr(fallback)` substitutes, and a required failure refuses to build a page at all.
5. **The page names its gaps.** `DeliveryStatus.unknown()` says it cannot check rather than guessing "in transit", and `OrderDetailsPage.missingSections()` lists what is absent, because a quietly dropped section is indistinguishable from an order that has not shipped.
6. **Availability multiplies, in code.** Three 99.9% services give a 99.7% page — 129.5 minutes a month against 43.2 — computed by `Availability` and pinned by `AvailabilityTest` so the prose cannot drift from the arithmetic.
7. **The slowest dependency sets the pace.** With Shipping at 400ms the page is 430ms, asserted, so the limit of the pattern is a test rather than a caveat.
...
- `./gradlew build` passes. 22 test methods across `AvailabilityTest`, `DemoRunsTest`, `OrderDetailsComposerTest`, `SequentialOrderDetailsComposerTest`.
...
| `api-composition-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/api-composition-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the API Composition pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: CQRS.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py api-composition`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py api-composition
python3 ../../docs/make_specs.py api-composition
```
