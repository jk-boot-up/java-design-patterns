#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Retry with Resilience4j video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Retry with Resilience4j  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # One layer versus two layers -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "two layers, 3 x 3",
         "9 calls to a dead gateway", tag="MULTIPLIED")
    pill(d, 1070, 556, 700, 170, GREEN, "one layer, 3 attempts",
         "3 calls", tag="CONTAINED")
    ...
    d.text((W // 2, 744), "An annotation and four settings.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Retry in one layer.",
