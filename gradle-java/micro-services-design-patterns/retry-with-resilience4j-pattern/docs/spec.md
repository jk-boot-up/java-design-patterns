# Retry with Resilience4j Pattern — Project Specification
...
[`retry-with-resilience4j-pattern-explained.md`](retry-with-resilience4j-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.retryr4j`:
...
1. **Calls and charges are counted,** never timed.
2. **Waits are the values Resilience4j chose,** read from its events.
3. **The double charge is asserted,** and so is its fix.
...
- `./gradlew build` passes. 7 test methods across `DemoRunsTest`, `RetryTest`.
...
| `retry-with-resilience4j-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/retry-with-resilience4j-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Retry with Resilience4j pattern in
...
| Runtime | ~5:00 over 16 scenes, 73 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Bulkhead with Resilience4j.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py retry-with-resilience4j`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py retry-with-resilience4j
python3 ../../docs/make_specs.py retry-with-resilience4j
```
