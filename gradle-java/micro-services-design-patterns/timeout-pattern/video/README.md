# Timeout Pattern — Teaching Video

A narrated, slide-based video that teaches Timeout, and argues for when not to use it,
...
| File | What it is |
| --- | --- |
| `timeout-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. |
| `timeout-pattern-explained.m4a` | Audio-only version. |
| `timeout-pattern-explained.srt` | Subtitles. |
...
| # | Scene | Covers |
| --- | --- | --- |
| 1 | Timeout | Title card and the definition |
...
`python3 ../../docs/make_narration.py --force timeout`, then
...
Upload `timeout-pattern-explained.mp4`, with
