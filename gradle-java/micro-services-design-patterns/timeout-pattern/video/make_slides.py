#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Timeout video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Timeout  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("TIMEOUT", 152, TEXT)
    fitted("NEVER WAIT FOREVER", 288, ACCENT)
    ...
    # A thread waiting versus a limit -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "no limit",
         "thread WAITING, forever", tag="STUCK")
    pill(d, 1070, 556, 700, 170, GREEN, "limit of 100 ms",
         "stock unknown, try again", tag="MOVED ON")
    ...
    d.text((W // 2, 744), "Decide how long to wait.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Then stop waiting.",
