# Timeout Pattern — Project Specification
...
[`timeout-pattern-explained.md`](timeout-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.timeoutpattern`:
...
1. **Silence is exact:** the supplier is held at a gate.
2. **Waits are polled, never slept.**
3. **The abandoned work is asserted to finish anyway.**
4. **A budget never spends more than it has,** checked over a range.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `TimeoutTest`.
...
| `timeout-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/timeout-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Timeout pattern in
...
| Runtime | ~5:03 over 14 scenes, 76 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Queue-Based Load Leveling.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py timeout`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py timeout
python3 ../../docs/make_specs.py timeout
```
