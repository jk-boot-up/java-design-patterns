# Rate Limiter Pattern — Teaching Video

A narrated, slide-based video that teaches Rate Limiter, and argues for when not to use it,
...
| File | What it is |
| --- | --- |
| `rate-limiter-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. |
| `rate-limiter-pattern-explained.m4a` | Audio-only version. |
| `rate-limiter-pattern-explained.srt` | Subtitles. |
...
| # | Scene | Covers |
| --- | --- | --- |
| 1 | Rate Limiter | Title card and the definition |
...
`python3 ../../docs/make_narration.py --force rate-limiter`, then
...
Upload `rate-limiter-pattern-explained.mp4`, with
