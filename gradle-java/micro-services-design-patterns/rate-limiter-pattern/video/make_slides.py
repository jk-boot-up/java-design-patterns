#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Rate Limiter video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Rate Limiter  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("RATE LIMITER", 152, TEXT)
    fitted("SAY NO EARLY", 288, ACCENT)
    ...
    # No limit versus a bucket -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "1000 requests, no limit",
         "900 over capacity", tag="OVERWHELMED")
    pill(d, 1070, 556, 700, 170, GREEN, "a bucket of 10",
         "10 allowed, 10 refused", tag="LIMITED")
    ...
    d.text((W // 2, 744), "A burst, then a steady rate.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Told when to come back.",
