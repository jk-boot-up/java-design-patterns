# Rate Limiter Pattern — Project Specification
...
[`rate-limiter-pattern-explained.md`](rate-limiter-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.ratelimiter`:
...
2. **Arithmetic is exact:** milli-tokens, no floating point.
3. **A steady rate at the refill rate is never refused,** asserted over 300 requests.
4. **Retry-after is exact:** early is refused, on time is allowed.
...
- `./gradlew build` passes. 11 test methods across `DemoRunsTest`, `RateLimiterTest`.
...
| `rate-limiter-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/rate-limiter-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Rate Limiter pattern in
...
| Runtime | ~5:09 over 14 scenes, 77 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Timeout.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py rate-limiter`. They are the one part of
...
integrated, -3.50 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py rate-limiter
python3 ../../docs/make_specs.py rate-limiter
```
