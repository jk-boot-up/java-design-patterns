# Queue-Based Load Leveling Pattern — Project Specification
...
[`queue-based-load-leveling-pattern-explained.md`](queue-based-load-leveling-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.loadleveling`:
...
1. **Every run is exact:** ticks and integer arithmetic, no clocks or threads.
2. **Every order is accounted for:** arrived = processed + refused + lost + waiting, over a range of limits and crash points.
3. **The wait is exact:** first 0, last 9, average 4.5.
4. **An in-memory queue's loss is asserted.**
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `LoadLevelingTest`.
...
| `queue-based-load-leveling-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/queue-based-load-leveling-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Queue-Based Load Leveling pattern in
...
| Runtime | ~5:04 over 14 scenes, 78 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Competing Consumers.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py queue-based-load-leveling`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py queue-based-load-leveling
python3 ../../docs/make_specs.py queue-based-load-leveling
```
