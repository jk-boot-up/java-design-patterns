#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Pipes and Filters video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Pipes and Filters  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("PIPES AND FILTERS", 152, TEXT)
    fitted("SMALL STEPS", 288, ACCENT)
    ...
    # One loop versus a pipeline -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "one loop, 5 jobs",
         "3 lines dropped, no reason", tag="ONE BIG LOOP")
    pill(d, 1070, 556, 700, 170, GREEN, "parse | validate | price | tax",
         "3 rejected, each with a reason", tag="FIVE STEPS")
    ...
    d.text((W // 2, 744), "Each step does one thing.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Join them end to end.",
