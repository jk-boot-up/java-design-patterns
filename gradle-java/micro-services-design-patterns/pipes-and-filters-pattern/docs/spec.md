# Pipes and Filters Pattern — Project Specification
...
[`pipes-and-filters-pattern-explained.md`](pipes-and-filters-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.pipesfilters`:
...
1. **The pipeline and the big method agree** on the good lines.
2. **Each filter is tested alone.**
3. **Rejects carry the step and the reason,** in order.
4. **Streaming holds 1 item and stage-by-stage holds 2000 for 1000 lines,** with identical output.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `PipesAndFiltersTest`.
...
| `pipes-and-filters-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/pipes-and-filters-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Pipes and Filters pattern in
...
| Runtime | ~5:14 over 14 scenes, 74 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Scatter-Gather.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py pipes-and-filters`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py pipes-and-filters
python3 ../../docs/make_specs.py pipes-and-filters
```
