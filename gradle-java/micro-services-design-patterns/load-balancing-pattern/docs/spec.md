# Client-Side Load Balancing Pattern — Project Specification
...
[`load-balancing-pattern-explained.md`](load-balancing-pattern-explained.md); the problem it
...
- Not a load balancer appliance or an nginx configuration. The balancing here is client-side, which is where the pattern lives in a service mesh too.
- Not service discovery. The list of instances is a given; this project is about picking from it.
- Not a benchmark. The cluster's latencies are fixed and deliberately uneven so that the comparison is arithmetic rather than luck.
...
14 production classes under `com.jk.explore.loadbalancing`:
...
1. **Round-robin is exactly fair.** Nine calls across three instances give three each, asserted per instance rather than in aggregate.
2. **Always-first is measurably worse**, and its own test class passes: it sends all nine calls to one instance and the total time says what that costs.
3. **Least-latency tries everyone once.** No instance is written off unmeasured, and after the measuring round the fast instances take the majority of the traffic.
4. **Two clients do not collide.** Each client keeps its own cursor, so two round-robin clients each take perfect turns — which is also why client-side balancing is not global balancing, and the notes say so.
5. **The pattern is named as Strategy.** The notes state that this is the behavioural pattern from project 15 applied to a network, so the reader meets one idea twice rather than two ideas once.
...
- `./gradlew build` passes. 21 test methods across `DemoRunsTest`, `FirstInstanceBalancerTest`, `LoadBalancerTest`.
...
| `load-balancing-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/load-balancing-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Client-Side Load Balancing pattern in
...
| Runtime | ~18:31 over 16 scenes, 265 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Client-Side Load Balancing in Java - Catalog Reads"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Retry with Backoff.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py load-balancing`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py load-balancing
python3 ../../docs/make_specs.py load-balancing
```
