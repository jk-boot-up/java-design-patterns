...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Client-Side Load Balancing  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson: the client points at the interface and at
    # nothing else, and the four strategies point *into* the interface. Each
    # strategy carries the share it produced and what that share cost, because
    # the numbers are the argument.
    box(d, 70, 232, 560, 130, "CatalogClient",
        "«caller»   chooses afresh on every request", CLIENT,
        label_size=38, sub_size=24)

    arrow(d, 200, 366, 200, 448, CLIENT, 5)
    d.text((250, 386), "balancer.choose(candidates)",
           font=f(MONO_B, 26), fill=CLIENT, anchor="la")
    d.text((250, 418), "and it never asks which balancer it holds",
           font=f(SANS, 22), fill=MUTED, anchor="la")

    box(d, 70, 452, 560, 140, "LoadBalancer",
        "«the choice»  one method, four answers", AMBER,
        label_size=38, sub_size=24)

    arrow(d, 200, 596, 200, 636, GREEN, 4)
    box(d, 70, 640, 560, 110, "observed(instance, ms)",
        "«default»  only the learner implements it", GREEN,
        label_size=28, sub_size=22)

    d.text((1465, 372), "twelve requests  ·  three machines  ·  one slow",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "same answer every time — a different bill each time",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # The four strategies, each labelled with the share it produced and the
    # total it cost. The naive one is rose because it is the fastest and the
    # one you must not ship, which is the hardest idea in the project.
    strategies = [
        ("FirstInstanceBalancer", "12 / 0 / 0  ·  120ms  —  fastest, and wrong", ROSE),
        ("RoundRobinBalancer", "4 / 4 / 4  ·  320ms  —  fair, and not fast", CYAN),
        ("LeastLatencyBalancer", "10 / 1 / 1  ·  170ms  —  fast, and it can herd", GREEN),
        ("RandomBalancer", "no state  ·  seeded, so tests can assert it", MUTED),
    ]
    # The spine runs into the interface, because implementing is something the
    # strategies do to it rather than something it does to them.
    d.line([(630, 522), (1084, 522)], fill=LINE, width=3)
    d.line([(1084, 522), (1084, 848)], fill=LINE, width=3)

    y = 452
    for label, sub, colour in strategies:
        box(d, 1160, y, 690, 96, label, sub, colour,
            label_size=30, sub_size=22)
        arrow_left(d, 1154, y + 48, 1088, colour, 3)
        y += 116

    # The other answer entirely, kept on screen so the honest ending is part
    # of the picture rather than an afterthought at the end of the talk.
    box(d, 70, 790, 560, 110, "one balancer in front",
        "«server-side»  it can see every request", AMBER,
        label_size=30, sub_size=22)

    d.text((W // 2, 960),
           "A client can only balance the traffic it can see.  Two clients "
           "taking perfect turns can leave a machine idle.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 152), "LOAD", font=f(SANS_B, 122), fill=TEXT)
    d.text((110, 288), "BALANCING", font=f(SANS_B, 122), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, 'candidates.get(0)',
         "three machines bought, one machine used", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'balancer.choose(list)',
         "all three used  —  and the slow one spared", tag="AFTER")
    ...
    d.text((W // 2, 744), "The fastest strategy here is the one you must not ship.",
           font=f(SANS_B, 46), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Fair is not the same as fast.",
