# Claim Check Pattern — Project Specification
...
[`claim-check-pattern-explained.md`](claim-check-pattern-explained.md); the problem it
...
8 production classes under `com.jk.explore.claimcheck`:
...
1. **Bytes are counted,** never timed.
2. **Expiry uses a clock the demo controls.**
3. **A tampered payload is refused by its checksum.**
4. **Sequential claims are shown to be guessable,** and random claims are not.
...
- `./gradlew build` passes. 9 test methods across `ClaimCheckTest`, `DemoRunsTest`.
...
| `claim-check-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/claim-check-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Claim Check pattern in
...
| Runtime | ~5:09 over 14 scenes, 79 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Leader Election.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py claim-check`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py claim-check
python3 ../../docs/make_specs.py claim-check
```
