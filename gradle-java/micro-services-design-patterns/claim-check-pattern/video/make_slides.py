#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Claim Check video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Claim Check  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("CLAIM CHECK", 152, TEXT)
    fitted("SEND THE TICKET", 288, ACCENT)
    ...
    # Luggage versus a ticket -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "100 invoices, whole",
         "500000 bytes", tag="HEAVY")
    pill(d, 1070, 556, 700, 170, GREEN, "100 invoices, by claim",
         "5900 bytes", tag="LIGHT")
    ...
    d.text((W // 2, 744), "Store the luggage.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Send the ticket.",
