#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Service Discovery with Spring Cloud Consul video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Service Discovery with Spring Cloud Consul  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("SERVICE DISCOVERY", 152, TEXT)
    fitted("WITH SPRING CLOUD CONSUL", 288, ACCENT)
    ...
    # A stale entry after a crash -- a real line from the project.
    pill(d, 150, 556, 700, 170, RED, "crashed, still listed",
         "3 of 6 requests failed", tag="STALE")
    pill(d, 1070, 556, 700, 170, GREEN, "health check fails",
         "all 6 requests answered", tag="REMOVED")
    ...
    d.text((W // 2, 744), "Copies register themselves.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "The list is only as fresh as its check.",
