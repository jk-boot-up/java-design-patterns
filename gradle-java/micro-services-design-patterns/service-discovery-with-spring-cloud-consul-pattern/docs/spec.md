# Service Discovery with Spring Cloud Consul Pattern — Project Specification
...
[`service-discovery-with-spring-cloud-consul-pattern-explained.md`](service-discovery-with-spring-cloud-consul-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.discoveryconsul`:
...
1. **Consul is real,** started on free ports and stopped afterwards.
2. **Waits are bounded polls,** never fixed sleeps; only counts are asserted.
3. **Tests are skipped when the `consul` program is missing.**
...
- `./gradlew build` passes. 5 test methods across `DemoRunsTest`, `DiscoveryTest`.
...
| `service-discovery-with-spring-cloud-consul-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/service-discovery-with-spring-cloud-consul-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Service Discovery with Spring Cloud Consul pattern in
...
| Runtime | ~4:52 over 16 scenes, 68 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Layered Architecture with Spring Boot.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py service-discovery-with-spring-cloud-consul`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py service-discovery-with-spring-cloud-consul
python3 ../../docs/make_specs.py service-discovery-with-spring-cloud-consul
```
