# Circuit Breaker Pattern — Project Specification
...
[`circuit-breaker-pattern-explained.md`](circuit-breaker-pattern-explained.md); the problem it
...
- Not a replacement for retry. The two answer different questions, and the project states the question that separates them: is the next attempt plausibly going to work?
- Not a rate limiter or a timeout. The breaker's only input is the recent failure history of one dependency.
- Not an argument that every dependency deserves a fallback. One of this project's classes exists to show a fallback that must never be written.
...
16 production classes under `com.jk.explore.circuitbreaker`:
...
1. **Retrying an outage is shown to be worse**, with its own passing test: ten shoppers cost 90,000ms of simulated time and thirty calls reach a service that is down.
2. **The breaker opens on consecutive failures** — three in a row — and one success resets the count, because a service that answers three times and fails once is not down.
3. **An open breaker is free.** Twenty pages served after the trip advance the simulated clock by zero milliseconds and make zero calls.
4. **Half-open lets exactly one call through** after the cooling period, closes on success, and re-opens for a full wait on a single failed probe.
5. **Both answers to the open state are implemented.** Recommendations is hidden behind a `degraded` flag on the page; Payments is refused honestly with zero cards charged.
6. **The dishonest fallback is demonstrated and condemned.** `PretendItWorkedCheckoutService` returns a receipt while charging nothing; the test asserts a thanked shopper and zero charges, and the notes state that a fallback hiding a real failure is worse than the error it replaced.
...
- `./gradlew build` passes. 26 test methods across `CircuitBreakerTest`, `DemoRunsTest`, `FallbackChoiceTest`, `RetryingProductPageServiceTest`.
...
| `circuit-breaker-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/circuit-breaker-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Circuit Breaker pattern in
...
| Runtime | ~18:06 over 16 scenes, 255 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Circuit Breaker in Java - When a Service Stops"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Bulkhead.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py circuit-breaker`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py circuit-breaker
python3 ../../docs/make_specs.py circuit-breaker
```
