...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Circuit Breaker  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson: one breaker class on the left with the
    # three states under it, and on the right the four callers, each labelled
    # with what it does when the breaker refuses. The breaker holds no
    # fallback of its own, which is why the decision lives on the right.
    box(d, 70, 232, 560, 130, "CircuitBreaker",
        "«one class»   runs a Supplier, knows nothing else", CLIENT,
        label_size=38, sub_size=24)

    arrow(d, 200, 366, 200, 448, GREEN, 5)
    d.text((250, 386), "3 failures in a row  ->  trip",
           font=f(MONO_B, 26), fill=GREEN, anchor="la")
    d.text((250, 418), "5000ms later, let exactly ONE call through",
           font=f(SANS, 22), fill=MUTED, anchor="la")

    box(d, 70, 452, 560, 140, "BreakerState",
        "«closed»  current flows, so calls go through", AMBER,
        label_size=38, sub_size=24)

    arrow(d, 200, 596, 200, 636, AMBER, 4)
    box(d, 70, 640, 560, 110, "SimulatedClock",
        "«the timer»  no scheduler, no background thread", AMBER,
        label_size=30, sub_size=22)

    d.text((1465, 372), "one outage  ·  3000ms a call  ·  four callers",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "and only one of them says something untrue",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # The four callers. The liar is rose because it is the one that costs
    # real money while every dashboard stays green.
    callers = [
        ("ProductPageService", "optional  ·  serves the page, no suggestions", GREEN),
        ("CheckoutService", "essential  ·  an honest no, in 0ms", CYAN),
        ("PretendItWorkedCheckoutService", "invents a receipt  ·  0 cards charged", ROSE),
        ("RetryingProductPageService", "no breaker  ·  9000ms, every single time", MUTED),
    ]
    # The spine runs into the state box, because what each caller sees is
    # decided by which state the breaker happens to be in.
    d.line([(630, 522), (1084, 522)], fill=LINE, width=3)
    d.line([(1084, 522), (1084, 848)], fill=LINE, width=3)

    y = 452
    for label, sub, colour in callers:
        box(d, 1160, y, 690, 96, label, sub, colour,
            label_size=28, sub_size=22)
        arrow_left(d, 1154, y + 48, 1088, colour, 3)
        y += 116

    # The two exceptions, kept on screen because the difference between them
    # is the whole cost argument: one waited three seconds, one waited none.
    box(d, 70, 790, 560, 110, "CircuitOpenException",
        "«no call was made»  the answer cost nothing", ROSE,
        label_size=28, sub_size=22)

    d.text((W // 2, 960),
           "The breaker runs a Supplier and has never heard of shopping.  "
           "Only the caller can know whether a fallback is true.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 152), "CIRCUIT", font=f(SANS_B, 122), fill=TEXT)
    d.text((110, 288), "BREAKER", font=f(SANS_B, 104), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, 'for (int i = 0; i < 3; i++)',
         "every shopper waits 9000ms  —  and calls again", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'breaker.call(...)  ->  OPEN',
         "refused in 0ms  —  no call is made", tag="AFTER")
    ...
    d.text((W // 2, 744), "A breaker does not make failures disappear.",
           font=f(SANS_B, 46), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "It makes them fail fast  —  and then you must choose.",
