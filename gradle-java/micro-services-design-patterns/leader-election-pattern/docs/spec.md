# Leader Election Pattern — Project Specification
...
[`leader-election-pattern-explained.md`](leader-election-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.leaderelection`:
...
1. **Time is a clock the demo controls,** never a real wait.
2. **The token goes up at every change of holder.**
3. **A replaced leader's write is refused with fencing and accepted without.**
4. **A lease shorter than the renewal interval loses a healthy leader,** at an exact second.
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `LeaderElectionTest`.
...
| `leader-election-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/leader-election-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Leader Election pattern in
...
| Runtime | ~5:14 over 14 scenes, 79 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Publisher-Subscriber.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py leader-election`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py leader-election
python3 ../../docs/make_specs.py leader-election
```
