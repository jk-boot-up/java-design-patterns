#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Leader Election video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Leader Election  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("LEADER ELECTION", 152, TEXT)
    fitted("ONE IN CHARGE", 288, ACCENT)
    ...
    # Two leaders versus a fencing token -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "A wakes, still leading",
         "report sent twice", tag="TWO LEADERS")
    pill(d, 1070, 556, 700, 170, GREEN, "token 1 is older than 2",
         "only B's report sent", tag="FENCED")
    ...
    d.text((W // 2, 744), "A lease elects the leader.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "A token stops the old one.",
