#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Bulkhead with Resilience4j video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Bulkhead with Resilience4j  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # One shared compartment versus one each -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "one shared compartment",
         "checkout refused", tag="STARVED")
    pill(d, 1070, 556, 700, 170, GREEN, "a compartment each",
         "checkout sold", tag="SELLING")
    ...
    d.text((W // 2, 744), "A slow feed fills its own room.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Not the whole ship.",
