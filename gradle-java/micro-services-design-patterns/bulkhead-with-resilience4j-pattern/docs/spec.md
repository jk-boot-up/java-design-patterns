# Bulkhead with Resilience4j Pattern — Project Specification
...
[`bulkhead-with-resilience4j-pattern-explained.md`](bulkhead-with-resilience4j-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.bulkheadr4j`:
...
1. **Slow calls are held at a gate,** never slept.
2. **Permits are read from the registry.**
...
- `./gradlew build` passes. 5 test methods across `BulkheadTest`, `DemoRunsTest`.
...
| `bulkhead-with-resilience4j-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/bulkhead-with-resilience4j-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Bulkhead with Resilience4j pattern in
...
| Runtime | ~4:45 over 16 scenes, 68 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: API Gateway with Spring Cloud Gateway.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py bulkhead-with-resilience4j`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py bulkhead-with-resilience4j
python3 ../../docs/make_specs.py bulkhead-with-resilience4j
```
