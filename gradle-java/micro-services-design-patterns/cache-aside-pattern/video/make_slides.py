#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Cache-Aside video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Cache-Aside  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("CACHE-ASIDE", 152, TEXT)
    fitted("ASK THE CACHE FIRST", 288, ACCENT)
    ...
    # No cache versus a cache -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "1000 views",
         "1000 database reads", tag="NO CACHE")
    pill(d, 1070, 556, 700, 170, GREEN, "1000 views",
         "10 database reads", tag="CACHE-ASIDE")
    ...
    d.text((W // 2, 744), "Look in the cache first.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "On a miss, remember the answer.",
