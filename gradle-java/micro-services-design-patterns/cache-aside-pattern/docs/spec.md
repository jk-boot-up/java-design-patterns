# Cache-Aside Pattern — Project Specification
...
[`cache-aside-pattern-explained.md`](cache-aside-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.cacheaside`:
...
1. **Reads are counted,** never timed.
2. **Expiry uses a clock the demo controls.**
3. **The stampede is deterministic:** every caller has missed before any read completes, and the counts are 50 and 1.
4. **A forgotten invalidation is asserted to serve stale data.**
...
- `./gradlew build` passes. 7 test methods across `CacheAsideTest`, `DemoRunsTest`.
...
| `cache-aside-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/cache-aside-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Cache-Aside pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Rate Limiter.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py cache-aside`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py cache-aside
python3 ../../docs/make_specs.py cache-aside
```
