#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the CQRS video."""
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson. Down the left is the design being
    # replaced, and under it the fix everybody reaches for first -- the
    # viewer has to have taken the cache seriously before the right-hand
    # column means anything. On the right is the split, and the one fact
    # that makes it a pattern: the copy is told.
    box(d, 70, 232, 560, 120, "ComposingOrderHistory",
        "«done properly»   asks Orders, asks Catalog", MUTED,
        label_size=30, sub_size=24)

    arrow(d, 200, 356, 200, 430, RED, 5)
    d.text((250, 370), "on every single view",
           font=f(MONO_B, 26), fill=RED, anchor="la")
    d.text((250, 402), "90ms and 2 service calls, for a page that did not change",
           font=f(SANS, 21), fill=MUTED, anchor="la")

    box(d, 70, 434, 560, 130, "CachedOrderHistory",
        "the tempting fix  ·  corrected by a timer, and nothing else", AMBER,
        label_size=30, sub_size=22)

    d.text((80, 590), "no arrow reaches it from the events",
           font=f(SANS_B, 26), fill=RED, anchor="la")
    d.text((80, 622), "which is why a rename cannot invalidate it",
           font=f(SANS, 22), fill=MUTED, anchor="la")

    # Bottom left: the evidence. Both designs return the same page, so the
    # only way to compare them is to count calls and read the clock.
    box(d, 70, 680, 560, 110, "CallLog",
        "the timeline, and the bill  ·  SimulatedClock", MUTED,
        label_size=28, sub_size=22)

    d.text((1465, 372), "the write side announces what it did, and the copy listens",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "a read costs one lookup and calls nobody at all",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # The split. StockLedger is green because it is where a sale is decided.
    # OrderHistoryReadModel is cyan because it is the copy -- fast, useful,
    # and never allowed to decide anything.
    work = [
        ("OrderWriteService → StockLedger", "reserves first  ·  where a sale is decided", GREEN),
        ("EventBus", "OrderPlaced  ·  ProductRenamed  ·  StockChanged", MUTED),
        ("OrderHistoryReadModel", "rows kept ready  ·  5ms  ·  0 service calls", CYAN),
        ("rebuildFrom(history)", "read it as: a read model is throwaway", AMBER),
    ]
    # A spine on the right only. It must not touch the left-hand column: the
    # four boxes below are the replacement design, not consequences of the
    # cache going stale above them.
    ...
    d.text((W // 2, 960),
           "Show a read model's number.  Never decide a sale with it  —  "
           "and nothing in the compiler will stop you.",
...
def kind_poster(scene, img, d):
    ...
    # "CQRS" is four letters, so it can be set enormous — which is exactly
    # what a thumbnail needs. The accent line underneath spells the acronym
    # out, because a viewer who does not already know what CQRS stands for is
    # precisely the viewer this video is for.
    d.text((110, 146), "CQRS", font=f(SANS_B, 180), fill=TEXT)
    d.text((110, 348), "COMMAND  /  QUERY", font=f(SANS_B, 72), fill=ACCENT)

    d.rectangle([110, 462, 620, 472], fill=RULE)

    # One box around the whole idea — the two ways of getting the same page,
    # and the sentence that explains the difference — so the tagline reads as
    # part of the pattern rather than as a caption floating under it.
    d.rounded_rectangle([110, 516, W - 110, 866], radius=26,
                        outline=ACCENT, width=4)

    # Both numbers are real: act one and act two of the demo, for a page that
    # comes out identical either way.
    pill(d, 150, 556, 700, 170, RED, 'compose every view',
         "90ms  ·  2 service calls  ·  every single time", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'a copy that is told',
         "5ms  ·  0 service calls  ·  corrected by events", tag="AFTER")
    ...
    d.text((W // 2, 750), "A cache is a copy that cannot know it is wrong. A read model is told.",
           font=f(SANS_B, 36), fill=TEXT, anchor="ma")
    d.text((W // 2, 800), "The price is a window where it is behind — and one rule.",
