# CQRS Pattern — Project Specification
...
[`cqrs-pattern-explained.md`](cqrs-pattern-explained.md); the problem it
...
- Not event sourcing. Events update a projection here; the write side is still the system of record.
- Not caching. A cache is implemented alongside so the difference can be shown rather than asserted.
- Not a recommendation. For a page nobody looks at, composing on demand is the right answer and is always up to the second.
...
18 production classes under `com.jk.explore.cqrs`:
...
1. **The read is one lookup.** Three views cost 15ms and zero calls to other services, against 270ms and six calls for composition.
2. **The work moved rather than vanished**, and the demo says so: Catalog is called once, when the order is placed.
3. **The stale window is shown and it closes by itself.** `EventBus.holdEvents()` lets a test assert a paid, final order that the customer's own history page does not yet show — with no polling, retrying or timer involved in fixing it.
4. **The cache comparison is code.** `CachedOrderHistory` passes all its tests, and `itServesAPageItKnowsNothingAbout` plus `thereIsNoFreeSetting` pin the difference: a read model is corrected by the event that made it wrong, a cache is wrong for however long its timer says.
5. **The write side decides the sale.** `StockLedger` checks and decrements in one step, so two shoppers cannot both take the last kettle; a test shows the read model still offering it and the ledger refusing.
6. **The projection can be rebuilt.** `rebuildFrom` discards and replays, because a read model that cannot be rebuilt is a second copy of the truth rather than a projection.
7. **Reads survive an outage.** `readsSurviveAnOutage` answers while Catalog is down, since nobody is called at read time.
...
- `./gradlew build` passes. 18 test methods across `CachedOrderHistoryTest`, `CqrsTest`, `DemoRunsTest`.
...
| `cqrs-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/cqrs-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the CQRS pattern in
...
| Runtime | ~17:26 over 16 scenes, 257 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"CQRS in Java - Order History Without the Joins"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Saga.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py cqrs`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py cqrs
python3 ../../docs/make_specs.py cqrs
```
