...
#   database-per-service-pattern-explained.m4a   audio-only version (podcast / revision)
...
OUT_VIDEO=database-per-service-pattern-explained.mp4
OUT_AUDIO=database-per-service-pattern-explained.m4a
OUT_SUBS=database-per-service-pattern-explained.srt
