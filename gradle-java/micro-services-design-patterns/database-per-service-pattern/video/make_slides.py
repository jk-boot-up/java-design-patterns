#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Database per Service video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Database per Service  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # replaced -- one schema, one query -- because the viewer has to have
    # appreciated it before the right-hand column means anything. On the
    # right is the split, and the single fact that makes it a pattern: every
    # database method takes the name of whoever is asking.
    box(d, 70, 232, 560, 130, "SharedSchema",
        "«the way it usually starts»   orders + products", MUTED,
        label_size=36, sub_size=24)

    arrow(d, 200, 366, 200, 448, RED, 5)
    d.text((250, 386), "one team renames a column",
           font=f(MONO_B, 26), fill=RED, anchor="la")
    d.text((250, 418), "their migration is correct, and their tests are green",
           font=f(SANS, 22), fill=MUTED, anchor="la")

    box(d, 70, 452, 560, 140, "ColumnNotFoundException",
        "in somebody else's repository  ·  not a bug", RED,
    ...
    box(d, 70, 660, 560, 110, "CallLog",
        "the timeline, and the bill  ·  SimulatedClock", MUTED,
        label_size=28, sub_size=22)

    d.text((1465, 372), "each service owns its data, and nobody else may read it",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "every database method takes the name of whoever is asking",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # The split. OrderHistoryPage is cyan because it is the price made into a
    # class: it is what the join turned into. NotYourDataException is the
    # mechanism, and it is amber rather than red because refusing is correct.
    work = [
        ("OrderHistoryPage", "ask Orders  ·  ask Catalog once  ·  stitch", CYAN),
        ("OrderService → OrderDatabase", "reads as \"Orders\"  ·  owns the order rows", GREEN),
        ("CatalogService → CatalogDatabase", "reads as \"Catalog\"  ·  owns the product rows", GREEN),
        ("NotYourDataException", "read it as: the database refused", AMBER),
    ]
    # A spine on the right only. It must not touch the exception box on the
    # left: the four boxes below are the replacement design, not consequences
    # of the failure above them.
    ...
    d.text((W // 2, 960),
           "In a real shop nothing throws that exception.  "
           "The rule is database credentials, not a comment asking people not to.",
...
def kind_poster(scene, img, d):
    ...
    # The name is two words and neither is short, so it is set on two lines
    # and the accent line carries the second half of the name rather than a
    # separate slogan. At the ~360 px wide thumbnail YouTube serves in search
    # results, "DATABASE" is the word that has to be readable.
    d.text((110, 152), "DATABASE", font=f(SANS_B, 122), fill=TEXT)
    d.text((110, 296), "PER SERVICE", font=f(SANS_B, 106), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, 'one schema  ·  1 query',
         "they rename a column  —  and your page dies", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'two databases  ·  2 calls',
         "they rename a column  —  and nothing happens", tag="AFTER")
    ...
    d.text((W // 2, 750), "A correct migration, by the team that owns the column, breaks somebody else.",
           font=f(SANS_B, 36), fill=TEXT, anchor="ma")
    d.text((W // 2, 800), "The fix costs you the join, and the foreign key.",
