# Database per Service Pattern — Project Specification
...
[`database-per-service-pattern-explained.md`](database-per-service-pattern-explained.md); the problem it
...
- Not a claim that splitting is faster. The split page in this project is measurably slower than the join it replaces.
- Not a recommendation to split. If the two teams are the same three people, the shared schema wins, and the notes say so.
- Not an ORM, a migration tool or a real permissions system. `NotYourDataException` stands in for a database refusing credentials it was never given.
...
13 production classes under `com.jk.explore.databaseperservice`:
...
1. **The shared schema is shown working first**, in one round trip, because the rest of the category is the story of giving that up.
2. **The cross-team break is a passing test.** Every test in `SharedSchemaTest` passes, including the one where a correct migration breaks another team's page.
3. **Ownership is enforced.** Reading Catalog's data through Orders throws `NotYourDataException`, and the notes explain that in production the refusal comes from credentials rather than from Java.
4. **The rename is a non-event after the split**, asserted against an unchanged page.
5. **The batch call is deliberate.** `CatalogService.namesFor` takes a list, and the notes state that one call per sku turns a fifty-row page into fifty calls.
6. **The lost foreign key is demonstrated.** Catalog deletes a product an order refers to, nothing prevents it, and the page renders `(no longer in the catalogue)` rather than crashing — with the notes saying that a rule which was impossible to break is now merely impolite to break.
...
- `./gradlew build` passes. 16 test methods across `DatabasePerServiceTest`, `DemoRunsTest`, `SharedSchemaTest`.
...
| `database-per-service-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/database-per-service-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Database per Service pattern in
...
| Runtime | ~17:26 over 16 scenes, 260 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Database per Service in Java - Losing the Join"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: API Composition.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py database-per-service`. They are the one part of
...
integrated, -3.95 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py database-per-service
python3 ../../docs/make_specs.py database-per-service
```
