# API Gateway with Spring Cloud Gateway Pattern — Project Specification
...
[`api-gateway-with-spring-cloud-gateway-pattern-explained.md`](api-gateway-with-spring-cloud-gateway-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.apigatewaysc`:
...
1. **All traffic is real HTTP** over local sockets.
2. **A rejected request is asserted** to have reached no service.
3. **The slow service is held at a gate,** and only the 504 is asserted.
...
- `./gradlew build` passes. 6 test methods across `DemoRunsTest`, `GatewayTest`.
...
| `api-gateway-with-spring-cloud-gateway-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/api-gateway-with-spring-cloud-gateway-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the API Gateway with Spring Cloud Gateway pattern in
...
| Runtime | ~5:20 over 16 scenes, 78 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Load Balancing with Spring Cloud LoadBalancer.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py api-gateway-with-spring-cloud-gateway`. They are the one part of
...
Last verified: all eleven items pass. The delivered MP4 measures -16.04 LUFS
...
```bash
python3 ../../docs/make_youtube_docs.py api-gateway-with-spring-cloud-gateway
python3 ../../docs/make_specs.py api-gateway-with-spring-cloud-gateway
```
