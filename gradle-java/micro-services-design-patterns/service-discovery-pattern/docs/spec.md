# Service Registry and Discovery Pattern — Project Specification
...
[`service-discovery-pattern-explained.md`](service-discovery-pattern-explained.md); the problem it
...
- Not Eureka, Consul or DNS. `ServiceRegistry` is a map with leases; the point is the contract those products implement, not their features.
- Not load balancing. Choosing *which* healthy instance to call is the next pattern; this one is about getting a list at all.
- Not health checking as a subsystem. A lease that expires when heartbeats stop is the whole health model here, deliberately.
- Not an argument that discovery removes the outage. It bounds the window and makes the bound a number you choose.
...
11 production classes under `com.jk.explore.servicediscovery`:
...
1. **A new instance is used without anybody deploying a caller.** Registering a fourth instance is visible to the very next lookup.
2. **The hardcoded client breaks on the same event**, and its test says so by passing — it fails when its one address is restarted.
3. **A crashed instance is still on the list.** A test asserts the registry reports an instance that is not answering, because a crash cannot deregister.
4. **The caller survives a stale address.** It tries the next instance and gives up only when the list is exhausted, logging `STALE` for the dead one.
5. **The lease expires on the clock.** With a 3000ms lease and heartbeats stopped, the instance is listed at 2000ms and gone at 4000ms; the simulated clock makes that assertion exact.
6. **The stale window is named as the cost**, in the demo output and the notes, together with why a shorter lease only trades it for traffic.
...
- `./gradlew build` passes. 19 test methods across `DemoRunsTest`, `HardcodedPricingClientTest`, `ServiceRegistryTest`.
...
| `service-discovery-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/service-discovery-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Service Registry and Discovery pattern in
...
| Runtime | ~17:39 over 16 scenes, 255 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Service Discovery in Java - Finding a Live Instance"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Client-Side Load Balancing.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py service-discovery`. They are the one part of
...
integrated, -3.92 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py service-discovery
python3 ../../docs/make_specs.py service-discovery
```
