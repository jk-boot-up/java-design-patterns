#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Service Discovery video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Service Discovery  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson: the arrows into the registry all come
    # *from* the instances on the right, and the client only ever asks. Nothing
    # in the picture polls anybody to find out whether they are alive.
    box(d, 70, 232, 560, 130, "DiscoveringPricingClient",
        "«caller»   asks before every single call", CLIENT,
        label_size=34, sub_size=24)

    arrow(d, 200, 366, 200, 448, CLIENT, 5)
    d.text((250, 386), 'instances("Pricing")',
           font=f(MONO_B, 26), fill=CLIENT, anchor="la")
    d.text((250, 418), "asked again on the next call, and the next",
           font=f(SANS, 22), fill=MUTED, anchor="la")

    box(d, 70, 452, 560, 140, "ServiceRegistry",
        "«the list»  one lease per instance, 3000ms each", AMBER,
        label_size=38, sub_size=24)

    arrow(d, 200, 596, 200, 636, GREEN, 4)
    box(d, 70, 640, 560, 110, "Lease",
        "«record»  instance + lastHeartbeatAt", GREEN,
        label_size=32, sub_size=22)

    d.text((1465, 372), "register  ·  heartbeat  ·  deregister",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "the instances write the list themselves",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # Pricing, running four times. The crashed one is drawn in rose because it
    # is the only interesting instance: still on the list, and not answering.
    instances = [
        ("pricing-1", "deregistered  —  off the list at once", MUTED),
        ("pricing-2", "crashed  —  still listed for 3000ms", ROSE),
        ("pricing-3", "running  ·  heartbeating", CYAN),
        ("pricing-4", "started this morning  ·  already usable", CYAN),
    ]
    # The spine runs into the registry, because registration is something the
    # instances do to it rather than something it does to them.
    d.line([(630, 522), (1084, 522)], fill=LINE, width=3)
    d.line([(1084, 522), (1084, 848)], fill=LINE, width=3)

    y = 452
    for label, sub, colour in instances:
    ...
    box(d, 70, 790, 560, 110, "HardcodedPricingClient",
        "«the trap»  one address, and no way to ask", RED,
        label_size=30, sub_size=22)

    d.text((W // 2, 960),
           "The registry is confidently wrong for a few seconds after a "
           "crash.  So the caller tries the next name.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 152), "SERVICE", font=f(SANS_B, 122), fill=TEXT)
    d.text((110, 288), "DISCOVERY", font=f(SANS_B, 122), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, 'pinned = "pricing-1"',
         "one routine deploy  —  and checkout is down", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'registry.instances(x)',
         "deploy, scale up, crash  —  all survivable", tag="AFTER")
    ...
    d.text((W // 2, 744), "A written-down address is an outage with a date on it.",
           font=f(SANS_B, 46), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Ask where it is  —  and try the next name.",
