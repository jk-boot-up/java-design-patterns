#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Scatter-Gather video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Scatter-Gather  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("SCATTER-GATHER", 152, TEXT)
    fitted("ASK ALL, WAIT FOR A DEADLINE", 288, ACCENT)
    ...
    # In turn versus at once with a deadline -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "asked in turn",
         "the page waits 1300 ms", tag="SLOW")
    pill(d, 1070, 556, 700, 170, GREEN, "asked at once, 500 ms deadline",
         "3 of 4 quotes, best shown", tag="GATHERED")
    ...
    d.text((W // 2, 744), "Ask everyone at once.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Do not wait for the slowest.",
