# Scatter-Gather Pattern — Project Specification
...
[`scatter-gather-pattern-explained.md`](scatter-gather-pattern-explained.md); the problem it
...
6 production classes under `com.jk.explore.scattergather`:
...
1. **Latency is arithmetic,** never a sleep.
2. **Four suppliers are asked at the same moment,** asserted by holding them at a gate, over repeated runs.
3. **A slow supplier is left out and named,** and a failing one is named with its reason.
4. **When nobody answers there is no best price,** not an error.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `ScatterGatherTest`.
...
| `scatter-gather-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/scatter-gather-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Scatter-Gather pattern in
...
| Runtime | ~4:51 over 14 scenes, 72 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Double-Checked Locking.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py scatter-gather`. They are the one part of
...
integrated, -3.80 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py scatter-gather
python3 ../../docs/make_specs.py scatter-gather
```
