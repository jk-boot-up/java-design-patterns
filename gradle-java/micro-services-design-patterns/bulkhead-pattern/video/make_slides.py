#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Bulkhead video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Bulkhead  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson: on the left the whole of the mechanism,
    # which is disappointingly small, and on the right the work, labelled by
    # what it MEANS to the business rather than by what it extends. "Must
    # never be starved" and "nobody is waiting for it" are the most important
    # facts on the slide, and neither appears anywhere in the type system,
    # which is why they are drawn here.
    box(d, 70, 232, 560, 130, "Bulkhead(\"feed\", 2, 2)",
        "\u00aba decision, not an algorithm\u00bb   name, threads, queue", CLIENT,
        label_size=34, sub_size=24)

    arrow(d, 200, 366, 200, 448, GREEN, 5)
    d.text((250, 386), "is barely more than",
           font=f(MONO_B, 26), fill=GREEN, anchor="la")
    d.text((250, 418), "no strategy, nothing adaptive, nothing to tune at runtime",
           font=f(SANS, 22), fill=MUTED, anchor="la")

    box(d, 70, 452, 560, 140, "ThreadPoolExecutor",
        "2 fixed threads  \u00b7  ArrayBlockingQueue(2)", AMBER,
        label_size=36, sub_size=24)

    arrow(d, 200, 596, 200, 636, AMBER, 4)
    box(d, 70, 640, 560, 110, "BulkheadFullException",
        "thrown in 0ms  \u00b7  the caller still has time to act", AMBER,
        label_size=28, sub_size=22)

    d.text((1465, 372), "two kinds of work  \u00b7  one pot of threads",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "two jobs, and only one may ever be allowed to lose",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # The work, plus the arrangement everybody starts with. Checkout is cyan
    # because it is the one that must never be starved; the shared pool is
    # muted because it is the design being replaced, not a third kind of work.
    work = [
        ("Checkout", "MUST NEVER BE STARVED  \u00b7  needs one thing: a thread", CYAN),
        ("SupplierFeed", "nobody is waiting  \u00b7  holds a thread while it waits", GREEN),
        ("Gate", "the slow partner, without sleeping  \u00b7  CountDownLatch", GREEN),
        ("one shared pool of 4", "faster on a good day  \u00b7  loses the sale on a bad one", MUTED),
    ]
    # The spine leaves the ThreadPoolExecutor box, because which worker a job
    # lands on is decided there and nowhere else.
    # It stops at Gate on purpose. The shared pool is not a third kind of work,
    # and in that version nothing is submitted to a bulkhead at all -- it is
    # the design being replaced, so it hangs off nothing.
    ...
    # On screen because this is the only project in the category with real
    # threads, so it is the only one where the evidence itself has to survive
    # four workers writing to it at once.
    box(d, 70, 790, 560, 110, "JobLog",
        "\u00abCopyOnWriteArrayList\u00bb  who ran, on which worker", ROSE,
        label_size=28, sub_size=22)

    d.text((W // 2, 960),
           "Checkout and SupplierFeed never reference each other.  "
           "The coupling is the pool, which neither of them names.",
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 152), "BULKHEAD", font=f(SANS_B, 122), fill=TEXT)
    # The pattern name is one word, so the second line carries the thesis
    # instead of the rest of the name. It is set a little smaller than the
    # name so the word "BULKHEAD" still wins at thumbnail size.
    d.text((110, 296), "STOP SHARING THE POOL", font=f(SANS_B, 88), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, 'one pool  ·  4 threads',
         "the import takes all four  —  and the sale is lost", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'feed: 2  ·  checkout: 2',
         "the import is still stuck  —  and the shop is still selling", tag="AFTER")
    ...
    d.text((W // 2, 744), "A background job nobody was waiting for stops the shop selling.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "The wall is free to build, and paid for in idle threads.",
