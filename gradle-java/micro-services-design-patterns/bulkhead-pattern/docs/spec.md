# Bulkhead Pattern — Project Specification
...
[`bulkhead-pattern-explained.md`](bulkhead-pattern-explained.md); the problem it
...
- Not a thread-pool tutorial, and not a performance exercise — the partitioned version is deliberately slower overall.
- Not a circuit breaker. Nothing here is broken; the resource is simply taken.
- Not a claim that bulkheads are free. Idle capacity is the price, and the demo prints idle threads next to queued jobs.
...
7 production classes under `com.jk.explore.bulkhead`:
...
1. **The shared pool starves checkout**, proven by a bounded `Future.get` timing out after 250ms — the shopper's patience — while a second test shows the same checkout succeeding the instant a thread frees.
2. **The partition holds with the feed just as stuck.** A test asserts the feed's threads are all busy, so isolation is demonstrably the reason checkout completed and not luck; another puts twenty sales through during the jam.
3. **The queue is bounded and a full bulkhead refuses at once**, in single-digit milliseconds, so the caller can shed or degrade.
4. **The cost is asserted, not admitted.** A test shows the shared pool running all four batches at once where the partitioned pools leave two threads idle beside two queued jobs.
5. **No test sleeps.** This is the category's only real-threads project, and it uses a closed `Gate` and bounded waits rather than `Thread.sleep`, with `JobLog` on a `CopyOnWriteArrayList` because worker threads write to it concurrently.
...
- `./gradlew build` passes. 12 test methods across `BulkheadTest`, `DemoRunsTest`.
...
| `bulkhead-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/bulkhead-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Bulkhead pattern in
...
| Runtime | ~17:18 over 16 scenes, 256 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Bulkhead Pattern in Java - Isolating a Slow Job"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Database per Service.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py bulkhead`. They are the one part of
...
integrated, -3.41 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py bulkhead
python3 ../../docs/make_specs.py bulkhead
```
