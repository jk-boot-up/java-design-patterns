# Idempotent Consumer Pattern — Project Specification
...
[`idempotent-consumer-pattern-explained.md`](idempotent-consumer-pattern-explained.md); the problem it
...
- Not a claim that the duplicate is somebody's fault. A sender whose acknowledgement is lost must choose between a duplicate and a lost message, and every production system chooses the duplicate.
- Not exactly-once delivery. The delivery is still at-least-once; only the *effect* happens once.
- Not a dedupe library. The pattern is a transaction, and the project's point is that a `HashSet` is not one.
...
15 production classes under `com.jk.explore.idempotentconsumer`:
...
1. **Exactly once out of at-least-once.** Two deliveries of the same message produce one confirmation and one stored id.
2. **The id and the effect share one commit**, logged as one confirmation and one handled id together, so a crash before the commit writes neither and the redelivery handles the message properly.
3. **Both naive failures are shown, and both tests pass.** A restart empties the in-memory set and produces two confirmations; a crash after the work and before recording the id does the same.
4. **The constraint is stated out loud.** The effect is a database row, which is the only reason it can share the transaction; an external effect sends you back to the outbox, which is why these two patterns are taught together.
5. **Natural idempotence comes first.** `ShipmentStatusConsumer` has no store, no transaction and no expiry, because setting a status twice sets the same status.
6. **Rewriting beats storing.** `LoyaltyPointsConsumer.handle` doubles a running total to 140 points, and `awardForOrder` sets 70 per order and stays at 70 when handled twice.
7. **The expiry window is a guess, and it costs.** With a thirty-second memory, a duplicate a minute later is handled as new — a passing test and a demo line, not a caveat — and the notes add that the message must carry a stable id at all.
...
- `./gradlew build` passes. 19 test methods across `DemoRunsTest`, `IdempotentConsumerTest`, `NaiveNotificationConsumerTest`.
...
| `idempotent-consumer-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/idempotent-consumer-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Idempotent Consumer pattern in
...
| Runtime | ~10:52 over 16 scenes, 166 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Idempotent Consumer in Java - The Duplicate Message"*,
   51 characters. The suffix after the dash names the worked e-commerce scenario, so the title says what the viewer will actually watch rather than only which pattern it is about.
...
> `python3 ../../docs/make_youtube_docs.py idempotent-consumer`. They are the one part of
...
integrated, -4.12 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py idempotent-consumer
python3 ../../docs/make_specs.py idempotent-consumer
```
