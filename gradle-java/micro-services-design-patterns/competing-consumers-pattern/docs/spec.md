# Competing Consumers Pattern — Project Specification
...
[`competing-consumers-pattern-explained.md`](competing-consumers-pattern-explained.md); the problem it
...
5 production classes under `com.jk.explore.competingconsumers`:
...
1. **Every count is exact,** held by gates, and asserted over repeated runs.
2. **Each message is handled once in the normal case:** 500 and 1000 messages, distinct ids equal total.
3. **Ordering loss is exact:** [2, 3, 1].
4. **A crash after the effect produces a duplicate,** and a remembering consumer does not.
...
- `./gradlew build` passes. 7 test methods across `CompetingConsumersTest`, `DemoRunsTest`.
...
| `competing-consumers-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/competing-consumers-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Competing Consumers pattern in
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Claim Check.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py competing-consumers`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py competing-consumers
python3 ../../docs/make_specs.py competing-consumers
```
