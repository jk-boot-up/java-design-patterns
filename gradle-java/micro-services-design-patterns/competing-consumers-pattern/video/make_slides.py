#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Competing Consumers video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Competing Consumers  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("COMPETING CONSUMERS", 152, TEXT)
    fitted("ONE QUEUE, MANY WORKERS", 288, ACCENT)
    ...
    # One worker versus three, and a duplicate -- real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "crash after charging",
         "charges made: 2", tag="DUPLICATE")
    pill(d, 1070, 556, 700, 170, GREEN, "3 consumers, 6 jobs",
         "3 in progress, 3 waiting", tag="SHARED")
    ...
    d.text((W // 2, 744), "Take from the same queue.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Expect a message twice.",
