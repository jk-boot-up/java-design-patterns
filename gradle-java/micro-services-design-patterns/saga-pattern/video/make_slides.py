#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Saga video."""
...
def kind_diagram(scene, img, d):
    ...
    # The layout carries the lesson. Down the left is what everybody writes
    # first and what it leaves behind, so the viewer has felt the problem
    # before the right-hand column means anything. On the right is the
    # mechanism, and the three prices printed underneath it.
    box(d, 70, 232, 560, 120, "NaiveCheckoutService",
        "four calls in a try block  ·  a catch that logs", MUTED,
        label_size=30, sub_size=24)

    arrow(d, 200, 356, 200, 430, RED, 5)
    d.text((250, 370), "the courier refuses",
           font=f(MONO_B, 26), fill=RED, anchor="la")
    d.text((250, 402), "and the catch writes a log line nobody reads",
           font=f(SANS, 21), fill=MUTED, anchor="la")

    box(d, 70, 434, 560, 130, "£70.95 taken, 0 parcels",
        "stock reserved  ·  order CONFIRMED  ·  nothing threw", AMBER,
        label_size=30, sub_size=22)

    d.text((80, 590), "@Transactional would not have helped",
           font=f(SANS_B, 26), fill=RED, anchor="la")
    d.text((80, 622), "it covers this service's own database, and nothing else",
           font=f(SANS, 22), fill=MUTED, anchor="la")

    # Bottom left: the evidence. Both designs call the same five services, so
    # the only way to compare them is to watch who was called and when.
    box(d, 70, 680, 560, 110, "CallLog",
        "the timeline, in reverse  ·  SimulatedClock", MUTED,
        label_size=28, sub_size=22)

    d.text((1465, 372), "each step commits on its own, and carries its own undo",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")
    d.text((1465, 408), "forward keeping a list  ·  backward undoing it",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    # The mechanism. SagaStep is green because it is the whole idea.
    # SagaOutcome is cyan because the third value on it is the honest one.
    work = [
        ("SagaStep", "execute  ·  compensate  ·  canBeCompensated", GREEN),
        ("SagaOrchestrator", "run(context)  ·  never throws  ·  ~20 lines", MUTED),
        ("SagaOutcome", "COMPLETED  ·  COMPENSATED  ·  NEEDS_HUMAN_HELP", CYAN),
        ("EmailService", "no compensate exists, so this step goes last", AMBER),
    ]
    ...
    d.text((W // 2, 960),
           "A refund is a new fact, not an erasure.  The undo can be refused  —  "
           "and some steps have no undo at all.",
...
def kind_poster(scene, img, d):
    ...
    # "SAGA" is four letters, so it can be set enormous — which is exactly
    # what a thumbnail needs. The accent line underneath says what the word
    # means, because a viewer who has only ever heard it used as jargon is
    # precisely the viewer this video is for.
    d.text((110, 146), "SAGA", font=f(SANS_B, 180), fill=TEXT)
    d.text((110, 348), "UNDO  ACROSS  SERVICES", font=f(SANS_B, 66), fill=ACCENT)

    d.rectangle([110, 462, 620, 472], fill=RULE)

    # One box around the whole idea — the two ways of handling the same
    # failure, and the sentence that explains the difference — so the tagline
    # reads as part of the pattern rather than as a caption floating under it.
    d.rounded_rectangle([110, 516, W - 110, 866], radius=26,
                        outline=ACCENT, width=4)

    # Both outcomes are real: act five and act two of the demo, from the same
    # refusal by the same courier.
    pill(d, 150, 556, 700, 170, RED, 'four calls in a try block',
         "£70.95 taken  ·  0 parcels  ·  nothing threw", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, 'five steps, five undos',
         "£0.00 held  ·  20 of 20 back  ·  and it says so", tag="AFTER")
    ...
    d.text((W // 2, 750), "You cannot roll back across five services. You can only undo it yourself.",
           font=f(SANS_B, 36), fill=TEXT, anchor="ma")
    d.text((W // 2, 800), "And the undo leaves a trace, can be refused, or may not exist.",
