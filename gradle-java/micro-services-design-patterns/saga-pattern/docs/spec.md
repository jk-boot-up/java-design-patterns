# Saga Pattern — Project Specification
...
[`saga-pattern-explained.md`](saga-pattern-explained.md); the problem it
...
- Not distributed two-phase commit. There is no point in the run where the five services hold their breath, and the project says so rather than pretending otherwise.
- Not rollback. Compensation adds a new fact — a refund, a cancellation — and never erases the old one.
- Not choreography. This is an orchestrated saga; the alternative is described, with the reason for the choice.
- Not a message broker. The steps are method calls over a simulated network so the ordering is exact.
...
20 production classes under `com.jk.explore.saga`:
...
1. **Compensation runs in reverse**, asserted as an exact list — create order, take payment, reserve stock — because later steps depend on earlier ones and must come apart in the opposite order.
2. **A refund is a new fact.** The payment ledger holds two entries, `CHARGE £70.95` and `REFUND -£70.95`, and nets to zero; the test asserts two entries rather than an empty ledger.
3. **An early failure is cheap.** A declined card costs one released reservation, which is why the steps most likely to fail go first.
4. **A failed compensation is a third outcome.** `SagaOutcome` has `NEEDS_HUMAN_HELP`, the unwinding continues past the failure, and the outcome names the step that could not be undone.
5. **The non-compensatable step is shown both ways.** The confirmation email declares `canBeCompensated() == false` and goes last, and `PlaceOrderSteps.withTheEmailInTheWrongPlace` exists so that a test can show the promise the shop then has to take back.
6. **The naive version passes every test.** `NaiveCheckoutServiceTest` is green while the money stays taken, the stock stays reserved and no shipment exists — and the notes state why `@Transactional` on that method protects nothing outside its own database.
...
- `./gradlew build` passes. 24 test methods across `DemoRunsTest`, `MoneyTest`, `NaiveCheckoutServiceTest`, `SagaTest`.
...
| `saga-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/saga-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Saga pattern in
...
| Runtime | ~18:21 over 16 scenes, 261 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Saga Pattern in Java - Placing an Order, Safely"*,
   47 characters. The suffix after the dash names the worked e-commerce scenario, so the title says what the viewer will actually watch rather than only which pattern it is about.
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Transactional Outbox.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py saga`. They are the one part of
...
integrated, -3.63 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py saga
python3 ../../docs/make_specs.py saga
```
