# Template Method with Spring Pattern — Project Specification
...
[`template-method-with-spring-pattern-explained.md`](template-method-with-spring-pattern-explained.md); the problem it
...
4 production classes under `com.jk.explore.templatespring`:
...
1. **Connections are counted** through Hikari's pool MXBean.
2. **The leak is asserted** so a fix to the by-hand method fails the test.
3. **Rollback is asserted** by counting rows and stock.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `TemplateBeanTest`.
...
| `template-method-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/template-method-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Template Method with Spring pattern in
...
| Runtime | ~5:21 over 16 scenes, 76 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Chain of Responsibility with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py template-method-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py template-method-with-spring
python3 ../../docs/make_specs.py template-method-with-spring
```
