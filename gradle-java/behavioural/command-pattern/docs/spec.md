# Command Pattern — Project Specification
...
[`command-pattern-explained.md`](command-pattern-explained.md); the problem it
...
- Not Memento. Undo here is per-edit inverses, not snapshots of the whole cart; the notes and the video set the two against each other explicitly rather than pretending Command is the only answer to undo.
- Not a task queue. Commands run synchronously, in-process, on one cart. Queuing and scheduling are named as things the pattern makes possible, and are not implemented.
- Not thread-safety. `CartHistory` is single-threaded by design; undo ordering is a contract, and concurrent editing of one cart is out of scope.
- Not a lambda tutorial. `execute` alone would be a `Runnable`; the project exists to show what the *second* method needs, which is state that survives between two calls.
...
12 production classes under `com.jk.explore.command`:
...
1. **Undo state is captured inside `execute`, from the receiver.** No command may work out its inverse in its constructor, because an earlier undo can invalidate that guess before the command runs. A field the constructor does not set is the visible sign of it.
2. **Undo restores what was there, not the opposite of what was asked.** Undoing a merged add returns the previous quantity; undoing a removal restores the line's *position*; undoing a coupon restores the coupon it replaced. Each has a test written against a cart that was not empty.
3. **`CartHistory` names no edit.** Searching the class for `coupon`, `quantity` or `sku` must find nothing, and a command defined outside the shipped source must execute and undo unchanged — a test declares its own gift-wrapping `CartCommand` to prove it.
4. **A command that throws is never pushed onto the undo stack**, because reversing a half-applied edit applies the inverse of something that never fully happened.
5. **A new command clears the redo stack**, since taking a different branch makes the old future unreachable — the behaviour of every editor the learner has used.
...
- `./gradlew build` passes. 37 test methods across `CartCommandTest`, `CartHistoryTest`, `NaiveCartEditorTest`.
...
| `command-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/command-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Command pattern in
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Command Pattern in Java - Undoable Cart Edits"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Template Method.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py command`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py command
python3 ../../docs/make_specs.py command
```
