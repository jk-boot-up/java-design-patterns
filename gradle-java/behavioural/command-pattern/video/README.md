# Command Pattern — Teaching Video

A narrated, slide-based video that teaches the Command pattern using this
...
| File | What it is |
| --- | --- |
| `command-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `command-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `command-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Scenario | Four cart edits, and the history each one leaves behind |
| 3 | The Obvious First Move | Change the cart, write a note beside it |
| 4 | The Naive Approach | `NaiveCartEditor` — the note records the request, not the cart |
| 5 | Why That Hurts | The growing `switch` is the complaint; the lost discount is the bug |
| 6 | The Pattern | The formal definition, and what a method call cannot do |
| 7 | The Order Slip | How to remember it forever — slip, kitchen, waiter |
| 8 | The Roles | Command, concrete commands, receiver, invoker — and the captured fields |
| 9 | The Command | `CartCommand` — three methods, and why the cart is a parameter |
| 10 | A Concrete Command | `AddItemCommand` — capture inside `execute`, never in the constructor |
| 11 | The Invoker | `CartHistory`: `undone.clear()`, and why a throwing command is not pushed |
| 12 | The Proof | A gift-wrapping command defined entirely inside a test |
| 13 | Running It | The same two edits, undone wrongly and then correctly |
| 14 | Wrap Up | The honest cost, and Command versus Memento |
...
**Upload `command-pattern-explained.mp4`, with `poster.png` as the
...
`ApplyCouponCommand`. Uploading `command-pattern-explained.srt` gives
