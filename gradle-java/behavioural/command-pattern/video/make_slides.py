#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Command pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Command Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("make the action"):
...
def box(d, x, y, w, h, label, sub, colour):
    ...
    fnt = f(SANS_B, 30)
    while fnt.getlength(label) > w - 24 and fnt.size > 16:
        fnt = f(SANS_B, fnt.size - 2)
    d.text((x + w // 2, y + h // 2 - 26), label, font=fnt,
           fill=TEXT, anchor="ma")
...
def kind_diagram(scene, img, d):
    ...
    # The command interface, centred: the only type the invoker depends on.
    d.text((960, 250), "COMMAND", font=f(SANS_B, 24), fill=GREEN, anchor="ma")
    box(d, 700, 290, 520, 100, "CartCommand",
        "«interface»  execute(Cart), undo(Cart)", GREEN)

    # The invoker, left: two stacks of the interface, and nothing else.
    d.text((270, 250), "INVOKER", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    box(d, 80, 290, 380, 100, "CartHistory", "two stacks of them", CLIENT)
    arrow(d, 460, 340, 690, 340, CLIENT, 4)
    d.text((575, 296), "0..*", font=f(SANS_B, 22), fill=CLIENT, anchor="ma")

    # The receiver, right: changed by the commands, aware of none of them.
    d.text((1650, 250), "RECEIVER", font=f(SANS_B, 24), fill=AMBER, anchor="ma")
    box(d, 1440, 290, 420, 100, "Cart", "lines and one coupon", AMBER)
    # Points at the Cart, not away from it: the commands edit the receiver, and
    # the receiver has no reference back.
    arrow(d, 1230, 340, 1435, 340, AMBER, 3)
    d.text((1332, 296), "edits", font=f(SANS_B, 22), fill=AMBER, anchor="ma")

    # Four concrete commands, peers, each implementing the interface. The
    # sub-labels name the field each one captures, because that capture is the
    # part of the pattern people get wrong.
    # Sits above the fan of inheritance arrows, which reach y=500 by x=400.
    d.text((60, 424), "CONCRETE COMMANDS", font=f(SANS_B, 24), fill=ROSE)
    d.text((60, 456), "each captures what its own undo needs",
           font=f(SANS, 22), fill=MUTED)
    labels = [
        ("AddItemCommand", "previousQuantity", 60),
        ("RemoveItemCommand", "removed line + position", 530),
        ("ChangeQuantityCommand", "previousQuantity", 1000),
        ("ApplyCouponCommand", "previousCoupon", 1470),
    ]
    for label, sub, x in labels:
        box(d, x, 530, 390, 110, label, sub, ROSE)
        arrow(d, x + 195, 530, 960, 400, LINE)

    d.text((W // 2, 730),
           "CartHistory has no field called coupon, quantity or SKU.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")
    d.text((W // 2, 786),
           "It pops an object and sends it undo. The command decides what that means.",
           font=f(SANS, 28), fill=MUTED, anchor="ma")

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 660, 858, 600, 110, "NaiveCartEditor",
        "«the trap»  one note, one switch", AMBER)
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "COMMAND", font=f(SANS_B, 128), fill=TEXT)
    ...
    # Undoing a note about the request vs. undoing the edit that actually ran.
    pill(d, 150, 556, 700, 170, RED, "cart.removeLine(sku)",
         "reversing what was asked for", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "command.undo(cart)",
         "reversing what actually happened", tag="AFTER")
    ...
    d.text((W // 2, 758), "Make the action an object. Then you can take it back.",
