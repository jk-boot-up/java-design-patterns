# Interpreter with SpEL Pattern — Project Specification
...
[`interpreter-with-spel-pattern-explained.md`](interpreter-with-spel-pattern-explained.md); the problem it
...
3 production classes under `com.jk.explore.interpreterspel`:
...
1. **Rules are asserted against three known orders.**
2. **Both failure times are asserted** with the exact exception type.
3. **The read-only context is asserted to refuse** static calls and method calls.
...
- `./gradlew build` passes. 8 test methods across `DemoRunsTest`, `SpelInterpreterTest`.
...
| `interpreter-with-spel-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/interpreter-with-spel-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Interpreter with SpEL pattern in
...
| Runtime | ~5:07 over 16 scenes, 72 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Circuit Breaker with Resilience4j.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py interpreter-with-spel`. They are the one part of
...
Last verified: all eleven items pass. The delivered MP4 measures -15.99 LUFS
...
```bash
python3 ../../docs/make_youtube_docs.py interpreter-with-spel
python3 ../../docs/make_specs.py interpreter-with-spel
```
