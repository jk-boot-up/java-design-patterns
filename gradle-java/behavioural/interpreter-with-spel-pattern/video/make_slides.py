#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Interpreter with SpEL video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Interpreter with SpEL  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("INTERPRETER", 152, TEXT)
    fitted("WITH SPEL", 288, ACCENT)
    ...
    # A full context versus a read-only one -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "full context: T(System) runs",
         "a rule reaches the program", tag="OPEN")
    pill(d, 1070, 556, 700, 170, GREEN, "read-only context",
         "T(System) is refused", tag="SAFE")
    ...
    d.text((W // 2, 744), "A ready-made interpreter.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Choose the context.",
