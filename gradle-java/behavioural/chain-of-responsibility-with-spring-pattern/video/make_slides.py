#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Chain of Responsibility with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Chain of Responsibility with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    fitted("CHAIN OF RESPONSIBILITY", 152, TEXT)
    ...
    # Cheap checks first versus the paid one first -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "fraud first",
         "5 paid calls", tag="COSTLY")
    pill(d, 1070, 556, 700, 170, GREEN, "address, stock, fraud",
         "3 paid calls", tag="CHEAP FIRST")
    ...
    d.text((W // 2, 744), "The container sorts the links.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "The order is the cost.",
