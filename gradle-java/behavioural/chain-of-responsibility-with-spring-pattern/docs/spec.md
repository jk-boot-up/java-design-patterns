# Chain of Responsibility with Spring Pattern — Project Specification
...
[`chain-of-responsibility-with-spring-pattern-explained.md`](chain-of-responsibility-with-spring-pattern-explained.md); the problem it
...
10 production classes under `com.jk.explore.chainspring`:
...
1. **Order is asserted** as a list of names.
2. **Cost is counted** as paid calls, never timed.
3. **A throwing link is asserted** as a referral.
...
- `./gradlew build` passes. 8 test methods across `ChainBeanTest`, `DemoRunsTest`.
...
| `chain-of-responsibility-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/chain-of-responsibility-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Chain of Responsibility with Spring pattern in
...
| Runtime | ~4:48 over 16 scenes, 73 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Interpreter with SpEL.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py chain-of-responsibility-with-spring`. They are the one part of
...
integrated, -3.35 dBTP true peak, and both streams start at 0.000.
...
```bash
python3 ../../docs/make_youtube_docs.py chain-of-responsibility-with-spring
python3 ../../docs/make_specs.py chain-of-responsibility-with-spring
```
