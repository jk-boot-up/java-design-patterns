# Strategy with Spring Pattern — Project Specification
...
[`strategy-with-spring-pattern-explained.md`](strategy-with-spring-pattern-explained.md); the problem it
...
11 production classes under `com.jk.explore.strategyspring`:
...
1. **Prices are asserted in pence.**
2. **Startup failures are asserted** by the exception's root cause.
3. **The fifth rule and the ambiguous class are registered by the demo,** so the default run has four rules.
...
- `./gradlew build` passes. 9 test methods across `DemoRunsTest`, `StrategyBeanTest`.
...
| `strategy-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/strategy-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Strategy with Spring pattern in
...
| Runtime | ~4:35 over 16 scenes, 67 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Template Method with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py strategy-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py strategy-with-spring
python3 ../../docs/make_specs.py strategy-with-spring
```
