#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Strategy with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Strategy with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # A map of rules versus an ambiguous injection -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "ShippingCostRule rule",
         "found 4 beans, expected 1", tag="AMBIGUOUS")
    pill(d, 1070, 556, 700, 170, GREEN, "Map of String to rule",
         "all four, keyed by name", tag="COLLECTED")
    ...
    d.text((W // 2, 744), "The container keeps the table.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "You choose the name.",
