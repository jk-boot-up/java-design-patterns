#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Memento video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Memento  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # Originator on the left, caretaker on the right, memento underneath and
    # between them. The layout is the lesson: both of the upper boxes touch the
    # snapshot, and only the left one can see into it.
    box(d, 90, 240, 640, 170, "Basket",
        "«originator»   save(label)  ·  restore(snapshot)", GREEN,
        label_size=34, sub_size=22)
    box(d, 1190, 240, 640, 170, "BasketHistory",
        "«caretaker»   a stack of snapshots, capped at 20", CYAN,
        label_size=34, sub_size=22)

    # The client sits between them at the top, because it drives both: it
    # changes the basket and it presses undo.
    d.text((960, 208), "CLIENT", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    box(d, 790, 240, 340, 110, "BasketUndoDemo",
        "shops, then undoes", CLIENT, label_size=25, sub_size=20)
    arrow_left(d, 786, 296, 736, CLIENT, 3)
    arrow_right(d, 1134, 296, 1184, CLIENT, 3)

    box(d, 630, 560, 660, 200, "BasketSnapshot",
        "«memento»   a sealed copy of the lines and the voucher", AMBER,
        label_size=34, sub_size=22)

    # Two diagonals into the memento, deliberately drawn in the two owners'
    # colours so the asymmetry between them is visible before it is read.
    arrow(d, 410, 418, 740, 552, GREEN, 4)
    d.text((140, 494), "creates them,\nand reads them back",
           font=f(SANS_B, 22), fill=GREEN, anchor="la", spacing=8)
    arrow(d, 1510, 418, 1180, 552, CYAN, 4)
    d.text((1790, 494), "only holds them,\nnever opens one",
           font=f(SANS_B, 22), fill=CYAN, anchor="ra", spacing=8, align="right")

    d.text((960, 776),
           "label() is public   ·   lines() and voucher() are package-private",
           font=f(SANS, 26), fill=MUTED, anchor="ma")

    d.text((W // 2, 828),
           "Two arrows into the envelope.  Only one of them can open it.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 620, 890, 680, 92, "NaiveBasket",
        "«the trap»  savedLines = lines, and no voucher at all", RED,
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 288), "MEMENTO", font=f(SANS_B, 122), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, "saved = lines",
         "undo empties the basket", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "List.copyOf(lines)",
         "undo puts every bit of it back", tag="AFTER")
    ...
    d.text((W // 2, 744), "Hand a friend a sealed envelope.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "They can hold it. They can't open it.",
