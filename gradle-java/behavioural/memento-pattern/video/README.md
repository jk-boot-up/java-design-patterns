# Memento Pattern — Teaching Video

A narrated, slide-based video that teaches the Memento pattern using this
...
| File | What it is |
| --- | --- |
| `memento-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `memento-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `memento-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Scenario | A basket with lines and a voucher, and a ticket that says "add an undo button" |
| 3 | Look Closely at One Line | `savedLines = lines` records where the list is, not what is in it |
| 4 | The Naive Approach | `NaiveBasket` — the aliasing bug, and a voucher that was never saved at all |
| 5 | Why That Hurts | Silent omissions, one chance to forget per field, public fields as the "fix", and the missing inverse |
| 6 | The Pattern | The formal definition, and why "without violating encapsulation" is the half people skip |
| 7 | An Analogy | The sealed envelope, the friend who holds it, and the label on the outside |
| 8 | The Roles | `Basket`, `BasketSnapshot`, `BasketHistory` — two arrows in, one of them able to open it |
| 9 | The Memento | `List.copyOf`, and the wide/narrow interface built from access modifiers alone |
| 10 | The Originator | `save` and `restore`, and a caretaker defined by what it cannot do |
| 11 | The Tests | The reflection test on encapsulation, and pinning the naive basket's wrong answer on purpose |
| 12 | Running It | The two baskets side by side: an empty £0 basket, against 4 items, SAVE5 and £65 |
| 13 | Wrap Up | Memento against Command and Prototype, the memory cost, and why the shallow copy is safe |
...
**Upload `memento-pattern-explained.mp4`, with `poster.png` as the
...
`BasketSnapshot`. Uploading `memento-pattern-explained.srt` gives
