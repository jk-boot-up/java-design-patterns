#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Visitor video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Visitor  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # Two hierarchies, side by side. That is the whole picture: the one on the
    # left stopped growing years ago, the one on the right never will.
    d.text((480, 226), "THE STRUCTURE  ·  3 types, settled",
           font=f(SANS_B, 26), fill=CYAN, anchor="ma")
    d.text((1440, 226), "THE OPERATIONS  ·  6 reports, growing",
           font=f(SANS_B, 26), fill=GREEN, anchor="ma")

    box(d, 90, 280, 780, 100, "CatalogComponent",
        "«interface»   name(),  accept(CatalogVisitor)", CYAN,
        label_size=32, sub_size=22)
    box(d, 1050, 280, 780, 100, "CatalogVisitor",
        "«interface»   visit(Product) / visit(Bundle) / visit(Category)", GREEN,
        label_size=32, sub_size=21)

    # The only arrow between the two sides, and it points the way that
    # surprises people: the structure names the visitor, never the reports.
    arrow_right(d, 878, 330, 1042, ACCENT, 4)
    d.text((960, 396), "accept(v)", font=f(SANS_B, 24), fill=ACCENT, anchor="ma")

    nodes = [
        ("Product", "price, stock, restriction"),
        ("Bundle", "a kit, and its contents"),
        ("Category", "children, and the walk"),
    ]
    d.line([(215, 430), (1745, 430)], fill=LINE, width=0)  # no shared rail
    d.line([(215, 430), (745, 430)], fill=LINE, width=3)
    arrow_up(d, 480, 430, 386, LINE, 3)
    for i, (label, sub) in enumerate(nodes):
        x = 90 + i * 267
        box(d, x, 460, 246, 115, label, sub, CYAN, label_size=28, sub_size=19)
        d.line([(x + 123, 460), (x + 123, 430)], fill=LINE, width=3)

    visitors = [
        ("InventoryValue", "price x stock"),
        ("CategoryCount", "lines per branch"),
        ("CsvExport", "one quoting rule"),
        ("ComplianceAudit", "what cannot fly"),
        ("DispatchTrace", "the walk, printed"),
        ("LowStock", "written this morning"),
    ]
    d.line([(1175, 430), (1705, 430)], fill=LINE, width=3)
    arrow_up(d, 1440, 430, 386, LINE, 3)
    for i, (label, sub) in enumerate(visitors):
        x = 1050 + (i % 3) * 267
        y = 460 if i < 3 else 592
        box(d, x, y, 246, 115, label, sub, GREEN, label_size=24, sub_size=19)
        if i < 3:
            d.line([(x + 123, 460), (x + 123, 430)], fill=LINE, width=3)
        else:
            d.line([(x + 123, 592), (x + 123, 575)], fill=LINE, width=3)

    # The one line every node carries, spelled out, because it is the thing
    # people do not believe until they see how little of it there is.
    d.rounded_rectangle([90, 616, 870, 700], radius=14, fill=(2, 6, 23),
                        outline=LINE, width=2)
    d.text((480, 634), "void accept(CatalogVisitor v) { v.visit(this); }",
           font=f(MONO_B, 24), fill=TEXT, anchor="ma")
    d.text((480, 672), "the same line, in all three — and the last one they need",
           font=f(SANS, 22), fill=MUTED, anchor="ma")

    d.text((W // 2, 762),
           "Adding to the right costs nothing. Adding to the left costs every box on it.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")
    d.text((W // 2, 810),
           "Count your own two numbers before you reach for this.",
           font=f(SANS, 28), fill=MUTED, anchor="ma")

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 620, 856, 680, 100, "NaiveCatalogNode",
        "«the trap»  a report method on every node class", RED)
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 152), "VISITOR", font=f(SANS_B, 122), fill=TEXT)
    d.text((110, 288), "PATTERN", font=f(SANS_B, 122), fill=ACCENT)
    ...
    # A report bolted onto the model vs. a report that arrives as a visitor.
    pill(d, 150, 556, 700, 170, RED, "node.exportCsv()",
         "a method per report, on the model", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "node.accept(report)",
         "a class per report, model untouched", tag="AFTER")
    ...
    d.text((W // 2, 758), "New reports, without touching the model.",
