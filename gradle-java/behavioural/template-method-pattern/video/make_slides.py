#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Template Method video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Template Method Pattern  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The abstract class, centred: the only thing that knows the order.
    d.text((880, 232), "ABSTRACT CLASS", font=f(SANS_B, 24), fill=ACCENT, anchor="ma")
    box(d, 560, 270, 640, 110, "FulfilmentProcess",
        "«abstract»   public final fulfil(Order)", ACCENT)

    # The client, left: calls one method and gets a report back.
    d.text((280, 232), "CLIENT", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    box(d, 80, 270, 400, 110, "FulfilmentDemo", "calls fulfil(order)", CLIENT)
    arrow_right(d, 484, 325, 554, CLIENT, 4)

    # The report, right: every step writes itself onto it, which is how the
    # project can show the sequence rather than claim it.
    d.text((1560, 232), "EVIDENCE", font=f(SANS_B, 24), fill=GREEN, anchor="ma")
    box(d, 1280, 270, 560, 110, "FulfilmentReport",
        "every step writes itself here", GREEN)
    arrow_right(d, 1206, 325, 1276, GREEN, 3)

    # The six steps, in the order fulfil calls them, labelled with what kind of
    # hole each one is. This strip is the pattern.
    steps = [
        ("validate", "private", MUTED),
        ("reserveStock", "abstract", ROSE),
        ("charge", "abstract", ROSE),
        ("pack", "default", AMBER),
        ("dispatch", "abstract", ROSE),
        ("notifyCustomer", "default", AMBER),
    ]
    sw, gap, sx = 270, 20, 100
    for i, (label, sub, colour) in enumerate(steps):
        box(d, sx + i * (sw + gap), 425, sw, 96, label, sub, colour,
            label_size=26, sub_size=20)

    # A rail gathering the three routes, with a single arrow up at the strip:
    # every route fills these in, and none of them owns the order.
    d.line([(330, 600), (1590, 600)], fill=LINE, width=3)
    arrow_up(d, 960, 600, 530, LINE, 3)

    routes = [
        ("WarehouseFulfilment", "overrides 4 methods", 80),
        ("MarketplaceFulfilment", "overrides 6 methods", 710),
        ("DigitalFulfilment", "overrides 7 methods", 1340),
    ]
    for label, sub, x in routes:
        box(d, x, 660, 500, 120, label, sub, CYAN)
        d.line([(x + 250, 660), (x + 250, 600)], fill=LINE, width=3)

    d.text((W // 2, 812),
           "Three routes, three different bodies, one identical list of steps.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")
    d.text((W // 2, 862),
           "No route chose that order. fulfil did, and it is final.",
           font=f(SANS, 28), fill=MUTED, anchor="ma")

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 660, 900, 600, 100, "NaiveFulfilment",
        "«the trap»  the sequence, typed three times", AMBER)
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "TEMPLATE", font=f(SANS_B, 128), fill=TEXT)
    ...
    # An order retyped by hand vs. an order the compiler holds for you.
    pill(d, 150, 556, 700, 170, RED, "notify(); dispatch();",
         "an order typed out three times", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "final fulfil(order)",
         "an order no subclass can change", tag="AFTER")
    ...
    d.text((W // 2, 758), "The subclass decides how. The base class decides when.",
