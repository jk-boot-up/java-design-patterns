# Template Method Pattern — Teaching Video

A narrated, slide-based video that teaches the Template Method pattern using
...
| File | What it is |
| --- | --- |
| `template-method-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `template-method-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `template-method-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
| `poster.png` | The opening title card, 1920×1080. Upload it as the video's custom thumbnail. |

**Runtime:** approximately 13 minutes 30 seconds.
...
| 2 | The Scenario | Six steps, three routes, and an order that is not a style choice |
| 3 | The Obvious First Move | Write each route out from beginning to end |
| 4 | The Naive Approach | `NaiveFulfilment` — notify before dispatch, and an email with no key in it |
| 5 | Why That Hurts | Duplication is the complaint; the customer charged and given nothing is the bug |
| 6 | The Pattern | The formal definition, and the phrase "without changing the algorithm's structure" |
| 7 | The Recipe | How to remember it forever — the flavouring, the icing, and the baking |
| 8 | The Roles | The abstract class, the three routes, and the report that proves the claim |
| 9 | The Template Method | `fulfil` — seven calls, and the `final` that makes them an order |
| 10 | Three Kinds of Hole | abstract, default and hook — and why hooks are permissions |
| 11 | Why the Order Matters | `notifyCustomer` reading what `dispatch` wrote, safely and forever |
| 12 | The Proof | `RecordingRoute` — a subclass whose steps only write down that they ran |
| 13 | Running It | Three routes, three bodies, one identical list of step names |
| 14 | Wrap Up | The inheritance slot it spends, and Template Method versus Factory Method and Strategy |
...
**Upload `template-method-pattern-explained.mp4`, with `poster.png` as the
...
`MarketplaceFulfilment`. Uploading `template-method-pattern-explained.srt` gives
