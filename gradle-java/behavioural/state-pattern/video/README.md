# State Pattern — Teaching Video

A narrated, slide-based video that teaches the State pattern using this
...
| File | What it is |
| --- | --- |
| `state-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `state-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `state-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Scenario | The lifecycle, the six requests, and why the answer depends on where the order is |
| 3 | Look Closely at One Row | `cancel` in five states — four different jobs and one refusal |
| 4 | The Naive Approach | `NaiveOrder` — a blocklist, an allowlist, and two drifts |
| 5 | Why That Hurts | The third copy is right, so the button is never drawn and the endpoint takes the call |
| 6 | The Pattern | The formal definition, and the phrase "will appear to change its class" |
| 7 | The Vending Machine | How to remember it forever — and where the condition came from |
| 8 | The Roles | The context, the interface, and seven states that each list what they allow |
| 9 | Refusing By Default | Every request has a body, and every body throws |
| 10 | The Context | One line per method, and `transitionTo` kept package-private |
| 11 | Same Verb, Different Work | `cancel` in `PaidState`, `PackedState` and `ShippedState` |
| 12 | The Proof | A passing test that asserts the bug, paired with the same scenario refused |
| 13 | Running It | A ledger at -£97.49, two refusals with reasons, and the button list per state |
| 14 | Wrap Up | State versus Strategy, and the transition table you gave up to get here |
...
**Upload `state-pattern-explained.mp4`, with `poster.png` as the
...
`IllegalTransitionException`. Uploading `state-pattern-explained.srt` gives
