# Strategy Pattern — Project Specification
...
[`strategy-pattern-explained.md`](strategy-pattern-explained.md); the problem it
...
- Not a shipping-rate engine. The four rules are arithmetic chosen to be checkable by hand, not carrier tariffs.
- Not a claim that the `switch` disappears. It moves to `ShippingRules`, runs once at the edge instead of on every quote, and the written notes say so plainly rather than pretending otherwise.
- Not State. The rules here are chosen from outside and do not decide what comes next; the difference is spelled out in the notes and the video.
...
13 production classes under `com.jk.explore.strategy`:
...
1. **`CheckoutService` contains no branch that depends on which rule it holds.** Not a `switch`, not an `instanceof`, not a name comparison — and the strategy interface must expose nothing that would let it ask.
2. **Each rule reads only the `Shipment` fields it needs** and knows nothing about the others, so its arithmetic can be tested on its own without a checkout.
3. **A rule defined outside the shipped source works unchanged.** A test declares its own `ShippingCostRule` and hands it to `CheckoutService`, which proves the extension point is real rather than a list of four.
4. **Selecting by an unknown name fails loudly.** `ShippingRules.byName` throws and names the rules it knows, because the naive version's silent `default` is the specific bug this project is written to kill.
...
- `./gradlew build` passes. 17 test methods across `CheckoutServiceTest`, `ShippingCostRuleTest`, `ShippingRulesTest`.
...
| `strategy-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/strategy-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Strategy pattern in
...
| Runtime | ~10:07 over 15 scenes, 149 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Strategy Pattern in Java - Shipping Rules"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Observer.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py strategy`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py strategy
python3 ../../docs/make_specs.py strategy
```
