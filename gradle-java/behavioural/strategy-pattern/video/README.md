# Strategy Pattern — Teaching Video

A narrated, slide-based video that teaches the Strategy pattern using this
...
| File | What it is |
| --- | --- |
| `strategy-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `strategy-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `strategy-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Scenario | Four delivery pricing rules for one online shop |
| 3 | The Obvious First Move | Why one method with one `switch` looks like the answer |
| 4 | The Naive Approach | `NaiveCheckoutService.quote` — and the `default` branch that ships for free |
| 5 | Why That Hurts | Five separate costs of branching on a flag |
| 6 | The Pattern | The formal definition, in plain language |
| 7 | Getting Across Town | How to remember it forever |
| 8 | The Three Roles | Context, strategy, concrete strategies — and the selection step |
| 9 | The Strategy | `ShippingCostRule` and the `Shipment` parameter object |
| 10 | A Concrete Strategy | `WeightBandedRule` — bands, and nothing else |
| 11 | The Context | `CheckoutService`, and where the branch actually went |
| 12 | The Proof | A rule defined entirely inside a test the shipped code never saw |
| 13 | Running It | The real program output, every rule against the same shipment |
| 14 | Wrap Up | When to use it, and Strategy versus State |
...
**Upload `strategy-pattern-explained.mp4`, with `poster.png` as the
...
`FreeOverThresholdRule`. Uploading `strategy-pattern-explained.srt` gives
...
> marketing invents a rule, and quietly shipping for free when it meets a
> value it does not recognise — and end with four tiny rule classes behind
> one interface, a checkout service that holds a rule without ever asking
> which one it is, and an honest look at where that switch actually went. No
