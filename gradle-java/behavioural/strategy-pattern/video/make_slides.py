#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Strategy pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Strategy Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("pass in"):
...
def kind_diagram(scene, img, d):
    ...
    # The strategy interface, centred: the one type the context depends on.
    d.text((960, 250), "STRATEGY", font=f(SANS_B, 24), fill=GREEN, anchor="ma")
    box(d, 700, 290, 520, 100, "ShippingCostRule",
        "«interface»  costFor(Shipment)", GREEN)

    # The context, left: holds one rule, and cannot ask which one it is.
    d.text((270, 250), "CONTEXT", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    box(d, 80, 290, 380, 100, "CheckoutService", "holds one rule", CLIENT)
    arrow(d, 460, 340, 690, 340, CLIENT, 4)
    d.text((575, 296), "one field", font=f(SANS_B, 22), fill=CLIENT, anchor="ma")

    # Selection, right: the last remaining branch, run once at the edge.
    d.text((1650, 250), "SELECTION", font=f(SANS_B, 24), fill=AMBER, anchor="ma")
    box(d, 1440, 290, 420, 100, "ShippingRules", "config name -> rule, once", AMBER)
    arrow(d, 1440, 340, 1230, 340, AMBER, 3)

    # Four concrete strategies, peers, each implementing the interface.
    # Left-anchored, clear of the fan of arrows converging on the interface.
    d.text((60, 462), "CONCRETE STRATEGIES", font=f(SANS_B, 24), fill=ROSE)
    d.text((60, 494), "four peers, one interface", font=f(SANS, 22), fill=MUTED)
    labels = [
        ("FlatRateRule", "reads nothing", 60),
        ("WeightBandedRule", "reads weightKg", 530),
        ("DistanceBasedRule", "reads distanceMiles", 1000),
        ("FreeOverThresholdRule", "reads orderSubtotal", 1470),
    ]
    ...
    d.text((W // 2, 730),
           "CheckoutService has no branch that depends on which rule it holds.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")
    d.text((W // 2, 786),
           "There is no message it could send to find out -- so a fifth rule cannot break it.",
           font=f(SANS, 28), fill=MUTED, anchor="ma")

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 660, 858, 600, 110, "NaiveCheckoutService",
        "«the trap»  one switch, four rules", AMBER)
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "STRATEGY", font=f(SANS_B, 128), fill=TEXT)
    ...
    # A switch over a mode flag vs. the behaviour handed in as an object.
    pill(d, 150, 556, 700, 170, RED, "switch (method) { ... }",
         "four rules in one method", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "rule.costFor(shipment)",
         "one call, any rule, no branch", tag="AFTER")
    ...
    d.text((W // 2, 758), "Pass in the behaviour, don't branch on a flag.",
