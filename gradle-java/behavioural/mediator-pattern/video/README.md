# Mediator Pattern — Teaching Video

A narrated, slide-based video that teaches the Mediator pattern using this
...
| File | What it is |
| --- | --- |
| `mediator-pattern-explained.mp4` | The video — 1920×1080, H.264, 30 fps, stereo AAC. Upload-ready for YouTube. |
| `mediator-pattern-explained.m4a` | Audio-only version, for listening on the move or for revision. |
| `mediator-pattern-explained.srt` | Subtitles, timed from the encoded scene clips. Upload alongside the video for accurate captions. |
...
| 2 | The Scenario | The checkout page: five controls, and the four rules that tie them together |
| 3 | Look Closely at One Click | Changing the country withdraws gift wrap but leaves the tick — £2 for wrapping that will not happen |
| 4 | The Naive Approach | `NaiveCheckoutForm` — nine references between five controls, and a forgotten button re-check |
| 5 | Why That Hurts | No file holds the answer, the bugs are omissions, the wiring grows as n², nothing testable alone |
| 6 | The Pattern | The formal definition, and why "encapsulates how they interact" matters more than the reference count |
| 7 | An Analogy | Aircraft and the control tower — n² conversations against n |
| 8 | The Roles | `CheckoutMediator`, `FormWidget`, the concrete form and its five colleagues |
| 9 | The Colleague | `FormWidget` — two fields, and the field it deliberately does not have |
| 10 | The Mediator | `changed()` — one branch, and two refreshes that run every time |
| 11 | The Tests | Asserting the structure by reflection, and pinning the naive form's wrong answer on purpose |
| 12 | Running It | The two forms side by side: £42 and enabled, against £40 and disabled |
| 13 | Wrap Up | Mediator against Observer and Facade, the god-object cost, and when not to reach for it |
...
**Upload `mediator-pattern-explained.mp4`, with `poster.png` as the
...
`GiftWrapCheckbox`. Uploading `mediator-pattern-explained.srt` gives
