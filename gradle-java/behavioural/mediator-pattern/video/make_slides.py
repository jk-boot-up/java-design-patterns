#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Mediator video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Mediator  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The mediator interface sits at the top, on its own, because its size is
    # the surprise: one method is genuinely all a mediator needs.
    box(d, 660, 224, 600, 96, "CheckoutMediator",
        "«mediator»   changed(FormWidget source)", ACCENT,
        label_size=30, sub_size=22)

    # The colleague base class sits to the left at the same height as the
    # concrete mediator, and its subtitle is the point of the whole slide: the
    # field it does NOT have is what makes the tangle unrepresentable.
    box(d, 90, 412, 500, 150, "FormWidget",
        "«colleague»   a name and a mediator — nothing else", CYAN,
        label_size=30, sub_size=21)

    box(d, 660, 412, 600, 150, "CheckoutForm",
        "the rules of the page live here, and only here", GREEN,
        label_size=32, sub_size=22)
    arrow(d, 960, 404, 960, 328, ACCENT, 3)

    # Left to right: a widget's single reference. Drawn as one arrow because
    # there is only ever one.
    arrow_right(d, 594, 480, 654, MUTED, 3)
    d.text((624, 428), "mediator", font=f(SANS_B, 22), fill=MUTED, anchor="ma")

    # The client is the demo, tucked top right, out of the star.
    d.text((1600, 372), "CLIENT", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    box(d, 1440, 412, 320, 150, "CheckoutFormDemo",
        "builds one form, clicks", CLIENT, label_size=24, sub_size=20)
    arrow_right(d, 1436, 480, 1266, CLIENT, 3)

    # The five colleagues along the bottom, joined to the form by a single bus
    # line rather than five crossing arrows: the shape is the message, and five
    # separate diagonals would obscure exactly the thing worth seeing.
    widgets = [
        ("CountrySelector", "where it goes", GREEN),
        ("ShippingSelector", "options + choice", GREEN),
        ("GiftWrapCheckbox", "offered, ticked", GREEN),
        ("TotalLabel", "read-only", LINE),
        ("PlaceOrderButton", "enabled?", LINE),
    ]
    pw, gap, px = 330, 30, 75
    for i, (label, sub, colour) in enumerate(widgets):
        x = px + i * (pw + gap)
        box(d, x, 700, pw, 100, label, sub, colour, label_size=25, sub_size=20)
        d.line([(x + pw // 2, 700), (x + pw // 2, 648)], fill=LINE, width=3)
    d.line([(240, 648), (1680, 648)], fill=LINE, width=3)
    arrow_up(d, 960, 648, 570, GREEN, 4)
    d.text((1000, 592), "each one holds the form", font=f(SANS_B, 22),
           fill=GREEN, anchor="la")
    d.text((240, 664), "all five extend FormWidget", font=f(SANS, 22),
           fill=MUTED, anchor="la")

    d.text((W // 2, 828),
           "Five arrows, all pointing the same way.  A star, not a web.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 620, 890, 680, 92, "NaiveCheckoutForm",
        "«the trap»  nine references, and two of them mutual", RED,
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 288), "MEDIATOR", font=f(SANS_B, 122), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, "country.wire(a, b, c, d)",
         "5 controls, 9 references", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "mediator.changed(this)",
         "5 controls, 5 references", tag="AFTER")
    ...
    d.text((W // 2, 744), "Aircraft don't negotiate with each other.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "They all talk to the tower.",
