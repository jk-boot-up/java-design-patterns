# Interpreter Pattern — Project Specification
...
[`interpreter-pattern-explained.md`](interpreter-pattern-explained.md); the problem it
...
- Not a parser tutorial. `RuleParser` is two string splits and a `throw`, it is labelled as not part of the pattern everywhere it appears, and the one exercise that asks for brackets is deliberately unfinishable so the limit is felt rather than asserted.
- Not a rules engine, and not an argument against one. The notes say plainly that production shops often buy one, and that this project is what such a thing looks like on the inside.
- Not Composite, although it is built from one. The comparison section states that the difference is intent rather than structure: Composite is about treating one thing and many things alike, Interpreter is about meaning.
- Not a claim that an `if` is wrong. Three settled promotions are better written as three branches, and the explainer says so before it says anything else.
...
14 production classes under `com.jk.explore.interpreter`:

| Role | Types |
| --- | --- |
| Abstract expression | `Rule` — two methods, `matches(Order)` and `describe()`, no fields |
| Terminal expressions | `BasketOver`, `CountryIs`, `ItemsAtLeast`, `FirstOrder` — one comparison each, and no rule inside |
| Non-terminal expressions | `AndRule`, `OrRule`, `NotRule` — hold other rules and answer by asking them |
| Context | `Order` — basket total, country, item count, first order |
| Client-side parser | `RuleParser` — builds the tree, and is not part of the pattern |
| Holder | `Promotion` and `PromotionBook` — a code, a percentage, and a rule as text |
| Naive alternative | `NaiveVoucherRules` — one branch per offer, copied from the one above |
| Entry point | `VoucherRuleDemo` |

### Requirements

1. **Two kinds of class, and no third.** Every rule implements `Rule`; the terminals hold values and the non-terminals hold rules. The class diagram shows exactly one aggregation looping back into `Rule`, and that loop is the whole of the recursion.
2. **Only the leaves read the context.** `AndRule` and `OrRule` never touch an `Order`; they ask their parts and combine the answers, which is why a whole other tree drops into either slot without a line changing.
3. **A rule can say what it says.** `describe()` walks the same nodes as `matches` and rebuilds the sentence from the objects rather than remembering the line that was read in, and `parsingAndDescribingRoundTrip` asserts it — so the audit log and the tree the checkout obeys cannot drift apart.
4. **The grammar is asserted, not assumed.** `andBindsTighterThanOr` pins the precedence that falls out of splitting on `or` first, so the exercise that swaps the two splits fails the build instead of quietly changing what every promotion in the shop means.
5. **A typo is refused on Wednesday.** `RuleParser` does not guess: it names the phrase it cannot read and throws while the promotion is being saved. The contrast is stated explicitly — a mistyped Java condition is valid Java, and misprices an order on Friday.
6. **Adding a condition touches nothing that exists.** A new terminal is one record and one `if` in the parser; the session guide has the room do it and then asks what the same change would have been in `NaiveVoucherRules`.
7. **No fact about eligibility lives in code.** `PromotionBook` mentions no country, no basket threshold and no item count; every such fact is a line of text, which is the point of the whole exercise.
8. **The naive alternative is argued against honestly.** Both of its bugs are pinned by *passing* tests that assert the wrong answers, so the cost is stated by the build rather than claimed by a README.
9. **The limit is stated as plainly as the benefit.** One class per phrase is fine for seven phrases and unbearable for seventy; the parser is the part that grows; every node is an object and every evaluation a virtual call; and when the rules never change, two `if` statements are the better code.
10. **Java 21, no third-party runtime dependencies.** JUnit 5 for tests
   only, so the project is readable by someone who does not know a DI
   framework.
11. **Every class fits on a slide.** This is teaching code; a class that
   needs scrolling to read has failed its purpose regardless of its
   design.

### Verification

- `./gradlew build` passes. 21 test methods across `NaiveVoucherRulesTest`, `PromotionBookTest`, `RuleParserTest`, `RuleTest`.
...
| `interpreter-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/interpreter-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Interpreter pattern in
...
| Runtime | ~13:17 over 14 scenes, 190 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Interpreter Pattern in Java - Promotion Rules"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: API Gateway.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py interpreter`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py interpreter
python3 ../../docs/make_specs.py interpreter
```
