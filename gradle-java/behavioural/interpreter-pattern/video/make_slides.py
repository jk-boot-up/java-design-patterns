#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Interpreter video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Interpreter  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The interface on top, and underneath it the only two kinds of class there
    # are. The layout is the lesson: both sides implement the same thing, and
    # only the right-hand side has an arrow back up into it.
    box(d, 610, 224, 700, 140, "Rule",
        "«abstract expression»   matches(Order)  ·  describe()", AMBER,
        label_size=40, sub_size=24)

    d.text((230, 466), "TERMINAL EXPRESSIONS", font=f(SANS_B, 24),
           fill=GREEN, anchor="ma")
    box(d, 70, 504, 820, 170, "BasketOver · CountryIs · ItemsAtLeast · FirstOrder",
        "a leaf: one comparison, and no rule inside it", GREEN,
        label_size=30, sub_size=24)

    d.text((1650, 466), "NON-TERMINAL EXPRESSIONS", font=f(SANS_B, 24),
           fill=CYAN, anchor="ma")
    box(d, 1030, 504, 820, 170, "AndRule  ·  OrRule  ·  NotRule",
        "hold other rules, and answer by asking them", CYAN,
        label_size=36, sub_size=24)

    # The labels sit clear of the diagonals rather than on them: at this size a
    # word drawn over a line reads as a break in the line.
    arrow_diag_up(d, 480, 500, 700, 368, GREEN, 4)
    d.text((470, 414), "implements", font=f(SANS, 22), fill=GREEN, anchor="ra")
    arrow_diag_up(d, 1440, 500, 1220, 368, CYAN, 4)
    d.text((1450, 414), "implements", font=f(SANS, 22), fill=CYAN, anchor="la")

    # The one loop in the picture, drawn as an elbow so it cannot be mistaken
    # for another "implements": a connective holds Rules, which is what lets a
    # tree be any shape and any depth without a line of it changing.
    d.line([(1850, 560), (1890, 560)], fill=AMBER, width=4)
    d.line([(1890, 560), (1890, 294)], fill=AMBER, width=4)
    arrow_left(d, 1890, 294, 1314, AMBER, 4)
    d.text((1600, 250), "holds many Rules  —  the recursion",
           font=f(SANS_B, 24), fill=AMBER, anchor="ma")

    d.text((W // 2, 706),
           "Two kinds of class.  Only one of them can hold another rule.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")

    box(d, 70, 766, 820, 110, "Order",
        "«context»   basketPounds · country · itemCount · firstOrder", CLIENT,
        label_size=30, sub_size=22)
    arrow(d, 180, 678, 180, 760, GREEN, 4)

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 1030, 766, 820, 110, "NaiveVoucherRules",
        "«the trap»  every offer a branch, copied from the one above", RED,
        label_size=30, sub_size=22)

    d.text((480, 906), "only the leaves ever read the context",
           font=f(SANS, 24), fill=MUTED, anchor="ma")
    d.text((1440, 906), "RuleParser is not on this slide, and not part of the pattern",
           font=f(SANS, 24), fill=MUTED, anchor="ma")
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 288), "INTERPRETER", font=f(SANS_B, 122), fill=ACCENT)
    ...
    pill(d, 150, 556, 700, 170, RED, "if (basket > 100)",
         "a release, to change a number", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "\"basket over 100\"",
         "a line of text, edited by anyone", tag="AFTER")
    ...
    d.text((W // 2, 744), "A rule written in code can only be run.",
           font=f(SANS_B, 48), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "A rule written in words can be read.",
