#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Chain of Responsibility video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Chain of Responsibility  ·  Java 21",
...
def kind_diagram(scene, img, d):
    ...
    # The wiring, centred: it links the handlers and names the fallback.
    d.text((880, 232), "THE WIRING", font=f(SANS_B, 24), fill=ACCENT, anchor="ma")
    box(d, 560, 270, 640, 110, "ScreeningChain",
        "links the handlers, names the fallback", ACCENT)

    # The client, left: asks once, and does not know how many links there are.
    d.text((280, 232), "CLIENT", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    box(d, 80, 270, 400, 110, "CheckoutScreeningDemo",
        "chain.screen(request)", CLIENT)
    arrow_right(d, 484, 325, 554, CLIENT, 4)

    # The evidence, right: a behavioural pattern has to show its working.
    d.text((1560, 232), "EVIDENCE", font=f(SANS_B, 24), fill=GREEN, anchor="ma")
    box(d, 1280, 270, 560, 110, "ScreeningReport",
        "the decision, who made it, and what never ran", GREEN)
    arrow_right(d, 1206, 325, 1276, GREEN, 3)

    # The abstract link. One field, one method to write, and one walk that
    # subclasses cannot override.
    box(d, 460, 425, 1000, 100, "ScreeningHandler",
        "«abstract»   check() -> Optional<Decision>,  and a final walk", ROSE,
        label_size=28, sub_size=22)
    arrow(d, 960, 385, 960, 421, ACCENT, 3)

    # A rail gathering the four links, with one arrow up at the base class.
    d.line([(224, 600), (1328, 600)], fill=LINE, width=3)
    arrow_up(d, 960, 600, 531, LINE, 3)

    links = [
        ("address", "country, postcode", CYAN),
        ("stock", "can we pick it?", CYAN),
        ("fraud-score", "reject / refer / pass", CYAN),
        ("payment-limit", "does the card cover it?", CYAN),
    ]
    sw, gap, sx = 320, 48, 64
    for i, (label, sub, colour) in enumerate(links):
        x = sx + i * (sw + gap)
        box(d, x, 650, sw, 120, label, sub, colour, label_size=26, sub_size=20)
        d.line([(x + sw // 2, 650), (x + sw // 2, 600)], fill=LINE, width=3)
        if i:
            arrow_right(d, x - gap, 710, x - 4, LINE, 3)

    # The end of the chain. It is a box on this diagram because falling off the
    # end is a case somebody had to decide, not an absence.
    fx = sx + 4 * (sw + gap)
    arrow_right(d, fx - gap, 710, fx - 4, LINE, 3)
    box(d, fx, 650, sw, 120, "end of chain",
        "the fallback, named in the wiring", AMBER, label_size=26, sub_size=20)

    d.text((W // 2, 800),
           "Each link answers, or says nothing and passes it on.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")
    d.text((W // 2, 848),
           "No link knows the order. Reordering the policy edits none of them.",
           font=f(SANS, 28), fill=MUTED, anchor="ma")

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 620, 896, 680, 100, "NaiveScreening",
        "«the trap»  four checks welded into one method", RED)
...
def kind_poster(scene, img, d):
    ...
    # Two lines rather than three, so the type stays at poster size: the name
    # is long, and shrinking it to fit "OF RESPONSIBILITY" on its own line
    # costs more legibility at thumbnail size than the extra line buys.
    d.text((110, 152), "CHAIN OF", font=f(SANS_B, 122), fill=TEXT)
    d.text((110, 288), "RESPONSIBILITY", font=f(SANS_B, 122), fill=ACCENT)
    ...
    # An order welded into a method vs. an order you can hold in your hand.
    pill(d, 150, 556, 700, 170, RED, "if (bad) return no();",
         "the order is welded in", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "next.screen(request)",
         "one check per class, and it may stop", tag="AFTER")
    ...
    d.text((W // 2, 758), "Make the order of your checks a value, not a line.",
