#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Observer pattern video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Observer Pattern  ·  Java 21",
...
def kind_quote(scene, img, d):
    ...
    for ln in scene["body"]:
        ...
        elif ln.startswith("In plain") or ln.startswith("let the thing"):
...
def kind_diagram(scene, img, d):
    ...
    # The observer interface, centred: the one type the subject depends on.
    d.text((960, 250), "OBSERVER", font=f(SANS_B, 24), fill=GREEN, anchor="ma")
    box(d, 700, 290, 520, 100, "OrderListener",
        "«interface»  onStatusChanged(OrderEvent)", GREEN)

    # The subject, left: holds a list, and cannot ask what is on it.
    d.text((270, 250), "SUBJECT", font=f(SANS_B, 24), fill=CLIENT, anchor="ma")
    box(d, 80, 290, 380, 100, "Order", "holds a list of them", CLIENT)
    arrow(d, 460, 340, 690, 340, CLIENT, 4)
    d.text((575, 296), "0..*", font=f(SANS_B, 22), fill=CLIENT, anchor="ma")

    # The event, right: what travels, and the fact that it travels one way.
    d.text((1650, 250), "EVENT", font=f(SANS_B, 24), fill=AMBER, anchor="ma")
    box(d, 1440, 290, 420, 100, "OrderEvent", "id, from, to  --  a value", AMBER)
    arrow(d, 1440, 340, 1230, 340, AMBER, 3)

    # Four concrete observers, peers, each implementing the interface.
    # Left-anchored, clear of the fan of arrows converging on the interface.
    # Sits above the fan of inheritance arrows, which reach y=500 by x=400.
    d.text((60, 424), "CONCRETE OBSERVERS", font=f(SANS_B, 24), fill=ROSE)
    d.text((60, 456), "peers -- none knows the others", font=f(SANS, 22), fill=MUTED)
    labels = [
        ("InventoryListener", "SHIPPED, CANCELLED", 60),
        ("EmailListener", "every event", 530),
        ("AnalyticsListener", "every event", 1000),
        ("WarehouseFeedListener", "PAID, SHIPPED", 1470),
    ]
    ...
    d.text((W // 2, 730),
           "Order has no field called email, inventory, analytics or warehouse.",
           font=f(SANS_B, 34), fill=TEXT, anchor="ma")
    d.text((W // 2, 786),
           "There is no message it could send to find out who is listening.",
           font=f(SANS, 28), fill=MUTED, anchor="ma")

    # The rejected alternative, kept on screen so the contrast is explicit.
    box(d, 660, 858, 600, 110, "NaiveOrderService",
        "«the trap»  four calls, no try", AMBER)
...
def kind_poster(scene, img, d):
    ...
    d.text((110, 156), "OBSERVER", font=f(SANS_B, 128), fill=TEXT)
    ...
    # Four systems called by name vs. one announcement anyone can subscribe to.
    pill(d, 150, 556, 700, 170, RED, "email.notify(order)",
         "four systems, called by name", tag="BEFORE")
    pill(d, 1070, 556, 700, 170, GREEN, "order.moveTo(SHIPPED)",
         "one announcement, any audience", tag="AFTER")
    ...
    d.text((W // 2, 758), "Announce it once. Let whoever cares sign up.",
