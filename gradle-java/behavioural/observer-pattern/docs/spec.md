# Observer Pattern — Project Specification
...
[`observer-pattern-explained.md`](observer-pattern-explained.md); the problem it
...
- Not a message broker. Notification is synchronous, in-process and in-order; there is no queue, no delivery guarantee and no retry.
- Not a demonstration of `java.util.Observer`. That API was deprecated in Java 9 for reasons the notes explain; the project writes its own interface, which is what modern code does.
- Not thread-safety. `CopyOnWriteArrayList` is used so a listener can unsubscribe itself *during* a notification, and the notes say so, because the usual assumption is that it is there for concurrency.
- Not Mediator. The subject here coordinates nothing; the distinction is drawn explicitly in the notes and the video.
...
11 production classes under `com.jk.explore.observer`:
...
1. **`Order` names none of its listeners.** Searching the class for `email`, `inventory` or `warehouse` must find nothing, and the listener interface must expose nothing that would let `Order` tell them apart.
2. **One listener throwing does not stop the rest.** The `try`/`catch` sits inside the notification loop and every failure comes back as a named `ListenerFailure`, because the outage in the naive version is the specific bug this project is written to kill.
3. **A listener defined outside the shipped source works unchanged.** A test declares its own `OrderListener` and attaches it, which proves the extension point is real rather than a list of four.
4. **The status is updated before the listeners run, and an unchanged status announces nothing**, so a listener sees the world the event describes and no duplicate transition is ever delivered.
5. **A listener can remove itself while being notified.** The one-shot subscription is a normal case, not an error, and a test pins it.
...
- `./gradlew build` passes. 27 test methods across `NaiveOrderServiceTest`, `OrderListenerTest`, `OrderTest`.
...
| `observer-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/observer-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Observer pattern in
...
| Runtime | ~11:37 over 15 scenes, 168 subtitle cues |  |
...
1. **Title** — the exact string, ≤ 60 characters so search does not
   truncate it, leading with the pattern name. Currently *"Observer Pattern in Java - Order Status Events"*,
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Command.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py observer`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py observer
python3 ../../docs/make_specs.py observer
```
