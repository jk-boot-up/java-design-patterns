#!/usr/bin/env python3
"""Render one 1920x1080 PNG slide per scene for the Observer with Spring video."""
...
def footer(d, label):
    ...
    d.text((W - 70, H - 58), "Observer with Spring  ·  Java 21",
...
def kind_poster(scene, img, d):
    ...
    # One failing listener versus the rest -- both real lines from the project.
    pill(d, 150, 556, 700, 170, RED, "email throws: mail server timed out",
         "analytics never heard", tag="STOPPED")
    pill(d, 1070, 556, 700, 170, GREEN, "inventory, email, analytics",
         "all heard, in order", tag="DELIVERED")
    ...
    d.text((W // 2, 744), "The order knows no one.",
           font=f(SANS_B, 44), fill=TEXT, anchor="ma")
    d.text((W // 2, 796), "Delivery is synchronous.",
