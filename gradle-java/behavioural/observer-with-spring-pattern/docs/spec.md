# Observer with Spring Pattern — Project Specification
...
[`observer-with-spring-pattern-explained.md`](observer-with-spring-pattern-explained.md); the problem it
...
11 production classes under `com.jk.explore.observerspring`:
...
1. **Order is asserted** with `@Order` and a journal.
2. **The other thread is held at a gate,** never slept on.
3. **A missing listener is asserted** as a silent no-op.
...
- `./gradlew build` passes. 6 test methods across `DemoRunsTest`, `ObserverBeanTest`.
...
| `observer-with-spring-pattern-explained.md` | The pattern, the code, pitfalls, comparisons |
...
The finished video is `video/observer-with-spring-pattern-explained.mp4`, with an
...
1. **What the video is** — "This video explains the Observer with Spring pattern in
...
| Runtime | ~4:58 over 16 scenes, 72 subtitle cues |  |
...
7. **Cards and end screen** — which video comes next in the learning
   order. For this project: Strategy with Spring.

> **Requirement.** Chapter timings are generated from the built `.srt`,
> never written by hand, by
> `python3 ../../docs/make_youtube_docs.py observer-with-spring`. They are the one part of
...
```bash
python3 ../../docs/make_youtube_docs.py observer-with-spring
python3 ../../docs/make_specs.py observer-with-spring
```
